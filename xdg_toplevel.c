...
RunOnUnmap(XdgToplevel *toplevel, void (*unmap)(void *), void *data)
{
...
{
	XChangeProperty(compositor.display, XLWindowFromXdgRole(toplevel->role),
...
DestroyBacking(XdgToplevel *toplevel)
{
	...
	XLListFree(toplevel->resize_callbacks, XLSeatCancelResizeCallback);
...
#define ClientSide ZXDG_TOPLEVEL_DECORATION_V1_MODE_CLIENT_SIDE
	...
	if (toplevel->decor == DecorationModeClient)
		zxdg_toplevel_decoration_v1_send_configure(
		    toplevel->decoration->resource, ClientSide);
	else
		zxdg_toplevel_decoration_v1_send_configure(
		    toplevel->decoration->resource, ServerSide);
...
static void
SendConfigure(XdgToplevel *toplevel, unsigned int width, unsigned int height)
{
	uint32_t serial;
...
NoteConfigureTime(Timer *timer, void *data, struct timespec time)
{
	...
	if (!(toplevel->state & StatePendingConfigureSize))
		...
		TruncateScaleToSurface(toplevel->role->surface, effective_width,
		    effective_height, &effective_width, &effective_height);

		/* Compute the geometry for the configure event based on the
		   current size of the toplevel.  */
		XLXdgRoleCalcNewWindowSize(toplevel->role, effective_width,
		    effective_height, &width, &height);
...
static void
CurrentWindowGeometry(XdgToplevel *toplevel, int *width, int *height)
{
	/* Calculate the current window geometry for sending a configure
	   event.  */

	TruncateScaleToSurface(toplevel->role->surface, toplevel->width,
	    toplevel->height, width, height);

	XLXdgRoleCalcNewWindowSize(toplevel->role, *width, *height, width,
	    height);
}
...
RecordStateSize(XdgToplevel *toplevel)
{
	...
	if (XLWmSupportsHint(_GTK_FRAME_EXTENTS)) {
		/* Note that if _GTK_FRAME_EXTENTS is supported, the window
		   ...
		   upon minimization.  */
		XLXdgRoleGetCurrentGeometry(toplevel->role, NULL, NULL, &width,
		    &height);
...
HandleWmStateChange(XdgToplevel *toplevel)
{
	...
	rc = XGetWindowProperty(compositor.display, window, _NET_WM_STATE, 0,
	    65536, False, XA_ATOM, &actual_type, &actual_format, &actual_size,
	    &bytes_remaining, &tmp_data);

	if (rc != Success || !tmp_data || actual_type != XA_ATOM ||
	    actual_format != 32 || bytes_remaining)
		goto empty_states;
	...
	if (memcmp(&old, &state, sizeof *state) &&
	    !MaybePostDelayedConfigure(toplevel, StatePendingConfigureStates))
...
HandleAllowedActionsChange(XdgToplevel *toplevel)
{
	...
	rc = XGetWindowProperty(compositor.display, window,
	    _NET_WM_ALLOWED_ACTIONS, 0, 65536, False, XA_ATOM, &actual_type,
	    &actual_format, &actual_size, &bytes_remaining, &tmp_data);
...
static void
ApplyGtkFrameExtents(XdgToplevel *toplevel, int x, int y, int x2, int y2)
{
	long   cardinals[4];
	Window window;
	...
	XChangeProperty(compositor.display, window, _GTK_FRAME_EXTENTS,
	    XA_CARDINAL, 32, PropModeReplace, (unsigned char *)cardinals, 4);
}
...
HandleWindowGeometryChange(XdgToplevel *toplevel)
{
	...
	XLXdgRoleGetCurrentGeometry(toplevel->role, &x, &y, &width, &height);
	TruncateScaleToWindow(toplevel->role->surface, width, height, &width,
	    &height);
	...
	TruncateScaleToWindow(toplevel->role->surface, toplevel->min_width,
	    toplevel->min_height, &hints->min_width, &hints->min_height);
	...
	if (toplevel->max_width) {
		...
		TruncateScaleToWindow(toplevel->role->surface,
		    toplevel->max_width, toplevel->max_height,
	...
	XSetWMNormalHints(compositor.display,
	    XLWindowFromXdgRole(toplevel->role), hints);
}
...
GetClientMachine(XTextProperty *client_machine)
{
	...
	if (host_name_max == -1)
		/* The maximum host name is indeterminate.  Use a sane limit
		   like _POSIX_HOST_NAME_MAX.  */
...
WriteCredentialProperties(XdgToplevel *toplevel)
{
	...
	window	   = XLWindowFromXdgRole(toplevel->role);
	process_id = pid;
	XChangeProperty(compositor.display, window, _NET_WM_PID, XA_CARDINAL,
	    32, PropModeReplace, (unsigned char *)&process_id, 1);

	/* First, let Xlib write WM_CLIENT_MACHINE and WM_LOCALE_NAME.  */
	XSetWMProperties(compositor.display, window, NULL, NULL, NULL, 0, NULL,
	    NULL, NULL);
...
Attach(Role *role, XdgRoleImplementation *impl)
{
	...
	XSetWMProtocols(compositor.display, window, protocols, nproto);
...
Detach(Role *role, XdgRoleImplementation *impl)
{
	...
	XSetWMProtocols(compositor.display, XLWindowFromXdgRole(role), NULL, 0);
...
Unmap(XdgToplevel *toplevel)
{
	...
	memset(&toplevel->toplevel_state, 0, sizeof toplevel->toplevel_state);
	...
	XSetWMNormalHints(compositor.display, window, &toplevel->size_hints);
...
Map(XdgToplevel *toplevel)
{
	...
	/* Now, map the window.  */
	XMapWindow(compositor.display, XLWindowFromXdgRole(toplevel->role));
	...
	toplevel->state &= ~(StateMaximizeOnMap | StateUnmaximizeOnMap |
	    StateFullscreenOnMap | StateUnfullscreenOnMap);
}
...
SendOutputBounds(XdgToplevel *toplevel)
{
	...
	TruncateScaleToSurface(toplevel->role->surface, x_max - x_min + 1,
	    y_max - y_min + 1, &width, &height);
...
Commit(Role *role, Surface *surface, XdgRoleImplementation *impl)
{
	...
	if (!surface->current_state.buffer
		...
		if (wl_resource_get_version(toplevel->resource) >= 4)
			/* Send the maximum bounds of the window to the client.
			   It isn't possible to predict where the window will be
			   mapped, so unfortunately the precise output bounds
			   can't be used here.  */
			SendOutputBounds(toplevel);
...
AfterCommit(Role *role, Surface *surface, XdgRoleImplementation *impl)
{
	...
	if (!toplevel->conf_reply &&
		...
		XLXdgRoleMoveBy(role, toplevel->ack_west, toplevel->ack_north);
		...
		if (toplevel->state & StatePendingResize)
			PostResize1(toplevel, toplevel->resize_west,
			    toplevel->resize_north, toplevel->resize_width,
...
HandleConfigureEvent(XdgToplevel *toplevel, XEvent *event)
{
	...
	/* Also set the bounds width and height to avoid resizing the
	   window.  */
	XLXdgRoleSetBoundsSize(toplevel->role, toplevel->width,
	    toplevel->height);
	...
	if (!MaybePostDelayedConfigure(toplevel, StatePendingConfigureSize)) {
		...
		TruncateScaleToSurface(toplevel->role->surface,
		    ...
		    &configure_width, &configure_height);

		/* Calculate the new window size.  */
		XLXdgRoleCalcNewWindowSize(toplevel->role, configure_width,
		    configure_height, &width, &height);
    ...
    XPointer arg, struct timespec timeout)
{
	...
	/* Check if an event is already in the queue.  If it is, avoid
	   syncing.  */
	if (XCheckIfEvent(compositor.display, event_return, predicate, arg))
		return 0;
	...
	while (true) {
		...
		if (XCheckIfEvent(compositor.display, event_return, predicate,
			arg))
			return 0;
...
static void
NoteSize(Role *role, XdgRoleImplementation *impl, int width, int height)
{
	XdgToplevel *toplevel;
...
static void
NoteWindowPreResize(Role *role, XdgRoleImplementation *impl, int width,
    int height)
{
	...
	XLXdgRoleGetCurrentGeometry(toplevel->role, &x, &y, &gwidth, &gheight);

	/* Scale the window geometry to window dimensions.  */
	TruncateScaleToWindow(toplevel->role->surface, gwidth, gheight, &gwidth,
	    &gheight);
	TruncateSurfaceToWindow(toplevel->role->surface, x, y, &x, &y);
...
static void
NoteWindowResized(Role *role, XdgRoleImplementation *impl, int width,
    int height)
{
	XEvent	     event;
	int	     rc;
    ...
    int north_motion, int new_width, int new_height)
{
	...
	if (toplevel->role->surface)
		TruncateScaleToSurface(toplevel->role->surface, new_width,
...
Destroy(struct wl_client *client, struct wl_resource *resource)
{
	...
	if (toplevel->role)
		XLXdgRoleDetachImplementation(toplevel->role, &toplevel->impl);
...
UpdateWmTransientForProperty(XdgToplevel *child)
{
	...
	if (!child->transient_for)
		XDeleteProperty(compositor.display, window, WM_TRANSIENT_FOR);
	else
		XChangeProperty(compositor.display, window, WM_TRANSIENT_FOR,
		    XA_WINDOW, 32, PropModeReplace, (unsigned char *)&parent,
		    1);
}
...
UpdateParent(XdgToplevel *child, XdgToplevel *parent)
{
	...
	if (parent) {
		...
		child->parent_callback = RunOnUnmap(parent,
		    HandleParentUnmapped, child);
	}
...
ConvertWmName(XdgToplevel *toplevel, const char *title)
{
	...
	XChangeProperty(compositor.display, XLWindowFromXdgRole(toplevel->role),
	    WM_NAME, XA_STRING, 8, PropModeReplace, (unsigned char *)outbuf,
    ...
    const char *app_id)
{
	...
	XSetClassHint(compositor.display, XLWindowFromXdgRole(toplevel->role),
	    &class_hints);
}
...
HandleResizeDone(void *key, void *data)
{
	...
	toplevel		   = data;
	toplevel->resize_callbacks = XLListRemove(toplevel->resize_callbacks,
	    key);
    ...
    struct wl_resource *seat_resource, uint32_t serial, uint32_t edges)
{
	...
	ok = XLResizeToplevel(seat, toplevel->role->surface, serial, edges);

	if (!ok)
		return;

	/* Now set up the special resizing state.  */
	callback_key = XLSeatRunAfterResize(seat, HandleResizeDone, toplevel);
    ...
    int32_t width, int32_t height)
{
	...
	if (width < 0 || height < 0) {
		wl_resource_post_error(resource,
		    XDG_TOPLEVEL_ERROR_INVALID_SIZE, "invalid max size %d %d",
		    width, height);
		return;
	}
    ...
    int32_t width, int32_t height)
{
	...
	if (width < 0 || height < 0) {
		wl_resource_post_error(resource,
		    XDG_TOPLEVEL_ERROR_INVALID_SIZE, "invalid min size %d %d",
		    width, height);
		return;
	}
...
SetMinimized(struct wl_client *client, struct wl_resource *resource)
{
	...
	XIconifyWindow(compositor.display, XLWindowFromXdgRole(toplevel->role),
...
ReplyToPing(XEvent *event)
{
	...
	XSendEvent(compositor.display, copy.xclient.window, False,
	    (SubstructureRedirectMask | SubstructureNotifyMask), &copy);
}
...
NoteFocus(Role *role, XdgRoleImplementation *impl, FocusMode mode)
{
	...
	case SurfaceFocusOut:
		toplevel->focus_seat_count = MAX(toplevel->focus_seat_count - 1,
		    0);
		break;
	}
...
static void
Activate(Role *role, XdgRoleImplementation *impl, int deviceid, Time time,
    Surface *activator_surface)
{
	XEvent	     message;
	XdgToplevel *toplevel;
	Window	     window;
	...
	if (XLWmSupportsHint(_NET_ACTIVE_WINDOW)) {
		...
		XSendEvent(compositor.display,
		    DefaultRootWindow(compositor.display), False,
		...
		CatchXErrors();
		XISetFocus(compositor.display, deviceid, window, time);
		UncatchXErrors(NULL);
	}
}
    ...
    uint32_t id)
{
	...
	if (!XLWmSupportsHint(_NET_WM_STATE_FOCUSED))
		/* If _NET_WM_STATE_FOCUSED is unsupported, fall back to
		   utilizing focus in and focus out events to determine the
		   focus state.  */
...
XLHandleXEventForXdgToplevels(XEvent *event)
{
	...
	if (event->type == ClientMessage) {
		...
		if (event->xclient.message_type == WM_PROTOCOLS) {
			if (event->xclient.data.l[0] == WM_DELETE_WINDOW &&
				...
				/* _NET_WM_PING arrived.  Record the event and
				   send ping to the client.  toplevel->role
				   should be non-NULL here.  */
				XLXdgRoleHandlePing(toplevel->role, event,
				    ReplyToPing);
	...
	if (event->type == MapNotify) {
		...
		if (toplevel) {
			...
			XSetWMNormalHints(compositor.display,
			    event->xmap.window, &toplevel->size_hints);
		}
...
ParseWmProtocols(const char *string)
{
	...
	while (string < end) {
		...
		if (!strcmp(buffer, "netWmPing"))
			...
			fprintf(stderr,
			    "Warning: encountered invalid window manager "
			    "protocol: %s\n",
			    buffer);
...
static void
SetMode(struct wl_client *client, struct wl_resource *resource, uint32_t mode)
{
	XdgDecoration *decoration;
	...
	default:
		wl_resource_post_error(resource,
		    ...
		    "trying to set bogus decoration mode %u", mode);
		return;
	}
...
#undef UnconfiguredBuffer
	...
	decoration->resource = wl_resource_create(wl_resource_get_client(
						      resource),
