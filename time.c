...
StartAlarms(XSyncCounter counter, XSyncValue current_value)
{
	...
	value_mask = (XSyncCACounter | XSyncCATestType | XSyncCAValue |
	    XSyncCAEvents);
	...
	if (scalar_value >= HalfMonth) {
		...
		alarm_a = XSyncCreateAlarm(compositor.display,
		    value_mask | XSyncCADelta, &attributes);
		...
		/* Create the alarm.  */
		alarm_a = XSyncCreateAlarm(compositor.display, value_mask,
		    &attributes);
	}
	...
	if ((scalar_value & MaxTime) >= HalfMonth) {
		...
		/* Create the alarm.  */
		alarm_b = XSyncCreateAlarm(compositor.display, value_mask,
		    &attributes);
	} else {
...
InitTime(void)
{
	...
	supported = XSyncQueryExtension(compositor.display, &xsync_event_base,
	    &xsync_error_base);

	if (supported)
		supported = XSyncInitialize(compositor.display, &xsync_major,
		    &xsync_minor);

	if (!supported) {
		fprintf(stderr,
		    "A compatible version of the synchronization"
	...
	if (xsync_major < 3 || (xsync_major == 3 && xsync_minor < 1)) {
		fprintf(stderr,
		    "Sync fences are not supported by this X server\n");
		exit(1);
	}
	...
	if (!counter)
		fprintf(stderr,
		    "Server missing required system counter SERVERTIME\n");
	else {
