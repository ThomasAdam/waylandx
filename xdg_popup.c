...
Attach(Role *role, XdgRoleImplementation *impl)
{
	...
	XChangeWindowAttributes(compositor.display, window, CWOverrideRedirect,
	    &attrs);
	...
	XChangeProperty(compositor.display, window, _MOTIF_WM_HINTS,
	    _MOTIF_WM_HINTS, 32, PropModeReplace, (unsigned char *)&hints, 5);
}
...
Unmap(XdgPopup *popup)
{
	...
	XUnmapWindow(compositor.display, XLWindowFromXdgRole(popup->role));
}
...
RevertGrabTo(XdgPopup *popup, Role *parent_role)
{
	...
	DoGrab(parent, popup->grab_holder, popup->current_grab_serial);
}
...
Detach(Role *role, XdgRoleImplementation *impl)
{
	...
	XChangeWindowAttributes(compositor.display, XLWindowFromXdgRole(role),
...
MoveWindow(XdgPopup *popup)
{
	...
	XLXdgRoleGetCurrentGeometry(popup->parent, &parent_gx, &parent_gy, NULL,
	    NULL);
	XLXdgRoleGetCurrentGeometry(popup->role, &geometry_x, &geometry_y, NULL,
	    NULL);
	XLXdgRoleCurrentRootPosition(popup->parent, &root_x, &root_y);
	...
	/* geometry_x and geometry_y are relative to the local coordinate
	   system.  */
	TruncateSurfaceToWindow(popup->role->surface, geometry_x, geometry_y,
	    &geometry_x, &geometry_y);

	/* X and Y are relative to the parent coordinate system.  */
	TruncateSurfaceToWindow(popup->parent->surface, popup->x, popup->y, &x,
	    &y);
...
InternalReposition(XdgPopup *popup)
{
	...
	XLPositionerCalculateGeometry(&popup->positioner, popup->parent, &x, &y,
	    &width, &height);

	popup->pending_x = x;
	popup->pending_y = y;

	SendConfigure(popup, popup->pending_x, popup->pending_y, width, height);
...
SaveGrabHolder(XdgPopup *popup, Seat *seat)
{
	...
	if (seat) {
		popup->grab_holder	 = seat;
		popup->seat_callback_key = XLSeatRunOnDestroy(seat,
		    HandleGrabHolderDestroy, popup);
	}
}
...
{
	if (popup->resource && popup->role && popup->role->surface &&
...
Destroy(struct wl_client *client, struct wl_resource *resource)
{
	...
	if (popup->role)
		XLXdgRoleDetachImplementation(popup->role, &popup->impl);
