...
SendRoundtripMessage(void)
{
	...
	XSendEvent(compositor.display, round_trip_window, False, NoEventMask,
	    &event);
	return id;
}
	...
	while (callback != &buffer->idle_callbacks) {
		if (callback->target == target) {
			...
			callback->function((RenderBuffer)(void *)buffer,
			    callback->data);
			callback->buffer_next->buffer_last =
			    callback->buffer_last;
			callback->buffer_last->buffer_next =
			    callback->buffer_next;
			callback->target_next->target_last =
			    callback->target_last;
			callback->target_last->target_next =
			    callback->target_next;
			last	 = callback;
...
HandleActivityEvent(uint64_t counter)
{
	...
	while (record != &all_activity) {
		...
		if (last->id == counter) {
			/* Remove the record.  Then, run any callbacks
			   pertaining to it.  This code mandates that there only
			   be a single activity record for each buffer-target
			   combination on the global list at any given time.  */
...
FreeBackBuffer(PictureTarget *target, BackBuffer *buffer)
{
	XRenderFreePicture(compositor.display,
	    (buffer->picture & ~BufferSync & ~BufferBusy));
	...
	if (target->client &&
	    IntSubtractWrapv(target->client->n_pixels, buffer->n_pixels,
...
CreateBackBuffer(PictureTarget *target)
{
	...
	buffer->pixmap	   = XCreatePixmap(compositor.display, root_window,
		target->width, target->height, compositor.n_planes);
	buffer->picture	   = XRenderCreatePicture(compositor.display,
	       buffer->pixmap, compositor.argb_format, 0, &attrs);
	...
	if (IntMultiplyWrapv(target->width, target->height, &buffer->n_pixels))
...
SwapBackBuffers(PictureTarget *target, pixman_region32_t *damage)
{
	...
	if (target->render_mode == RenderModeAsync)
		XPresentPixmap(compositor.display, target->window,
		    back_buffer->pixmap, present_serial, None, region, 0, 0,
		    None, None, fence, PresentOptionAsync, 0, 0, 0, NULL, 0);
	else if (target->render_mode == RenderModeVsyncAsync)
		/* Present the pixmap asynchronously at the next frame.  */
		XPresentPixmap(compositor.display, target->window,
		    back_buffer->pixmap, present_serial, None, region, 0, 0,
		    None, None, fence, PresentOptionAsync, target->next_msc, 1,
		    0, NULL, 0);
	else
		/* Present the pixmap synchronously at the next frame.  */
		XPresentPixmap(compositor.display, target->window,
		    back_buffer->pixmap, present_serial, None, region, 0, 0,
		    None, None, fence, PresentOptionNone, target->next_msc, 1,
		    0, NULL, 0);
...
SwapBackBuffersWithCopy(PictureTarget *target, pixman_region32_t *damage)
{
	...
	for (i = 0; i < nboxes; ++i)
		XCopyArea(compositor.display, back_buffer->pixmap,
		    target->window, target->gc, boxes[i].x1, boxes[i].y1,
		    boxes[i].x2 - boxes[i].x1, boxes[i].y1 - boxes[i].y2,
...
PickVisual(int *depth)
{
	...
	visuals = XGetVisualInfo(compositor.display,
	    (VisualScreenMask | VisualClassMask | VisualDepthMask), &vinfo,
	    &n_visuals);
...
InitRenderFuncs(void)
{
	...
	if (!XPresentQueryExtension(compositor.display, &present_opcode,
		&error_base, &event_base) ||
	    !XPresentQueryVersion(compositor.display, &major, &minor)) {
		fprintf(stderr,
		    "The X presentation extension is not supported"
		    " by this X server\n");
		return False;
	}
	...
	round_trip_window	= XCreateWindow(compositor.display,
		  DefaultRootWindow(compositor.display), -1, -1, 1, 1, 0,
    ...
    unsigned long standard_event_mask)
{
	...
	if (window != None)
		...
		target->picture = XRenderCreatePicture(compositor.display,
		    drawable, compositor.argb_format, 0, &picture_attrs);
	...
	if (window) {
		...
		target->gc = XCreateGC(compositor.display, window, 0,
		    &gcvalues);
	}
...
static Bool
SetRenderMode(RenderTarget target, RenderMode mode, uint64_t target_msc)
{
...
SetClient(RenderTarget target, struct wl_client *client)
{
	...
	if (picture_target->client) {
		if (picture_target->client &&
			...
			picture_target->height, &pixels) &&
		    IntSubtractWrapv(picture_target->client->n_pixels, pixels,
...
NoteTargetSize(RenderTarget target, int width, int height)
{
	...
	if (width != pict_target->width || height != pict_target->height) {
		...
		if (pict_target->client &&
		    IntMultiplyWrapv(pict_target->width, pict_target->height,
			&pixels) &&
		    IntSubtractWrapv(pict_target->client->n_pixels, pixels,
		...
		if (pict_target->client &&
		    ...
		    IntAddWrapv(pict_target->client->n_pixels, pixels,
			&pict_target->client->n_pixels))
...
DestroyRenderTarget(RenderTarget target)
{
	...
	if (pict_target->picture)
		XRenderFreePicture(compositor.display, pict_target->picture);
    ...
    int nboxes, int min_x, int min_y)
{
	...
	XRenderFillRectangles(compositor.display, PictOpClear,
	    pict_target->picture, &color, rects, nboxes);
...
ServerRegionFromRegion(pixman_region32_t *region)
{
	...
	server_region = XFixesCreateRegion(compositor.display, rects, nboxes);
...
ClearRectangle(RenderTarget target, int x, int y, int width, int height)
{
	...
	XRenderFillRectangle(compositor.display, PictOpClear,
	    pict_target->picture, &color, x, y, width, height);
}
...
MaybeApplyTransform(PictureBuffer *buffer, DrawParams *params)
{
	...
	if (!params->flags)
		...
		if (params->flags & OffsetSet)
			MatrixTranslate(&ftransform, params->off_x,
			    params->off_y);
...
static void
Composite(RenderBuffer buffer, RenderTarget target, Operation op, int src_x,
    int src_y, int x, int y, int width, int height, DrawParams *draw_params)
{
	...
	XRenderComposite(compositor.display, ConvertOperation(op),
	    picture_buffer->picture, None, picture_target->picture,
    ...
    RenderCompletionFunc function, void *data)
{
	...
	while (tem) {
		...
		RecordBufferActivity(last->data, pict_target, roundtrip_id);
    ...
    pixman_region32_t *damage, PresentCompletionFunc callback, void *data)
{
	...
	if (pict_target->render_mode == RenderModeAsync)
		...
		XPresentPixmap(compositor.display, pict_target->window,
		    buffer->pixmap, ++present_serial, None, region, 0, 0, None,
		    None, None, PresentOptionAsync, 0, 0, 0, NULL, 0);
		...
		XPresentPixmap(compositor.display, pict_target->window,
		    buffer->pixmap, ++present_serial, None, region, 0, 0, None,
		    None, None, PresentOptionAsync, pict_target->next_msc, 1, 0,
		    NULL, 0);
	else
		/* Present the pixmap at the next msc.  */
		XPresentPixmap(compositor.display, pict_target->window,
		    buffer->pixmap, ++present_serial, None, region, 0, 0, None,
		    None, None, PresentOptionNone, pict_target->next_msc, 1, 0,
		    NULL, 0);
...
static RenderCompletionKey
NotifyMsc(RenderTarget target, RenderCompletionFunc callback, void *data)
{
	PictureTarget		  *pict_target;
	PresentCompletionCallback *callback_rec;
	...
	if (pict_target->render_mode == RenderModeVsync)
		...
		XPresentNotifyMSC(compositor.display, pict_target->window,
		    present_serial, pict_target->next_msc, 1, 0);
...
FindSupportedFormats(void)
{
	...
	do {
		format = XRenderFindFormat(compositor.display, 0, NULL, count);
		count++;
		...
		if (info &&
		    !HavePixmapFormat(info->depth, info->bits_per_pixel))
			continue;
...
MakeCheckWindow(void)
{
	...
	return XCreateWindow(compositor.display,
	    ...
	    compositor.n_planes, InputOutput, compositor.visual, flags, &attrs);
}
...
FindSupportedModifiers(int *pair_count_return)
{
	...
	for (i = 0; i < ArrayElements(all_formats); ++i) {
		if (all_formats[i].format) {
			cookies[i] = xcb_dri3_get_supported_modifiers(
			    compositor.conn, check_window, all_formats[i].depth,
			    all_formats[i].bits_per_pixel);

			/* pair_count is the number of format-modifier pairs
			   that will be returned.  First, add one for each
			   implicit modifier, and another one for each manually
			   specified modifier.  */
	...
	for (i = 0; i < ArrayElements(all_formats); ++i) {
		...
		all_formats[i].supported_modifiers = XLMalloc(
		    sizeof *mods * length);
		all_formats[i].n_supported_modifiers = length;

		for (j = 0; j < length; ++j) {
			/* Then, add length for each explicit modifier that
...
GetRenderDevices(int *num_devices)
{
	...
	reply  = xcb_randr_get_providers_reply(compositor.conn, cookie, NULL);
	...
	for (i = 0; i < nproviders; ++i) {
		open_reply = xcb_dri3_open_reply(compositor.conn,
		    open_cookies[i], &error);
		...
		devices[ndevices] = GetRenderDevice(open_reply,
		    &error_experienced);
		free(open_reply);
...
BufferFromDmaBuf(DmaBufAttributes *attributes, Bool *error)
{
	...
	check_cookie = xcb_dri3_pixmap_from_buffers(compositor.conn, pixmap,
	    DefaultRootWindow(compositor.display), attributes->n_planes,
	    attributes->width, attributes->height, attributes->offsets[0],
	    attributes->strides[0], attributes->offsets[1],
	    attributes->strides[1], attributes->offsets[2],
	    attributes->strides[2], attributes->offsets[3],
	    attributes->strides[3], depth, bpp, attributes->modifier,
	    attributes->fds);
	...
	picture = XRenderCreatePicture(compositor.display, pixmap, format, 0,
	    &picture_attrs);
...
FinishDmaBufRecord(DmaBufRecord *pending, Bool success)
{
	...
	if (success) {
		...
		picture = XRenderCreatePicture(compositor.display,
		    pending->pixmap, pending->format, 0, &picture_attrs);
...
BufferFromShm(SharedMemoryAttributes *attributes, Bool *error)
{
	...
	xcb_shm_create_pixmap(compositor.conn, pixmap,
	    DefaultRootWindow(compositor.display), attributes->width,
	    attributes->height, depth, seg, attributes->offset);
	xcb_shm_detach(compositor.conn, seg);

	/* Create the picture for the pixmap.  */
	picture = XRenderCreatePicture(compositor.display, pixmap, pict_format,
	    0, &picture_attrs);
    ...
    int32_t offset, int32_t stride, size_t pool_size)
{
	...
	wanted_stride = Roundup(width * (long)bpp, GetScanlinePad(depth));
    ...
    uint32_t alpha, Bool *error)
{
	...
	pixmap = XCreatePixmap(compositor.display,
	    DefaultRootWindow(compositor.display), 1, 1, compositor.n_planes);

	/* Create the picture.  */
	picture = XRenderCreatePicture(compositor.display, pixmap,
	    compositor.argb_format, 0, &picture_attrs);
	...
	XRenderFillRectangle(compositor.display, PictOpSrc, picture, &color, 0,
	    0, 1, 1);
...
FreeAnyBuffer(RenderBuffer buffer)
{
	...
	XFreePixmap(compositor.display, picture_buffer->pixmap);
	XRenderFreePicture(compositor.display, picture_buffer->picture);
...
SetupMitShm(void)
{
	...
	if (!ext || !ext->present) {
		fprintf(stderr,
		    "The MIT-SHM extension is not supported by this X server.\n");
		exit(1);
	}

	cookie = xcb_shm_query_version(compositor.conn);
	reply  = xcb_shm_query_version_reply(compositor.conn, cookie, NULL);

	if (!reply) {
		fprintf(stderr,
		    "The MIT-SHM extension on this X server is too old.\n");
		exit(1);
	...
	if (!HavePixmapFormat(24, 32)) {
		fprintf(stderr,
		    "X server does not support pixmap format"
...
InitBufferFuncs(void)
{
	...
	if (!x_formats) {
		fprintf(stderr,
		    "No pixmap formats could be retrieved from"
		    " the X server\n");
		return;
	}
	...
	error:
		fprintf(stderr,
		    "Warning: the X server does not support a new enough version of"
...
WaitForIdle(RenderBuffer buffer, RenderTarget target)
{
	...
	while (!IsBufferIdle(buffer, target)) {
		XIfEvent(compositor.display, &event, IdleEventPredicate, NULL);
	...
	while (callback != &all_completion_callbacks) {
		...
		if (last->id == complete->serial_number) {
			/* The presentation is complete.  Run and unlink the
			   callback.  */
			last->function(last->data, complete->msc,
			    complete->ust);
	...
	if (target) {
		...
		for (i = 0; i < ArrayElements(target->back_buffers); ++i) {
			if (target->back_buffers[i] &&
				...
				idle->serial_number)) {
				/* Now say that the target idle fence must be
				   waited for.  */
		...
		while (record != &target->pending) {
			if (record->buffer->pixmap == idle->pixmap) {
				/* The buffer was found.  Remove the
				   presentation record if the serial matches,
				   and return.  */
				if (record->serial == idle->serial_number) {
					...
					/* Run idle callbacks if this is now
					 * idle.  */
