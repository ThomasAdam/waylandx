...
HandleIntegerValue(char *name, int value, uint32_t last_change_serial)
{
	...
	for (listener = integer_listeners; listener;
...
ReadSettingsData(void)
{
	...
	rc = XGetWindowProperty(compositor.display, xsettings_window,
	    _XSETTINGS_SETTINGS, 0, LONG_MAX, False, _XSETTINGS_SETTINGS,
	    &actual_type, &actual_format, &nitems_return, &bytes_after,
	    &prop_data);
	...
	while (i < n_settings) {
		...
		case Integer:
			...
			HandleIntegerValue(name_buffer, value,
			    last_change_serial);
			break;
...
XLHandleOneXEventForXSettings(XEvent *event)
{
	if (event->type == ClientMessage &&
		...
		XSelectInput(compositor.display, xsettings_window,
		    PropertyChangeMask);
		if (UncatchXErrors(NULL))
			/* The settings window vanished; select for manager
			   events again until we obtain the new settings window.
			 */
			XLInitXSettings();
		else
...
XLInitXSettings(void)
{
	...
	if (xsettings_atom == None) {
		...
		xsettings_atom = XInternAtom(compositor.display, buffer, False);
	}
