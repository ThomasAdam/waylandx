...
static void
GetRelease(struct wl_client *client, struct wl_resource *resource, uint32_t id)
{
	Synchronization *synchronization;
	SyncRelease	*release;
...
#undef DuplicateRelease
	...
	wl_resource_set_implementation(release->resource, NULL, release,
...
static struct zwp_linux_surface_synchronization_v1_interface
...
#undef SynchronizationAlreadyExists
	...
	synchronization->destroy_listener = XLSurfaceRunOnFree(surface,
	    HandleSurfaceDestroy, synchronization);
	...
	wl_resource_set_implementation(synchronization->resource,
	    &synchronization_impl, synchronization,
...
static struct zwp_linux_explicit_synchronization_v1_interface
    explicit_sync_impl = {
	    .destroy		 = Destroy,
...
HandleBind(struct wl_client *client, void *data, uint32_t version, uint32_t id)
{
	...
	wl_resource_set_implementation(resource, &explicit_sync_impl, NULL,
	    NULL);
}
...
#define InvalidFence ZWP_LINUX_SURFACE_SYNCHRONIZATION_V1_ERROR_INVALID_FENCE
		/* Importing the fence failed; signal an error to the
		   server.  */
		wl_resource_post_error(surface->resource, InvalidFence,
		    "the specified sync"
...
XLInitExplicitSynchronization(void)
{
	...
	explicit_sync_global = wl_global_create(compositor.wl_display,
	    &zwp_linux_explicit_synchronization_v1_interface, 2, NULL,
	    HandleBind);
}
