    ...
    uint32_t id, struct wl_resource *pointer_resource)
{
	...
	wl_resource_set_implementation(gesture_resource, &gesture_swipe_impl,
	    gesture_swipe, HandleSwipeGestureResourceDestroy);
}
    ...
    uint32_t id, struct wl_resource *pointer_resource)
{
	...
	wl_resource_set_implementation(gesture_resource, &gesture_pinch_impl,
	    gesture_pinch, HandlePinchGestureResourceDestroy);
}
...
HandleBind(struct wl_client *client, void *data, uint32_t version, uint32_t id)
{
	...
	wl_resource_set_implementation(resource, &pointer_gestures_impl, NULL,
	    NULL);
}
...
XLInitPointerGestures(void)
{
	if (xi2_major > 2 || xi2_minor >= 4)
		...
		pointer_gestures_global = wl_global_create(
		    compositor.wl_display, &zwp_pointer_gestures_v1_interface,
		    3, NULL, HandleBind);
