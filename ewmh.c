...
GetWmCheckWindow(void)
{
	...
	rc	 = XGetWindowProperty(compositor.display,
		  DefaultRootWindow(compositor.display), _NET_SUPPORTING_WM_CHECK, 0,
		  1, False, XA_WINDOW, &actual_type, &actual_format, &actual_size,
		  &bytes_remaining, &tmp_data);

	if (rc != Success || actual_type != XA_WINDOW || actual_format != 32 ||
	    actual_size != 1 || !tmp_data) {
		if (tmp_data)
			XFree(tmp_data);
...
{
	CatchXErrors();
	XSelectInput(compositor.display, window, SubstructureNotifyMask);
...
start_check:
	...
	CatchXErrors();
	rc     = XGetWindowProperty(compositor.display,
		DefaultRootWindow(compositor.display), _NET_SUPPORTED, 0, 4096,
		False, XA_ATOM, &actual_type, &actual_format, &actual_size,
		&bytes_remaining, &tmp_data);
