...
Setup(Surface *surface, Role *role)
{
	...
	icon = IconSurfaceFromRole(role);
	ViewSetSubcompositor(surface->view, icon->subcompositor);
	ViewSetSubcompositor(surface->under, icon->subcompositor);

	/* Make sure the under view ends up beneath surface->view.  */
	SubcompositorInsert(icon->subcompositor, surface->under);
	SubcompositorInsert(icon->subcompositor, surface->view);
...
ReleaseBuffer(Surface *surface, Role *role, ExtBuffer *buffer)
{
	...
	if (RenderIsBufferIdle(render_buffer, icon->target))
		...
		ReleaseBufferWithHelper(icon->release_helper, buffer,
		    icon->target);
...
UpdateOutputs(IconSurface *icon)
{
	...
	XLUpdateSurfaceOutputs(icon->role.surface,
	    icon->x + icon->min_x + x_off, icon->y + icon->min_y + y_off,
	    icon->max_x - icon->min_x + 1, icon->max_y - icon->min_y + 1);
}
...
NoteBounds(void *data, int min_x, int min_y, int max_x, int max_y)
{
	...
	if (min_x != icon->min_x || min_y != icon->min_y ||
		...
		/* If the bounds changed, move the window to the right
		   position.  */
		XMoveResizeWindow(compositor.display, icon->window, x + min_x,
		    y + min_y, max_x - min_x + 1, max_y - min_y + 1);
...
XLGetIconSurface(Surface *surface)
{
	...
	XSetWMProtocols(compositor.display, role->window, &_NET_WM_SYNC_REQUEST,
	    1);

	/* Set _NET_WM_WINDOW_TYPE to _NET_WM_WINDOW_TYPE_DND.  */
	XChangeProperty(compositor.display, role->window, _NET_WM_WINDOW_TYPE,
