...
struct _DataConversion {
	...
	GetDataFunc (*clipboard_callback)(WriteTransfer *, Atom, Atom *,
	    struct wl_resource *,
	    void (*)(struct wl_resource *, const char *, int), Bool);
};
...
FindTranslationForMimeType(const char *mime_type, Bool is_primary)
{
	...
	for (i = 0; i < mapping_table.n_elements[idx]; ++i) {
		if (!strcmp(direct_transfer[buckets[i]].mime_type, mime_type) &&
		    HasSelectionTarget(MappingAtom(
					   &direct_transfer[buckets[i]]),
			is_primary)) {
...
static void
MaybeFinishDelayedTransfer(ReadTransfer *transfer, TransferInfo *info)
{
...
NoticeTransferWritable(int fd, void *data, WriteFd *writefd)
{
	...
	read_chunk:
		...
		chunk = ReadChunk(transfer, quantum / 4, &chunk_size,
		    &bytes_after);
		DebugPrint("Reading a piece of the property of size %ld\n",
		    quantum);

		/* If chunk is NULL, close fd.  The failure callback will also
		   be run soon.  */
		if (!chunk) {
	...
	DebugPrint("Writing %td bytes of chunk at offset %td\n",
	    info->chunk_size - info->bytes_into, info->bytes_into);
	...
	if (written < 0) {
		...
		if (errno == EPIPE) {
			/* EPIPE happened; set the fd to -1, skip the chunk if
			   it was not completely read, and return.  */
			if (!info->bytes_after)
				SkipChunk(transfer);
			...
			DebugPrint(
			    "EPIPE recieved while reading; cancelling transfer\n");
	...
	if (info->bytes_into == info->chunk_size) {
		DebugPrint("Chunk of %td completely written; bytes left "
			   "in property: %td\n",
		    info->chunk_size, info->bytes_after);
		...
		if (info->bytes_after)
			...
			/* The property has been completely read.  If a new
			   chunk already exists, read and try to write it now.
			   Otherwise, cancel the write callback and wait for
			   either a new property to be set, or for
			   DirectFinishCallback to be called.  */
...
DirectFinishCallback(ReadTransfer *transfer, Bool success)
{
	...
	if (info->chunk) {
		...
		DebugPrint(
		    "The transfer finished, but the chunk was not yet"
...
PostReceiveDirect(Time time, Atom selection, Atom target, int fd)
{
	...
	ConvertSelectionFuncs(selection, target, time, info, NULL,
	    DirectReadCallback, DirectFinishCallback);
}
...
#define ReceiveBody(selection, primary)                                     \
	...
	if (translation) {                                                  \
		if (!translation->translation_func)                         \
			/* If a corresponding target exists, ask to receive \
			 * it.  */                                          \
			PostReceiveDirect(time.milliseconds, selection,     \
			    MappingAtom(translation), fd);                  \
		else                                                        \
			/* Otherwise, use the translation function.  */     \
			translation->translation_func(time.milliseconds,    \
			    selection, MappingAtom(translation), fd);       \
	} else                                                              \
		close(fd)
...
CreateOffer(struct wl_client *client, Timestamp time)
{
	...
	wl_resource_set_implementation(resource, &wl_data_offer_impl,
	    AllocateTimestamp(time), HandleOfferResourceDestroy);
	return resource;
}
...
CreatePrimaryOffer(struct wl_client *client, Timestamp time)
{
	...
	resource = wl_resource_create(client,
	    &zwp_primary_selection_offer_v1_interface, 1, 0);
	if (!resource)
		/* If allocating the resource failed, return NULL.  */
		return NULL;
	...
	for (i = 0; i < ntargets; ++i) {
		...
		for (j = 0; j < mapping_table.n_atom_elements[idx]; ++j) {
			if (MappingIs(&direct_transfer[buckets[j]],
				...
				/* If it exists and was not previously offered,
...
SendOffers(struct wl_resource *resource, Timestamp time)
{
	...
	SendOffers1(resource, num_x_selection_targets, x_selection_targets,
...
SendPrimaryOffers(struct wl_resource *resource, Timestamp time)
{
	...
	SendOffers1(resource, num_x_primary_targets, x_primary_targets,
	    zwp_primary_selection_offer_v1_send_offer);
}

static void
HandleNewSelection(Time time, Atom selection, Atom *targets, int ntargets)
{
    ...
    ptrdiff_t size)
{
	...
	atoms = (Atom *)ReadChunk(transfer, size / sizeof(long), &size, NULL);
	...
	if (IntAddWrapv(n_atoms, data->n_atoms, &data->n_atoms)) {
	...
	data->atoms = XLRealloc(data->atoms, data->n_atoms * sizeof *atoms);
	for (i = 0; i < n_atoms; ++i)
...
TargetsFinishCallback(ReadTransfer *transfer, Bool success)
{
	...
	if (success)
		HandleNewSelection(GetTransferTime(transfer), data->selection,
...
NoticeClipboardChanged(Time time)
{
	...
	ConvertSelectionFuncs(CLIPBOARD, TARGETS, time, data, NULL,
	    TargetsReadCallback, TargetsFinishCallback);
}
...
NoticePrimaryChanged(Time time)
{
	...
	ConvertSelectionFuncs(XA_PRIMARY, TARGETS, time, data, NULL,
	    TargetsReadCallback, TargetsFinishCallback);
}
...
HandleSelectionNotify(XFixesSelectionNotifyEvent *event)
{
	...
	if (event->selection == CLIPBOARD &&
		...
		last_clipboard_change = TimestampFromClientTime(
		    event->selection_timestamp);

	if (event->selection == XA_PRIMARY &&
	    TimeIs(event->selection_timestamp, Later, last_primary_time))
		last_primary_time = TimestampFromClientTime(
		    event->selection_timestamp);
...
SelectSelectionInput(Atom selection)
{
	...
	XFixesSelectSelectionInput(compositor.display,
	    selection_transfer_window, selection, mask);
}
...
static GetDataFunc
GetClipboardCallback(WriteTransfer *transfer, Atom target, Atom *type)
{
	WriteInfo	   *info;
	int		    pipe[2], rc;
	DataConversion	   *conversion;
	...
	if (!XLDataSourceHasAtomTarget(selection_data_source, target)) {
		...
		if (conversion->clipboard_callback)
			return conversion->clipboard_callback(transfer, target,
			    type, resource, wl_data_source_send_send, False);

		/* Otherwise, use the default callback.  */
		DebugPrint(
		    "Conversion to type %lu specified with default callback\n",
		    target);
	}
	...
	wl_data_source_send_send(resource, MimeTypeFromTarget(target, False),
	    pipe[1]);
	close(pipe[1]);
    ...
    ptrdiff_t buffer_size, ptrdiff_t *nbytes)
{
	...
	if (buffer_size == -1) {
		DebugPrint(
		    "ConversionReadFunc called to free data for timeout\n");
	...
	/* Convert the appropriate amount of bytes into the output
	   buffer.  */
	outsize = buffer_size;
	nconv	= iconv(info->cd, &info->inptr, &info->inread, (char **)&buffer,
	      &outsize);
	...
	if (nconv == (size_t)-1 && errno != EINVAL) {
		if (errno == E2BIG) {
			...
			*nbytes = buffer_size - outsize;
			...
			if (*nbytes < 1) {
				DebugPrint(
				    "iconv failed with a buffer as large as the"
				    "max-request-size!\n");
...
GetConversionCallback(WriteTransfer *transfer, Atom target, Atom *type,
    ...
    void (*send_send)(struct wl_resource *, const char *, int), Bool is_primary)
{
	ConversionWriteInfo *info;
	int		     pipe[2], rc;
	iconv_t		     cd;
    ...
    ptrdiff_t size)
{
	...
	data = ReadChunk(transfer,
	    (format == 32 ? size / sizeof(long) : (size + 3) / 4), &size, NULL);
...
NoticeConversionTransferWritable(int fd, void *data, WriteFd *writefd)
{
	...
	if (info->outsize) {
		written = write(info->fd, info->output_buffer, info->outsize);

		if (written == -1) {
			DebugPrint("write: %s\n", strerror(errno));

			if (errno == EAGAIN)
				/* Writing the data failed, so return.  This
				   might cause us to spin.  */
				return;
			...
			/* The other end of the pipe was closed by the client.
			   Stop this transfer.  */
			FinishConversionTransfer(data, info);
			return;
		}

		/* Writing the data was successful.  Move the written part back
		   to the start.  */
	...
	if (info->buffer_size) {
		...
		if (nconv == (size_t)-1 && errno != EINVAL) {
			...
			if ((errno == E2BIG && outsize == BUFSIZ) ||
			    errno != E2BIG) {
				/* BUFSIZ is either too small to hold the output
				   contents (which shouldn't happen), or some
				   other error occured.  Punt.  */
				FinishConversionTransfer(data, info);
				return;
			}
		}
...
ConversionFinishCallback(ReadTransfer *transfer, Bool success)
{
	...
	DebugPrint(
	    "ConversionFinishCallback called; converting data in chunks.\n");
...
static GetDataFunc
GetDragCallback(WriteTransfer *transfer, Atom target, Atom *type)
{
	WriteInfo *info;
	int	   pipe[2], rc;
...
XLOwnDragSelection(Time time, DataSource *source)
{
	...
	rc = OwnSelection(TimestampFromClientTime(time), XdndSelection,
	    GetDragCallback, targets, ntargets);
	XLFree(targets);
...
XLNoteSourceDestroyed(DataSource *source)
{
	if (source == selection_data_source) {
		DebugPrint(
		    "Disowning CLIPBOARD at %u (vs. last change %u)\n"
...
#define NoteLocalSelectionBody(callback, time_1, time_2, atom, field, foreign, \
    n_foreign)                                                                 \
	Timestamp time;                                                        \
	Atom	 *targets;                                                     \
...
#define NoteLocalSelectionFooter(callback, atom, field, has_target)                                     \
	...
	for (i = 0; i < ArrayElements(data_conversions); ++i) {                                         \
		/* If the source provides MIME typed-data for a format we know                          \
		   how to convert, announce the associated selection                                    \
		   conversion targets.  */                                                                                           \
                                                                                                        \
		if (has_target(source, data_conversions[i].mime_type) &&                                \
		    !FindTargetInArray(targets, ntargets,                                               \
			data_conversions[i].type)) {                                                    \
			DebugPrint(                                                                     \
			    "Client doesn't provide standard X conversion"                              \
...
XLNoteLocalSelection(Seat *seat, DataSource *source)
{
	NoteLocalSelectionBody(GetClipboardCallback, last_clipboard_time,
	    ...
	    x_selection_targets, num_x_selection_targets);
	...
	NoteLocalSelectionFooter(GetClipboardCallback, CLIPBOARD,
	    selection_data_source, XLDataSourceHasTarget);
}

static GetDataFunc
GetPrimaryCallback(WriteTransfer *transfer, Atom target, Atom *type)
{
	WriteInfo	   *info;
	int		    pipe[2], rc;
	DataConversion	   *conversion;
...
#undef Function
	...
	zwp_primary_selection_source_v1_send_send(resource,
	    MimeTypeFromTarget(target, True), pipe[1]);
	close(pipe[1]);
...
XLNoteLocalPrimary(Seat *seat, PDataSource *source)
{
	...
	NoteLocalSelectionBody(GetPrimaryCallback, last_primary_time,
	    last_primary_time, XA_PRIMARY, primary_data_source,
	    x_primary_targets, num_x_primary_targets);
	...
	NoteLocalSelectionFooter(GetPrimaryCallback, XA_PRIMARY,
	    primary_data_source, XLPDataSourceHasTarget);
}
...
XLInitXData(void)
{
	...
	rc = XFixesQueryExtension(compositor.display, &fixes_event_base,
	    &fixes_error_base);
	...
	rc = XFixesQueryVersion(compositor.display, &major, &minor);
...
void
XLReceiveDataFromSelection(Time time, Atom selection, Atom target, int fd)
{
