...
Finish(struct wl_client *client, struct wl_resource *resource)
{
	...
	if (serial < dnd_state.serial || !dnd_state.used_action ||
	    !dnd_state.accepted || dnd_state.finished) {
...
SendStatus(void)
{
	...
	if (dnd_state.respond) {
		...
		if (dnd_state.version >= 3)
			event.xclient.data.l[4] = ConvertAction(
			    dnd_state.used_action);
		else
			/* The version of the data device manager protocol
			   spoken by the client doesn't support actions.  Use
			   XdndActionPrivate.  */
			event.xclient.data.l[4] = XdndActionPrivate;
	}

	CatchXErrors();
	XSendEvent(compositor.display, dnd_state.source_window, False,
	    NoEventMask, &event);
	UncatchXErrors(NULL);
}
...
CreateOffer(struct wl_client *client, int version)
{
	...
	resource = wl_resource_create(client, &wl_data_offer_interface, version,
	    0);
	...
	wl_resource_set_implementation(resource, &wl_data_offer_impl,
	    (void *)(intptr_t)dnd_state.serial, HandleResourceDestroy);
	dnd_state.resources = XLListPrepend(dnd_state.resources, resource);
...
FinishDndEntry(void)
{
	...
	if (dnd_state.seat &&
	    ...
	    && !dnd_state.dropped)
		XLDataDeviceSendLeave(dnd_state.seat, dnd_state.surface, NULL);
...
RespondToDndDrop(void)
{
	...
	if (dnd_state.proto >= 5 && dnd_state.used_action &&
	    dnd_state.accepted && dnd_state.seat && dnd_state.respond) {
		...
		if (dnd_state.version >= 3)
			 ...
			 */
			event.xclient.data.l[2] = ConvertAction(
			    dnd_state.used_action);
		else
			/* The version of the data device manager protocol
			   spoken by the client doesn't support actions.  Use
...
static void
HandleDndEntry(Surface *target, Window source, Atom *targets, int ntargets,
    int proto)
{
	int    i;
	char **names;
	...
	XGetAtomNames(compositor.display, targets, ntargets, names);
...
ReadXdndTypeList(Window window, int *nitems_return)
{
	...
	CatchXErrors();
	rc = XGetWindowProperty(compositor.display, window, XdndTypeList, 0,
	    LONG_MAX, False, XA_ATOM, &actual_type, &actual_format, &nitems,
...
HandleXdndEnterEvent(Surface *surface, XEvent *event)
{
	...
	if (event->xclient.data.l[1] & 1)
		...
		targets = ReadXdndTypeList(event->xclient.data.l[0], &ntargets);
	else {
	...
	if (!targets)
		/* For some reason we failed to retrieve XdndTypeList.  Ignore
		   the XdndEnter event.  */
		return True;

	proto = MIN(event->xclient.data.l[1] >> 24, XdndProtocolVersion);
...
HandleChildUnmap(void *data)
{
	...
	if (dnd_state.seat)
		XLDataDeviceSendLeave(dnd_state.seat, dnd_state.child, NULL);
	XLSurfaceCancelUnmapCallback(dnd_state.unmap_callback);
...
static Bool
HandleMotion(Surface *toplevel, int x, int y, uint32_t action, int *x_out,
    int *y_out)
{
	...
	if (child)
		/* x_out and y_out are only used if dnd_state.child ends up
		   non-NULL.  */
		TruncateWindowToSurface(child, x - x_off, y - y_off, x_out,
		    y_out);
	...
	if (child) {
		...
		dnd_state.unmap_callback = XLSurfaceRunAtUnmap(child,
		    HandleChildUnmap, NULL);
		...
		for (tem = dnd_state.resources; tem; tem = tem->next) {
			if (wl_resource_get_version(tem->data) >= 3)
				wl_data_offer_send_source_actions(tem->data,
				    action);
		}
...
ReadDndActionList(Window window)
{
	...
	CatchXErrors();
	rc = XGetWindowProperty(compositor.display, window, XdndActionList, 0,
	    LONG_MAX, False, XA_ATOM, &actual_type, &actual_format, &nitems,
	    &bytes_remaining, &tmp_data);
	if (UncatchXErrors(NULL) || rc != Success || actual_format != 32 ||
	    !tmp_data || actual_type != XA_ATOM || nitems < 1) {
...
HandleXdndPositionEvent(Surface *surface, XEvent *event)
{
	...
	XTranslateCoordinates(compositor.display,
	    DefaultRootWindow(compositor.display), XLWindowFromSurface(surface),
	...
	if (action == WL_DATA_DEVICE_MANAGER_DND_ACTION_ASK) {
		if (!(action & dnd_state.source_actions))
			/* Fetch the list of available actions, and give that to
			   the client along with the regular action list, if
			   XdndActionAsk is being specified for the first time.
			 */
...
XLDndWriteAwarenessProperty(Window window)
{
	...
	XChangeProperty(compositor.display, window, XdndAware, XA_ATOM, 32,
...
AddChild(WindowCacheEntry *parent, Window window,
    xcb_get_geometry_reply_t *geometry, xcb_query_tree_reply_t *children,
    ...
    xcb_shape_get_rectangles_reply_t  *input)
{
	...
	mask = (entry->old_event_mask | SubstructureNotifyMask |
	    PropertyChangeMask);
	...
	XLMakeAssoc(parent->cache->entries, window, entry);
...
AddChildren(WindowCacheEntry *entry, xcb_query_tree_reply_t *reply)
{
	...
	for (i = 0; i < n_children; ++i) {
		geometries[i] = xcb_get_geometry(compositor.conn, windows[i]);
		children[i]   = xcb_query_tree(compositor.conn, windows[i]);
		attributes[i] = xcb_get_window_attributes(compositor.conn,
		    windows[i]);
		boundings[i]  = xcb_shape_get_rectangles(compositor.conn,
		     windows[i], XCB_SHAPE_SK_BOUNDING);
		inputs[i]     = xcb_shape_get_rectangles(compositor.conn,
			windows[i], XCB_SHAPE_SK_INPUT);
	}
	...
	for (i = 0; i < n_children; ++i) {
		geometry  = xcb_get_geometry_reply(compositor.conn,
		     geometries[i], &error);
		tree	  = xcb_query_tree_reply(compositor.conn, children[i],
			 &error1);
	...
	for (i = 0; i < n_children; ++i) {
		...
		AddChild(entry, windows[i], all_geometries[i], all_trees[i],
		    all_attributes[i], all_boundings[i], all_inputs[i]);
...
MakeRootWindowEntry(WindowCache *cache)
{
	...
	if (!geometry || !tree) {
		...
		fprintf(stderr,
		    "failed to obtain window geometry or tree"
		    " of root window");
		abort();
	}
	...
	pixman_region32_init_rect(&entry->shape, entry->x, entry->y,
...
FreeWindowCacheEntry(WindowCacheEntry *entry)
{
	...
	XLDeleteAssoc(entry->cache->entries, entry->window);
	...
	if (entry->last) {
		...
		if (!(entry->flags & IsDestroyed)) {
			...
			xcb_shape_select_input(compositor.conn, entry->window,
			    0);
		}
	} else
	...
	if (event->xconfigure.above == None) {
		...
		while (next != parent->children) {
			if (next->window == event->xconfigure.above) {
				/* Move the item on top of next by placing it
				   before next.  */
				UnlinkWindowCacheEntry(window);
...
HandleCreateNotify(WindowCache *cache, XEvent *event)
{
	...
	bounding_cookie	  = xcb_shape_get_rectangles(compositor.conn,
	      event->xcreatewindow.window, XCB_SHAPE_SK_BOUNDING);
	input_cookie	  = xcb_shape_get_rectangles(compositor.conn,
		 event->xcreatewindow.window, XCB_SHAPE_SK_INPUT);
	...
	attributes = xcb_get_window_attributes_reply(compositor.conn,
	    attributes_cookie, &error2);
	bounding   = xcb_shape_get_rectangles_reply(compositor.conn,
	      bounding_cookie, &error3);
	input = xcb_shape_get_rectangles_reply(compositor.conn, input_cookie,
	    &error4);

	if (error || error1 || error2 || error3 || error4 || !geometry ||
	...
	CatchXErrors();
	AddChild(parent, event->xcreatewindow.window, geometry, tree,
	    attributes, bounding, input);
	UncatchXErrors(NULL);
...
EnsureShape(WindowCacheEntry *entry, Bool force)
{
	...
	bounding_cookie = xcb_shape_get_rectangles(compositor.conn,
	    entry->window, XCB_SHAPE_SK_BOUNDING);
	input_cookie = xcb_shape_get_rectangles(compositor.conn, entry->window,
	    XCB_SHAPE_SK_INPUT);

	/* Ask for replies from the X server.  */
	bounding = xcb_shape_get_rectangles_reply(compositor.conn,
	    bounding_cookie, &error);
	input	 = xcb_shape_get_rectangles_reply(compositor.conn, input_cookie,
	       &error1);
...
IsToplevelWindow(WindowCacheEntry *entry)
{
	...
	CatchXErrors();
	rc = XGetWindowProperty(compositor.display, entry->window, WM_STATE, 0,
	    2, False, WM_STATE, &actual_type, &actual_format, &actual_size,
	    &bytes_remaining, &tmp_data);
	if (UncatchXErrors(NULL) || rc != Success || actual_type != WM_STATE ||
...
static void
ReadProtocolProperties(Window window, int *version_return, Window *proxy_return)
{
	WindowCacheEntry	 *entry;
	...
	xdnd_proto_cookie = xcb_get_property(compositor.conn, 0, window,
	    XdndAware, XCB_ATOM_ATOM, 0, 1);
	xdnd_proxy_cookie = xcb_get_property(compositor.conn, 0, window,
	    XdndProxy, XCB_ATOM_WINDOW, 0, 1);
...
SendEnter(void)
{
	...
	message.xclient.data.l[1] = MIN(XdndProtocolVersion, drag_state.version)
	    << 24;
	...
	CatchXErrors();
	XSendEvent(compositor.display, drag_state.target, False, NoEventMask,
	    &message);
	UncatchXErrors(NULL);
}
...
SendPosition(short root_x, short root_y)
{
	...
	if (drag_state.flags & WaitingForStatus) {
		if (!(drag_state.flags & PendingDrop))
			/* If the drop already happened, don't bother sending
			   another position event.  */
	...
	if (drag_state.flags & NeedMouseRect &&
	    ...
	    root_x < (drag_state.mouse_rect.x + drag_state.mouse_rect.width) &&
	    root_y < (drag_state.mouse_rect.y + drag_state.mouse_rect.height))
		return;
...
ReportStateToSource(void)
{
	...
	if (!(drag_state.flags & WillAcceptDrop))
		...
		wl_data_source_send_target(resource, PickMimeType(source));
...
{
	if (drag_state.window_cache)
		ProcessEventForWindowCache(drag_state.window_cache, event);
...
XLDoDragMotion(Seat *seat, double root_x, double root_y)
{
	...
	if (!drag_state.seat) {
		...
		drag_state.seat_key  = XLSeatRunOnDestroy(seat,
		     HandleDragSeatDestroy, NULL);
		drag_state.modifiers = XLSeatGetEffectiveModifiers(seat);
		drag_state.mods_key  = XLSeatAddModifierCallback(seat,
		     HandleModifiersChanged, NULL);
	...
	toplevel = FindToplevelWindow(drag_state.window_cache, root_x, root_y);
	...
	if (toplevel && toplevel != drag_state.toplevel) {
		...
		if (proxy != None) {
			...
			/* Check the proxy to make sure its XdndProxy property
			   points to itself.  If it does not, the proxy property
			   is left over from a crash.  */
			if (self != proxy)
				proxy = None;
			else
				/* Otherwise, set the version to the value of
				   XdndAware on the proxy window.  */
	...
	if (toplevel != drag_state.toplevel) {
		...
		if (toplevel) {
			...
			drag_state.target  = (proxy != None ? proxy : toplevel);
			drag_state.version = version;

			/* Then, send XdndEnter followed by XdndPosition, and
			   wait for an XdndStatus event.  */
			SendEnter();
		}
	}
...
XLDoDragDrop(Seat *seat)
{
	...
	if (!(drag_state.flags & WaitingForStatus)) {
		...
		/* Set PendingDrop.  Then, return True, so the code in seat.c
		   does not clobber the data in drag_state.  */
		drag_state.flags |= PendingDrop;
