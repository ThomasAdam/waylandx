    ...
    uint32_t id, uint32_t r, uint32_t g, uint32_t b, uint32_t a)
{
	...
	error		      = False;
	buffer->render_buffer = RenderBufferFromSinglePixel(r, g, b, a, &error);
	...
	wl_resource_set_implementation(buffer->resource,
	    &single_pixel_buffer_impl, buffer, HandleResourceDestroy);
}
...
XLInitSinglePixelBuffer(void)
{
	single_pixel_buffer_global = wl_global_create(compositor.wl_display,
	    &wp_single_pixel_buffer_manager_v1_interface, 1, NULL, HandleBind);
}
