...
HandleClientDestroy(struct wl_listener *listener, void *data)
{
	...
	client			   = data;
	pending_disconnect_clients = XLListRemove(pending_disconnect_clients,
	    client);
...
ErrorDataForClient(struct wl_client *client)
{
	...
	listener = wl_client_get_destroy_listener(client, HandleClientDestroy);
...
static void
CategorizeClients(struct wl_list *client_list, struct wl_client **clients,
    ClientMemoryCategory *categories, ClientMemoryCategory *max_category)
{
	struct wl_list	 *list;
	struct wl_client *client;
	ClientErrorData	 *data;
...
SavePendingDisconnectClient(struct wl_client *client)
{
	...
	pending_disconnect_clients = XLListPrepend(pending_disconnect_clients,
	    client);
}
...
HandleBadAlloc(XErrorEvent *event)
{
	...
	if (wl_list_empty(client_list)) {
		...
		XGetErrorText(compositor.display, event->error_code, buf,
		    sizeof buf);
	...
	CategorizeClients(client_list, clients_by_category, categories,
	    &max_category);
