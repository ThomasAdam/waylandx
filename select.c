...
struct _SelectionOwnerInfo {
	...
	/* The callback for this selection.  NULL means the selection is no
	   longer owned.  */
	GetDataFunc (*get_transfer_function)(WriteTransfer *, Atom, Atom *);
};
...
AllocPropAtom(void)
{
	...
	if (free_list.next != &free_list) {
		...
		if (IntAddWrapv(prop_counter, 1, &prop_counter)) {
			fprintf(stderr,
			    "Failed to allocate selection property"
...
ConvertSelection(Atom selection, Atom target, Time time)
{
	...
	XConvertSelection(compositor.display, selection, target,
	    transfer->property->atom, selection_transfer_window, time);
...
FindReadTransfer(Atom selection, Atom target, Time time)
{
	...
	while (transfer != &read_transfers) {
		if (transfer->selection == selection &&
		    transfer->target == target && transfer->time == time)
			return transfer;
    ...
    ptrdiff_t *bytes_after_return)
{
	...
	rc = XGetWindowProperty(compositor.display, selection_transfer_window,
	    transfer->property->atom, transfer->read_offset, long_length, True,
	...
	if (actual_type == None || actual_format != transfer->read_format ||
	    rc != Success) {
		/* Same goes here.  */
		if (prop_data)
			XFree(prop_data);
...
StartSelectionRead(ReadTransfer *transfer)
{
	...
	rc = XGetWindowProperty(compositor.display, selection_transfer_window,
	    transfer->property->atom, 0, 0, True, AnyPropertyType, &actual_type,
	...
	if (transfer->data_start_func
		...
		transfer->data_start_func(transfer, actual_type, actual_format);
...
{
	CatchXErrors();
	XSendEvent(compositor.display, event->xany.window, False, NoEventMask,
	    event);
...
static GetDataFunc
GetTransferFunction(SelectionOwnerInfo *info, WriteTransfer *transfer,
...
FindWriteTransfer(Window requestor, Atom property, int ignore_state)
{
	...
	while (transfer != &write_transfers) {
		if (transfer->requestor == requestor &&
		    ...
		    (!ignore_state ||
			((transfer->state & ignore_state) != ignore_state)))
			return transfer;
...
SignalConversionPerformed(WriteTransfer *transfer)
{
	if (transfer->record) {
		...
		DebugPrint(
		    "Conversion complete; %d conversion(s) are still pending\n",
...
FlushTransfer(WriteTransfer *transfer, Bool force)
{
	...
	if ((transfer->state & IsStarted) || force) {
		...
		XChangeProperty(compositor.display, transfer->requestor,
		    transfer->property, transfer->type, 8, PropModeReplace,
		...
		CatchXErrors();
		XChangeProperty(compositor.display, transfer->requestor,
		    transfer->property, INCR, 32, PropModeReplace,
...
SelectPropertyNotify(Window window)
{
	...
	if (data)
		...
		CatchXErrors();
		XSelectInput(compositor.display, window, PropertyChangeMask);
		UncatchXErrors(NULL);
...
FreeTransfer(WriteTransfer *transfer)
{
	...
	if (transfer->record) {
		...
		CatchXErrors();
		XChangeProperty(compositor.display, transfer->requestor,
		    transfer->record->event.xselection.property, ATOM_PAIR, 32,
		    PropModeReplace, (unsigned char *)transfer->record->atoms,
...
TransferFinished(WriteTransfer *transfer)
{
	if (transfer->state & IsWaitingForDelete) {
		/* If we are still waiting for property deletion, simply set the
		   IsFinished flag.  Zero-length property data will be written
		   the next time the property is deleted to mark the completion
		   of this transfer.  */
		transfer->state |= IsFinished;

		DebugPrint(
		    "Transfer finished; waiting for property deletion\n");
...
TransferBecameReadable(WriteTransfer *transfer)
{
	...
	status = transfer->transfer_function(transfer,
	    (transfer->buffer + transfer->offset),
	    (transfer->size - transfer->offset), &bytes_read);
	...
	if (status != EndOfFile) {
		...
		if (transfer->offset == transfer->size
			...
			if (status == NeedBiggerBuffer)
				/* Since a bigger buffer is needed, that means
				   there is still data buffered up to be read.
				 */
				transfer->state |= IsReadable;
...
static void
ConvertSelectionTargets1(SelectionOwnerInfo *info, Window requestor,
    Atom property)
{
	Atom *targets;
	...
	memcpy(&targets[3], info->targets, info->ntargets * sizeof *targets);

	XChangeProperty(compositor.display, requestor, property, XA_ATOM, 32,
	    PropModeReplace, (unsigned char *)targets, 2 + info->ntargets);
}
...
{
	XChangeProperty(compositor.display, requestor, property, XA_ATOM, 32,
	    PropModeReplace, (unsigned char *)&info->time, 1);
}
    ...
    XEvent *notify)
{
	...
	rc = XGetWindowProperty(compositor.display,
	    ...
	    event->xselectionrequest.property, 0, 65535, False, ATOM_PAIR,
	...
	for (i = 0; i < nitems; i += 2) {
		DebugPrint(
		    "Verifying MULTIPLE transfer; target = %lu, property = %lu\n",
		    atoms[i + 0], atoms[i + 1]);
		...
		if (FindWriteTransfer(event->xselectionrequest.requestor,
			...
			atoms[1])) {
			DebugPrint(
			    "Found ongoing selection transfer with same requestor "
	...
	for (i = 0; i < nitems; i += 2) {
		...
		DebugPrint(
		    "Starting MULTIPLE transfer; target = %lu, property = %lu\n",
		    atoms[i + 0], atoms[i + 1]);
		...
		if (!CanConvertTarget(info, atoms[0])) {
			DebugPrint(
			    "Couldn't convert to target for a simple reason;"
		...
		if (atoms[0] == TIMESTAMP) {
			DebugPrint(
			    "Converting to special target TIMESTAMP...\n");
			ConvertSelectionTimestamp1(info,
			    event->xselectionrequest.requestor, atoms[1]);
		...
		transfer->transfer_function = GetTransferFunction(info,
		    transfer, transfer->target, &transfer->type);
	...
	if (prop_data_changed)
		XChangeProperty(compositor.display,
		    event->xselectionrequest.requestor,
		    event->xselectionrequest.property, ATOM_PAIR, 32,
		    PropModeReplace, prop_data, nitems);
...
HandleSelectionRequest(XEvent *event)
{
	...
	DebugPrint(
	    "Received SelectionRequest.  Time: %lu, requestor: %lu"
	    ", target: %lu, selection: %lu, property: %lu, serial: %lu\n",
	    event->xselectionrequest.time, event->xselectionrequest.requestor,
	    event->xselectionrequest.target, event->xselectionrequest.selection,
	    event->xselectionrequest.property, event->xselectionrequest.serial);
	...
	if (notify.xselection.target == TARGETS) {
		...
		DebugPrint(
		    "Converting selection to special target TIMESTAMP\n");
		ConvertSelectionTimestamp(info, &notify);
		return True;
	} else if (notify.xselection.target == MULTIPLE) {
		if (event->xselectionrequest.property == None) {
			DebugPrint(
			    "Got malformed MULTIPLE request with no property\n");
			notify.xselection.property = None;
			SendEvent(&notify);
...
HandleSelectionNotify(XEvent *event)
{
	...
	if (event->xselection.property == None) {
		transfer = FindReadTransfer(event->xselection.selection,
		    event->xselection.target, event->xselection.time);
...
HandlePropertyDelete(XEvent *event)
{
	...
	while (transfer != &write_transfers) {
		...
		if (transfer->state & IsFinished) {
			...
			/* The transfer is now complete; finish it by freeing
			   its data, and potentially writing zero-length data.
			 */
			FreeTransfer(transfer);
		} else if (transfer->state & IsWaitingForIncr) {
			/* If transfer is waiting for the INCR property to be
			   deleted, mark it as started, and flush it again to
			   write the first piece of property data.  */

			DebugPrint(
			    "Starting transfer in response to INCR property deletion\n");
			...
			if (transfer->state & IsReadable) {
				DebugPrint(
				    "Picking read back up from where it was left\n");
...
HandlePropertyNotify(XEvent *event)
{
	...
	if (event->xproperty.window != DefaultRootWindow(compositor.display))
		...
		DebugPrint("PropertyNotify event:\n"
			   ...
			   "time:\t%lu\n"
			   "state:\t%s\n",
		    event->xproperty.serial, event->xproperty.window,
		    event->xproperty.atom, event->xproperty.time,
...
OwnSelection(Timestamp time, Atom				      selection,
    GetDataFunc (*hook)(WriteTransfer *transfer, Atom, Atom *), Atom *targets,
    int ntargets)
{
	Window		    owner;
	SelectionOwnerInfo *info;
	...
	XSetSelectionOwner(compositor.display, selection,
	    selection_transfer_window, time.milliseconds);
	...
	if (!info) {
		...
		XLMakeAssoc(selection_owner_info, selection, info);
	} else
		XLFree(info->targets);
...
DisownSelection(Atom selection)
{
	...
	if (info && info->get_transfer_function) {
		XSetSelectionOwner(compositor.display, selection, None,
...
InitSelections(void)
{
	...
	selection_transfer_window = XCreateWindow(compositor.display,
	    ...
	    CopyFromParent, InputOnly, CopyFromParent, flags, &attrs);
