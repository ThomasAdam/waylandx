...
CurrentCursorX(TextInput *input)
{
	...
	TruncateSurfaceToWindow(input->client_info->focus_surface,
	    input->current_state.cursor_x, input->current_state.cursor_y, &x,
	    &y);
...
CurrentCursorWidth(TextInput *input)
{
	...
	TruncateScaleToWindow(input->client_info->focus_surface,
	    ...
	    input->current_state.cursor_height, &width, &height);
    ...
    const char *text, int cursor, int anchor)
{
	...
	input->pending_state.cursor = TextPositionFromBytePosition(text,
	    strlen(text), cursor);
...
static void
FitRect(XRectangle *input, int view_width, int view_height, int caret_x,
    int caret_y, int caret_width, int caret_height)
{
...
DoGeometryAllocation(TextInput *input)
{
	...
	if (xim_style & XIMPreeditPosition) {
		...
		if (!rc) {
			...
			if (input->current_state.pending &
			    PendingCursorRectangle) {
				needed->x = (CurrentCursorX(input) +
				    CurrentCursorWidth(input));
				needed->y = (CurrentCursorY(input) +
				    CurrentCursorHeight(input));

				FitRect(needed, ViewWidth(view),
				    ViewHeight(view), CurrentCursorX(input),
				    CurrentCursorY(input),
				    CurrentCursorWidth(input),
				    CurrentCursorHeight(input));
				...
				/* No caret was specified... Place the preedit
				   window on the bottom left corner of the view.
				 */
				needed->x = 0;
			...
			XSetICValues(input->xic, XNPreeditAttributes, attr,
			    NULL);
			XFree(attr);
	...
	if (xim_style & XIMStatusArea) {
		...
		if (!rc) {
			...
			DebugPrint(
			    "placed rectangle at bottom right: %d %d %d %d",
			    needed->x, needed->y, needed->width,
			    needed->height);

			/* Set the geometry.  */
			attr = XVaCreateNestedList(0, XNArea, needed, NULL);
			XSetICValues(input->xic, XNStatusAttributes, attr,
			    NULL);
			XFree(attr);
...
Commit(struct wl_client *client, struct wl_resource *resource)
{
	...
	if (input->pending_state.pending & PendingEnabled) {
		if (input->pending_state.enabled) {
			/* Check if there is another enabled text input in the
			   same client info structure.  */
			enabled = FindEnabledTextInput(input->client_info);

			if (enabled && enabled != input)
				/* Return, as the spec says we should ignore
				   requests to enable a text input.  */
				return;
		}
		...
		if (input->pending_state.pending & PendingCursorRectangle) {
			...
			if (input->current_state.enabled && input->xic)
				/* Perform geometry/position allocation on the
				 * IC.  */
				DoGeometryAllocation(input);
		}
		...
		if (input->pending_state.pending & PendingSurroundingText) {
			...
			/* Surrounding text changed.  Move the surrounding text
			   and cursor position over.  */
			input->current_state.surrounding_text =
			    input->pending_state.surrounding_text;
			input->current_state.cursor =
...
HandleResourceDestroy(struct wl_resource *resource)
{
	...
	if (input->client_info) {
		...
		if (input->client_info->inputs.next ==
			...
			XLSeatCancelDestroyListener(
			    input->client_info->seat_key);
			input->client_info->last->next =
			    input->client_info->next;
			input->client_info->next->last =
			    input->client_info->last;
...
NoticeEnter(TextInputClientInfo *info, Surface *surface)
{
	...
	DebugPrint("client info: %p, surface: %p", info, surface);
    ...
    uint32_t id, struct wl_resource *seat_resource)
{
	...
	if (XLSeatIsInert(seat)) {
		...
		if (!dummy)
			...
			wl_resource_set_implementation(dummy, &input_impl, NULL,
			    NULL);
	...
	input->resource = wl_resource_create(client,
	    &zwp_text_input_v3_interface, wl_resource_get_version(resource),
	    id);
	...
	wl_resource_set_implementation(input->resource, &input_impl, input,
	...
	if (info->focus_surface) {
		...
		DebugPrint("focusing newly created text input with info %p",
		    input);
...
static Bool
PreeditDeleteChars(PreeditBuffer *buffer, int start_char, int length)
{
	wchar_t wc;
	...
	while (chars < start_char) {
		if (start >= buffer->buffer + buffer->size) {
			DebugPrint("start %p out of bounds %p", start,
	...
	while (chars < old_chars + length) {
		if (end >= buffer->buffer + buffer->size) {
			DebugPrint("end %p out of bounds %p", end,
			    buffer->buffer + buffer->size);
	...
	buffer->buffer = XLRealloc(buffer->buffer, buffer->size);
...
static Bool
PreeditInsertChars(PreeditBuffer *buffer, int start_char, const char *string,
    size_t length, int char_length)
{
	wchar_t wc;
	...
	buffer->buffer = XLRealloc(buffer->buffer, buffer->size + length);
	...
	memmove(start + length, start, buffer->buffer + buffer->size - start);
	buffer->size += length;
...
UpdatePreedit(TextInput *input)
{
	...
	if (input->buffer) {
		...
		buffer = ConvertString(input->buffer->buffer,
		    input->buffer->size, &new_text_size);
		...
		if (input->caret_style != XIMIsInvisible)
			caret = TextPositionFromCharPosition(buffer,
			    new_text_size, input->caret);
		else
			/* The caret is hidden, so don't send any caret
			 * position.  */
	...
	no_buffer:
		...
		zwp_text_input_v3_send_preedit_string(input->resource, NULL, -1,
		    -1);
    ...
    XIMPreeditDrawCallbackStruct *call_data)
{
	...
	DebugPrint("chg_first: %d, chg_length: %d", call_data->chg_first,
	...
	DebugPrint("buffer text is now: %.*s, with the caret at %d",
	    (int)input->buffer->size, input->buffer->buffer, input->caret);
...
static TextPosition
ScanForwardWord(const char *string, size_t string_size, TextPosition caret,
    int factor)
{
	const char  *start;
	Bool	     punct_found;
	...
	while (start < string + string_size) {
		...
		while (isspace(*start) || ispunct(*start)) {
			...
			if (++start >= string + string_size)
				/* We are now at the end of the string, so just
				   return the position of caret_before, which
				   should be before this extraneous punctuation.
				 */
				return caret_before;
		...
		if (start == string + string_size - 1) {
			DebugPrint(
			    "returning caret_before at end of string: char: %d byte: %td",
			    caret_before.charpos, caret_before.bytepos);
...
static TextPosition
ScanBackwardWord(const char *string, size_t string_size, TextPosition caret,
    int factor)
{
	...
	while (start >= string) {
		...
		do {
			if (--start < string) {
				/* Invalid UTF-8 data was found in STRING.  Just
				   return the start of the string in this case.
				 */
				caret.charpos = 0;
				caret.bytepos = 0;
				return caret;
		...
		DebugPrint("caret_before: char: %d byte: %td, new char: %c",
		    caret_before.charpos, caret_before.bytepos, *start);
		...
		while (isspace(*start) || ispunct(*start)) {
			do {
				if (--start < string) {
					/* Invalid UTF-8 data was found in
					   STRING.  Just return the start of the
					   string in this case.  */
					caret.charpos = 0;
					caret.bytepos = 0;
					return caret;
		...
		if (punct_found && !(--factor)) {
			/* Punctuation was seen and the factor is now 0.  Return
			   the caret before the punctuation.  */
			DebugPrint("returning caret_before: char: %d byte: %td",
			    caret_before.charpos, caret_before.bytepos);
			return caret_before;
		}
...
static void
FindTextSections(const char *string, size_t string_size, TextPosition caret,
    XIMCaretDirection direction, int factor, TextPosition *start_return,
    TextPosition *end_return)
{
	...
	case XIMLineStart:
		...
		DebugPrint("start: found %p, found-string %td", found,
		    found - string);
	...
	end_line_start:
		DebugPrint("found %p string %p found+1-string %td", found,
	...
	DebugPrint("end: char: %d byte: %td", end.charpos, end.bytepos);
...
static Bool
MoveCaret(TextPosition *caret, const char *buffer, size_t buffer_size, int by)
{
...
#pragma GCC diagnostic pop
	...
	while (input_size > 0) {
		rc = iconv(cd, &inbuf, &input_size, &outptr, &outbytesleft);
		...
		if (rc == (size_t)-1) {
			...
			if (errno == E2BIG) {
				...
				outbuf = XLRealloc(outbuf,
				    outsize + BUFSIZ + 1);

				/* Move the outptr to the right location in the
				   new outbuf.  */
				...
				DebugPrint(
				    "expanding outsize to %zu, outbytesleft now %zu",
				    outsize, outbytesleft);
			} else {
				/* An error occured while encoding the string.
				   Normally, this is not such a big deal, but
				   the number of characters in the string is
				   later counted with mbtowc.  So, simply bail
				   out.  */
	...
	DebugPrint("conversion finished, size_out %zu", outsize - outbytesleft);
    ...
    XIMStringConversionCallbackStruct *call_data)
{
	...
	DebugPrint("string conversion; position: %d, factor: %d"
		   " operation: %u",
	    (short)call_data->position, call_data->factor,
	    call_data->operation);

	/* Obtain the actual caret position.  */
	caret = input->current_state.cursor;
	DebugPrint("current caret position: char: %d, byte: %td", caret.charpos,
	    caret.bytepos);
	...
	if (!MoveCaret(&caret, input->current_state.surrounding_text, length,
		position)) {
	...
	DebugPrint("new caret position: char %d, byte: %td", caret.charpos,
	    caret.bytepos);

	/* Now, obtain the start and end of the text to return.  */
	FindTextSections(input->current_state.surrounding_text, length, caret,
	    call_data->direction, call_data->factor, &start, &end);

	DebugPrint("start: %d, %td, end: %d, %td", start.charpos, start.bytepos,
	    end.charpos, end.bytepos);

	/* If either of those positions are invalid, signal failure.  */
	if (start.charpos < 0 || start.bytepos < 0 || end.charpos < 0 ||
	    end.bytepos < 0)
		goto failure;
	...
	buffer = EncodeIMString((input->current_state.surrounding_text +
				    start.bytepos),
	    end.bytepos - start.bytepos + 1, &num_characters);
...
#pragma GCC diagnostic pop
		...
		preedit_attr = XVaCreateNestedList(0, XNPreeditStartCallback,
		    &preedit_start_callback, XNPreeditDoneCallback,
		    &preedit_done_callback, XNPreeditDrawCallback,
		    &preedit_draw_callback, XNPreeditCaretCallback,
		    &preedit_caret_callback, NULL);
	}
	...
	DebugPrint("preedit attr: %p, status attr: %p", preedit_attr,
	    status_attr);
...
#pragma GCC diagnostic pop
	...
	if (preedit_attr && status_attr)
		input->xic = XCreateIC(current_xim, XNInputStyle, xim_style,
		    ...
		    XNStatusAttributes, status_attr, XNPreeditAttributes,
		    preedit_attr, XNStringConversionCallback,
		    &string_conversion_callback, NULL);
	else if (preedit_attr)
		input->xic = XCreateIC(current_xim, XNInputStyle, xim_style,
		    XNClientWindow, window, XNFocusWindow, window,
		    XNPreeditAttributes, preedit_attr,
		    XNStringConversionCallback, &string_conversion_callback,
		    NULL);
	else if (status_attr)
		input->xic = XCreateIC(current_xim, XNInputStyle, xim_style,
		    XNClientWindow, window, XNFocusWindow, window,
		    XNStatusAttributes, status_attr, XNStringConversionCallback,
		    &string_conversion_callback, NULL);
	else
		input->xic = XCreateIC(current_xim, XNInputStyle, xim_style,
		    XNClientWindow, window, XNFocusWindow, window,
...
static XIMStyle
CheckStyle(XIMStyles *styles, XIMStyle preedit_style, XIMStyle status_style)
{
	int i;
...
HandleNewIM(XIM xim)
{
	...
	XSetIMValues(xim, XNDestroyCallback, &destroy_callback, NULL);
    ...
    XPointer call_data)
{
	...
	newim = XOpenIM(compositor.display, XrmGetDatabase(compositor.display),
	    (char *)compositor.resource_name, (char *)compositor.app_name);
...
FocusInCallback(Seat *seat, Surface *surface)
{
	...
	info = GetClientInfo(wl_resource_get_client(surface->resource), seat,
	    False);
...
ConvertString(char *buffer, size_t nbytes, size_t *size_out)
{
	...
	/* Start converting.  */
	while (nbytes > 0) {
		rc = iconv(current_cd, &buffer, &nbytes, &outptr,
		    &outbytesleft);
...
static void
PreeditString(TextInput *input, const char *buffer, size_t buffer_size,
    ptrdiff_t cursor)
{
	char	    chunk[4000];
	...
	while (start < buffer_end) {
		...
		DebugPrint("end-start (%p-%p): %td", end, start, end - start);
...
static void
CommitString(TextInput *input, const char *buffer, size_t buffer_size)
{
	char	    chunk[4000];
	const char *start, *end;
	int	    skip;
...
LookupString(TextInput *input, XEvent *event, KeySym *keysym_return)
{
	...
	buffer = alloca(256);
	nbytes = XmbLookupString(input->xic, &event->xkey, buffer, 256, &keysym,
	    &status);
	...
	if (status == XBufferOverflow) {
		...
		nbytes = XmbLookupString(input->xic, &event->xkey, buffer,
		    nbytes + 1, &keysym, &status);
	}
...
static Bool
FilterInputCallback(Seat *seat, Surface *surface, void *event, KeyCode *keycode)
{
	XIDeviceEvent	    *xev;
	XEvent		     xkey;
	TextInputClientInfo *info;
	...
	DebugPrint("seat %p, surface %p, detail: %d, event: %lx", seat, surface,
	...
	if (info) {
		...
		if (input && input->xic) {
			DebugPrint(
			    "found enabled text input %p on client-seat info %p",
			    input, info);

			/* Convert the extension event into a fake core event
			   that the input method can understand.  */
			...
			if (!keysym && xev->type == XI_KeyRelease)
				*keycode = GetKeycode(&input->keysym_map,
				    xev->detail);
			else
				*keycode = CalculateKeycodeForEvent(&xkey,
				    keysym);
...
XLTextInputDispatchCoreEvent(Surface *surface, XEvent *event)
{
	...
	DebugPrint("dispatching core event to surface %p:\n"
		   ...
		   "\tstate: %x\n"
		   "\tkeycode: %x",
	    surface, event->xkey.type, event->xkey.serial,
	    event->xkey.send_event, event->xkey.window, event->xkey.subwindow,
	    event->xkey.subwindow, event->xkey.time, event->xkey.state,
	    event->xkey.keycode);
	...
	/* Next, find the client info associated with the surface for that
	   seat.  */
	info = GetClientInfo(wl_resource_get_client(surface->resource), im_seat,
	    False);
	...
	if (info) {
		...
		if (input) {
			DebugPrint("found enabled input %p on info %p", input,
			    info);

			/* Now, try to dispatch the core event.  First, look up
			   the text string.  */
			...
			if (event->xkey.subwindow & (1U << 31))
				DebugPrint(
				    "lookup failed; not dispatching event because"
				    " this is a key repeat");
			else {
				/* Since that failed, dispatch the event to the
				 * seat.  */
				DebugPrint(
				    "lookup failed; dispatching event to seat; "
				    "keysym is: %lu",
				    keysym);
				...
				/* If the event is a KeyPress event and a keysym
				   was looked up, calculate a keycode for the
				   keysym, and record in the text input's
				   keycode-keycode table, so the correct keycode
				   can be looked up upon the next KeyRelease
				   event.  X input methods tend not to filter
				   the KeyRelease events, so the KeyRelease
				   event for an event that changed the keysym
				   will be sent with the wrong keycode, which
				   does not matter much with X programs, but
				   leads to Wayland programs constantly
				   autorepeating the keycode for which a
				   KeyPress event was sent.  */

				if (event->xkey.type == KeyPress && keysym) {
					/* Compute the keycode.  */
					effective_keycode =
					    CalculateKeycodeForEvent(event,
						keysym);

					DebugPrint(
					    "inserting keycode %lu into map under %u",
					    effective_keycode,
					    event->xkey.keycode);
					InsertKeycode(&input->keysym_map,
					    event->xkey.keycode,
					    effective_keycode);
				} else if (event->xkey.type == KeyRelease) {
					/* If no keysym was committed by the
					   input method, use the keysym provided
					   in the last KeyPress event for the
					   keycode.  Otherwise, the input method
					   probably knows better than us, so use
					   the keysym provided by the input
					   method.  */

					if (!keysym) {
						effective_keycode = GetKeycode(
						    &input->keysym_map,
						    event->xkey.keycode);
						DebugPrint(
						    "obtained keycode %lu for keycode %u"
						    " while processing KeyRelease event",
						    keysym,
						    event->xkey.keycode);
					} else
						effective_keycode =
						    CalculateKeycodeForEvent(
							event, keysym);

					DebugPrint(
					    "removing keycode %u from map",
					    event->xkey.keycode);

					/* Remove the keycode from the
					   keycode-keysym map.  */
					RemoveKeysym(&input->keysym_map,
					    event->xkey.keycode);
				}

				/* Finally, if an effective keycode was
				   calculated, replace the keycode in the event
				   with it.  */

				if (effective_keycode)
					event->xkey.keycode = effective_keycode;

				XLSeatDispatchCoreKeyEvent(im_seat, surface,
				    event);
			}
		}
	}
...
InitFontset(void)
{
	...
	if (XrmQGetResource(rdb, namelist, classlist, &type, &value) &&
		...
		im_fontset = XCreateFontSet(compositor.display,
		    (char *)value.addr, &missing_charset_list,
...
XLInitTextInput(void)
{
	...
	text_input_manager_global = wl_global_create(compositor.wl_display,
	    &zwp_text_input_manager_v3_interface, 1, NULL, HandleBind);
	...
	if (im_fontset == NULL)
		fprintf(stderr,
		    "Unable to load any usable fontset for input methods\n");
