...
HandleResizeDone(void *key, void *data)
{
	...
	test		       = data;
	test->resize_callbacks = XLListRemove(test->resize_callbacks, key);
...
DestroyBacking(TestSurface *test)
{
	...
	XLListFree(test->resize_callbacks, XLSeatCancelResizeCallback);
...
NoteBounds(void *data, int min_x, int min_y, int max_x, int max_y)
{
	...
	if (test->bounds_width != bounds_width ||
		...
		XResizeWindow(compositor.display, test->window, bounds_width,
		    bounds_height);
...
MapTestSurface(TestSurface *test)
{
	...
	XResizeWindow(compositor.display, test->window, test->bounds_width,
	    test->bounds_height);
...
Commit(Surface *surface, Role *role)
{
	...
	if (surface->current_state.buffer && !(test->flags & IsSurfaceMapped))
		/* Map the surface now.  */
		MapTestSurface(test);
    ...
    Surface *activator_surface)
{
	...
	if (test->role.resource) {
		...
		test_surface_send_activated(test->role.resource,
		    timestamp.months, timestamp.milliseconds, resource);
	}
}

static void
GetResizeDimensions(Surface *surface, Role *role, int *width, int *height)
{
	TestSurface *test;
...
MoveResize(struct wl_client *client, struct wl_resource *resource,
    uint32_t edge, uint32_t serial, struct wl_resource *seat_resource)
{
	TestSurface *test;
	Seat	    *seat;
	void	    *key;
	...
	if (!test->role.surface) {
		wl_resource_post_error(resource,
		    TEST_MANAGER_ERROR_RESIZE_REJECTED,
	...
	test->resize_callbacks = XLListPrepend(test->resize_callbacks, key);
}
...
static void
SetScale(struct wl_client *client, struct wl_resource *resource, uint32_t scale)
{
	/* If the scale is invalid, reject it.  */
	if (!scale) {
		wl_resource_post_error(resource,
		    TEST_MANAGER_ERROR_INVALID_SCALE, "scale of 0 specified");
		return;
	}
    ...
    uint32_t id, struct wl_resource *surface_resource)
{
	...
	if (surface->role_type != AnythingType &&
		...
		wl_resource_post_error(resource,
		    TEST_MANAGER_ERROR_ROLE_PRESENT,
	...
	test->role.resource = wl_resource_create(client,
	    &test_surface_interface, wl_resource_get_version(resource), id);
	if (!test->role.resource) {
		wl_resource_post_no_memory(resource);
		XLFree(test);
	...
	SubcompositorSetNoteFrameCallback(test->subcompositor, NoteFrame, test);
    ...
    uint32_t id, uint32_t scale)
{
	...
	if (locked_output_scale) {
		/* The scale is already locked, so don't create another
		   lock.  */
		wl_resource_post_error(resource,
		    TEST_MANAGER_ERROR_SCALE_LOCK_EXISTS,
	...
	wl_resource_set_implementation(lock_resource, &scale_lock_impl, NULL,
	    HandleScaleLockResourceDestroy);
}

static void
GetTestSeat(struct wl_client *client, struct wl_resource *resource, uint32_t id)
{
...
HandleBind(struct wl_client *client, void *data, uint32_t version, uint32_t id)
{
	...
	resource = wl_resource_create(client, &test_manager_interface, version,
	    id);
	...
	wl_resource_set_implementation(resource, &test_manager_impl, NULL,
	    NULL);
...
XLInitTest(void)
{
	test_manager_global = wl_global_create(compositor.wl_display,
	    &test_manager_interface, 1, NULL, HandleBind);
}
