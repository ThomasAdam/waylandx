...
#define EglBufferSize(type) (offsetof(EglBuffer, u) + sizeof(type))
	...
	for (i = 0; i < num_x_formats; ++i) {
		if (formats[i].depth > format->depth &&
			...
			found = XRenderFindFormat(compositor.display,
				...
				PictFormatGreen | PictFormatBlue |
				PictFormatRedMask | PictFormatBlueMask |
				PictFormatGreenMask | PictFormatAlphaMask,
			    &target_format, 0);
			...
			if (found) {
				...
				for (j = 0; j < n_visuals; ++j) {
					...
					if (XRenderFindVisualFormat(
						compositor.display,
						visuals[j].visual) == found) {
						/* We got a usable visual with
						   an alpha channel otherwise
						   matching the characteristics
						   of the visual specified by
						   EGL.  Return.  */
...
FindVisual(VisualID visual, int *depth)
{
	...
	visuals = XGetVisualInfo(compositor.display,
	    VisualScreenMask | VisualIDMask, &vinfo, &nvisuals);
...
EglPickConfig(void)
{
	...
	if (!eglChooseConfig(egl_display, egl_config_attribs, &egl_config, 1,
		&n_configs))
    ...
    const char				    *fragment_shader)
{
	...
	glShaderSource(vertex, 1, &composite_rectangle_vertex_shader, NULL);
	...
	program->position = glGetAttribLocation(program->program, "pos");
	program->texture  = glGetUniformLocation(program->program, "texture");
	program->source	  = glGetUniformLocation(program->program, "source");
	program->invert_y = glGetUniformLocation(program->program, "invert_y");
...
EglCompileShaders(void)
{
	...
	clear_rect_program_pos_attrib = glGetAttribLocation(clear_rect_program,
	    "pos");
...
EglInitDisplay(void)
{
	...
	eglMakeCurrent(display, EGL_NO_SURFACE, EGL_NO_SURFACE, egl_context);
...
TryPreserveOnSwap(EGLSurface *surface)
{
	...
	eglSurfaceAttrib(egl_display, surface, EGL_SWAP_BEHAVIOR,
	    EGL_BUFFER_PRESERVED);
	value = 0;

	/* Check that enabling the attribute was successful.  */
	if (!eglQuerySurface(egl_display, surface, EGL_SWAP_BEHAVIOR, &value) ||
...
PictureFromTarget(RenderTarget target)
{
	...
	return XRenderCreatePicture(compositor.display, egl_target->source,
	    cursor_format, 0, &picture_attrs);
}
...
StartRender(RenderTarget target)
{
	...
	/* Set the viewport.  */
	glViewport(0, 0, egl_target->width, egl_target->height);
    ...
    int nboxes, int min_x, int min_y)
{
	...
	for (i = 0; i < nboxes; ++i) {
		...
		verts[i * 8 + 1] = -1.0f +
		    (egl_target->height - y2) / egl_target->height * 2;

		/* Top left.  */
		verts[i * 8 + 2] = -1.0f + x1 / egl_target->width * 2;
		verts[i * 8 + 3] = -1.0f +
		    (egl_target->height - y1) / egl_target->height * 2;
	...
	/* Upload the verts.  */
	glVertexAttribPointer(clear_rect_program_pos_attrib, 2, GL_FLOAT,
	    GL_FALSE, 0, verts);
...
FindProgram(EglBuffer *buffer)
{
	...
	default:
		/* Otherwise, return the ARGB or XRGB program depending on
		   whether or not an alpha channel is present.  */
		return (
		    buffer->flags & HasAlpha ? &argb_program : &xrgb_program);
	}
}
...
ComputeTransformMatrix(EglBuffer *buffer, DrawParams *params)
{
	...
	if (params->flags & TransformSet) {
		...
		ApplyInverseTransform(1, 1, &buffer->matrix, params->transform);
...
Composite(RenderBuffer buffer, RenderTarget target, Operation op, int src_x,
    int src_y, int x, int y, int width, int height, DrawParams *params)
{
	...
	if (egl_buffer->u.type != SinglePixelBuffer)
		...
		glUniform4f(program->source_color, egl_buffer->u.single_pixel.r,
		    egl_buffer->u.single_pixel.g, egl_buffer->u.single_pixel.b,
		    egl_buffer->u.single_pixel.a);

	glUniformMatrix3fv(program->source, 1, GL_FALSE, egl_buffer->matrix);
	glUniform1i(program->invert_y, egl_buffer->flags & InvertY);
	glVertexAttribPointer(program->position, 2, GL_FLOAT, GL_FALSE, 0,
	    verts);
	glVertexAttribPointer(program->texcoord, 2, GL_FLOAT, GL_FALSE, 0,
	    texcoord);
    ...
    RenderCompletionFunc callback, void *data)
{
	...
	if (egl_target->flags & IsPixmap)
		...
		ISwapBuffersWithDamage(egl_display, egl_target->surface, rects,
		    nboxes);
	}
...
WaitFence(RenderFence fence)
{
	...
	if (IWaitSync)
		...
		IClientWaitSync(egl_display, fence.pointer, 0, EGL_FOREVER_KHR);
...
BufferFromDmaBuf(DmaBufAttributes *attributes, Bool *error)
{
	...
	/* Find the DRM format in question so we can determine the right
	   target to use.  */
	format = FindDrmFormat(attributes->drm_format, attributes->modifier);
...
#define LoadPlane(no)                                                     \
	{                                                                 \
		...
		if (IQueryDmaBufModifiers &&                              \
		    attributes->modifier != DRM_FORMAT_MOD_INVALID) {     \
			attribs[i++] =                                    \
			    EGL_DMA_BUF_PLANE##no##_MODIFIER_LO_EXT;      \
			attribs[i++] = attributes->modifier & 0xffffffff; \
			attribs[i++] =                                    \
			    EGL_DMA_BUF_PLANE##no##_MODIFIER_HI_EXT;      \
...
BufferFromDmaBufAsync(DmaBufAttributes *attributes,
    DmaBufSuccessFunc success_callback, DmaBufFailureFunc failure_callback,
    void *callback_data)
{
	Bool	     error;
	RenderBuffer buffer;
...
static void
AddDrmFormat(uint32_t format, uint64_t modifier, int flags)
{
...
InitModifiersFor(uint32_t format)
{
	...
	/* First, look up how many modifiers are supported for the given
	   format.  */
	IQueryDmaBufModifiers(egl_display, format, 0, NULL, NULL,
	    &n_total_modifiers);
	...
	IQueryDmaBufModifiers(egl_display, format, n_total_modifiers, modifiers,
	    external_only, &n_total_modifiers);
...
InitBufferFuncs(void)
{
	...
	if (IQueryDisplayAttrib && IQueryDeviceString) {
		...
		if (HaveEglExtension1(extensions,
			"EGL_EXT_device_drm_render_node"))
			name = IQueryDeviceString(device,
			    EGL_DRM_RENDER_NODE_FILE_EXT);

		if (!name &&
		    HaveEglExtension1(extensions, "EGL_EXT_device_drm"))
			name = IQueryDeviceString(device,
	...
	failed:
		fprintf(stderr,
		    "Warning: failed to obtain device node of"
    ...
    DrawParams *params)
{
	...
	for (i = 0; i < nboxes; ++i) {
		...
		if (width <= 0 || height <= 0)
			/* The box is effectively empty because it straddles one
			   of the corners of the buffer, or is outside of the
			   buffer.  */
			continue;
...
static void
UpdateBuffer(RenderBuffer buffer, pixman_region32_t *damage, DrawParams *params)
{
	EglBuffer *egl_buffer;
	...
	if (!(egl_buffer->flags & IsTextureGenerated))
		...
		/* Upload all the contents to the buffer's texture if the buffer
		   ...
		   glEGLImageTargetTexture2DOES is still required and not doing
		   so may cause certain drivers to stop working in the future.
		   So still do it for buffers backed by EGLImages.  */
...
InitEgl(void)
{
	RegisterStaticRenderer("egl", &egl_render_funcs, &egl_buffer_funcs);
}
