...
HandleParamsResourceDestroy(struct wl_resource *resource)
{
	...
	if (params->flags & IsCallbackData)
		/* If params is callback data, simply clear params->resource,
		   and wait for a callback to be called.  */
		return;
...
static Buffer *
CreateBufferFor(BufferParams *params, RenderBuffer render_buffer, uint32_t id)
{
	Buffer		 *buffer;
	...
	buffer->resource = wl_resource_create(client, &wl_buffer_interface, 1,
	    id);
	...
	wl_resource_set_implementation(buffer->resource,
	    &zwp_linux_dmabuf_v1_buffer_impl, buffer,
    ...
    int32_t height, uint32_t format, uint32_t flags)
{
	...
	RenderBufferFromDmaBufAsync(&attributes, CreateSucceeded, CreateFailed,
	    params);
	return;
...
static void
CreatePlaceholderBuffer(struct wl_client *client, uint32_t id, int width,
    int height)
{
	Buffer *buffer;
	Bool	error;
	...
	/* A single pixel buffer is used because it is safe to read outside
	   them.  */
	error		      = False;
	buffer->render_buffer = RenderBufferFromSinglePixel(0, 0, 0, 0, &error);
    ...
    uint32_t id)
{
	...
	wl_resource_set_implementation(params->resource,
	    &zwp_linux_buffer_params_v1_impl, params,
	    HandleParamsResourceDestroy);
    ...
    uint32_t id)
{
	...
	wl_resource_set_implementation(feedback, &zld_feedback_v1_impl, NULL,
	    NULL);
	...
	zwp_linux_dmabuf_feedback_v1_send_format_table(feedback,
	    format_table_fd, format_table_size);
	...
	for (provider = 0; provider < num_device_nodes; ++provider) {
		...
		format_array_size = format_table_size /
		    sizeof(FormatModifierPair);
		format_array.size = format_array_size * sizeof(uint16_t);
		format_array.data = format_array_data = alloca(
		    format_array.size);
		...
		/* Send flags.  We don't currently support direct scanout, so
		   send nothing.  */
...
SendSupportedFormats(struct wl_resource *resource)
{
	...
	for (i = 0; i < n_drm_formats; ++i) {
		if (wl_resource_get_version(resource) < 3) {
			...
			if (supported_formats[i].drm_modifier ==
			...
			zwp_linux_dmabuf_v1_send_modifier(resource,
			    supported_formats[i].drm_format,
			    ModifierHigh(modifier), ModifierLow(modifier));
		}
	}
}
