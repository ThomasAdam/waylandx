...
DetermineServerTime(void)
{
	...
	if (IntAddWrapv(clock_ms, clock_spec.tv_nsec / 1000000, &clock_ms))
		goto overflow;
	...
	if (llabs((long long)server_time - (long long)truncated) <= 5)
		/* Since the difference between the server time and the
		   monotonic time is less than 5 ms, the server time is most
		   likely the monotonic time.  */
		compositor.server_time_monotonic = True;
	else {
	overflow:
		compositor.server_time_monotonic = False;
		fprintf(stderr,
		    "Warning: the X server time does not seem to"
...
HandleCmdline(Display *dpy, const char *socket, int argc, char **argv)
{
	...
	for (i = 1; i < argc; ++i) {
		...
		print_usage:
			...
			if (i + 1 >= argc) {
				fprintf(stderr,
				    "%s: option -class requires a value\n",
				    argv[0]);
				exit(1);
			}
			...
			if (i + 1 >= argc) {
				fprintf(stderr,
				    "%s: option -name requires a value\n",
				    argv[0]);
				exit(1);
			}
...
XLMain(int argc, char **argv)
{
	...
	compositor.wl_event_loop = wl_display_get_event_loop(wl_display);
