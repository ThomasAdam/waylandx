...
static void
DestroySurface(struct wl_client *client, struct wl_resource *resource)
{
    ...
    int32_t y, int32_t width, int32_t height)
{
	...
	pixman_region32_union_rect(&surface->pending_state.surface,
	    &surface->pending_state.surface, x, y, MIN(65535, width),
	    MIN(65535, height));
    ...
    uint32_t callback_id)
{
	...
	wl_resource_set_implementation(callback_resource, NULL, callback,
	    HandleCallbackResourceDestroy);
    ...
    struct wl_resource *region_resource)
{
	...
	if (region_resource) {
		...
		pixman_region32_union_rect(&surface->pending_state.input,
		    &surface->pending_state.input, 0, 0, 65535, 65535);
	}
...
{
	ViewSetTransform(surface->view, surface->current_state.transform);
}
...
ApplyScale(Surface *surface)
{
	...
	if (surface->factor != d) {
		...
		for (; subsurface; subsurface = subsurface->next) {
			...
			/* Make sure it still has a surface, since it should not
			   be in surface->subsurfaces otherwise.  */
...
ApplyOpaqueRegion(Surface *surface)
{
	...
	if (global_scale_factor == 1)
		ViewSetOpaque(surface->view, &surface->current_state.opaque);
	else {
...
ApplyInputRegion(Surface *surface)
{
	...
	/* These regions, along with the global damage, must be multipled by
	   the global scale factor.  */
	if (global_scale_factor == 1)
		ViewSetInput(surface->view, &surface->current_state.input);
	else {
		pixman_region32_init(&temp);
...
ApplyViewport(Surface *surface)
{
	...
	ViewSetViewport(surface->view, src_x, src_y, crop_width, crop_height,
...
CheckViewportValues(Surface *surface)
{
	...
	if (state->src_x + state->src_width - 1 >=
		width / state->buffer_scale ||
	    state->src_y + state->src_height - 1 >=
...
ApplySurfaceDamage(Surface *surface)
{
	...
	if (global_scale_factor == 1)
		ViewDamage(surface->view, &surface->current_state.surface);
	else {
		pixman_region32_init(&temp);
...
SavePendingState(Surface *surface)
{
	...
	if (surface->pending_state.pending & PendingBufferTransform)
		surface->cached_state.transform =
		    surface->pending_state.transform;
...
InternalCommit1(Surface *surface, State *pending)
{
	...
	if (pending->pending & PendingBuffer) {
		...
		if (pending->buffer) {
			AttachBuffer(&surface->current_state, pending->buffer);
			ApplyBuffer(surface);
			ClearBuffer(pending);

			/* Check that any applied viewport source rectangles
			   remain valid.  */
	...
	if (pending->pending & PendingFrameCallbacks) {
		...
		if (pending->frame_callbacks.next !=
		    &pending->frame_callbacks) {
...
Commit(struct wl_client *client, struct wl_resource *resource)
{
	...
	if (surface->role &&
	    surface->role->funcs.early_commit
...
static Bool
GetBufferTransform(int32_t wayland_transform, BufferTransform *transform)
{
...
static void
DamageBuffer(struct wl_client *client, struct wl_resource *resource, int32_t x,
    int32_t y, int32_t width, int32_t height)
{
	Surface *surface;
	...
	pixman_region32_union_rect(&surface->pending_state.damage,
	    &surface->pending_state.damage, x, y, MIN(65535, width),
	    MIN(65535, height));
...
InitState(State *state)
{
	...
	pixman_region32_init_rect(&state->input, 0, 0, 65535, 65535);
...
HandleSurfaceDestroy(struct wl_resource *resource)
{
	...
	XLListFree(surface->subsurfaces, NotifySubsurfaceDestroyed);
...
void
XLCreateSurface(struct wl_client *client, struct wl_resource *resource,
    uint32_t id)
{
	Surface *surface;
	...
	surface->scale_callback_key = XLAddScaleChangeCallback(surface,
	    HandleScaleChanged);
...
XLSurfaceRunFrameCallbacks(Surface *surface, struct timespec time)
{
	...
	if (IntMultiplyWrapv(time.tv_sec, 1000, &ms_time))
		ms_time = UINT64_MAX;
	else if (IntAddWrapv(ms_time, time.tv_nsec / 1000000, &ms_time))
		ms_time = UINT64_MAX;

	RunFrameCallbacks(&surface->current_state.frame_callbacks, ms_time);
...
CommitCallback *
XLSurfaceRunAtCommit(Surface *surface, void (*commit_func)(Surface *, void *),
    void		     *data)
{
	CommitCallback *callback;
...
UnmapCallback *
XLSurfaceRunAtUnmap(Surface *surface, void (*unmap_func)(void *), void *data)
{
	UnmapCallback *callback;
...
{
	InternalCommit(surface,
	    (use_pending ? &surface->pending_state : &surface->cached_state));
}

DestroyCallback *
XLSurfaceRunOnFree(Surface *surface, void (*destroy_func)(void *), void *data)
{
	DestroyCallback *callback;
...
void *
XLSurfaceGetClientData(Surface *surface, ClientDataType type, size_t size,
    void (*free_func)(void *))
{
	ClientData *data;
...
XLWindowFromSurface(Surface *surface)
{
	...
	return surface->role->funcs.get_window(surface, surface->role);
}
    ...
    int new_width, int new_height)
{
	...
	surface->role->funcs.post_resize(surface, surface->role, west_motion,
...
XLSurfaceMoveBy(Surface *surface, int west, int north)
{
	...
	surface->role->funcs.move_by(surface, surface->role, west, north);
}
...
XLSurfaceNoteFocus(Surface *surface, FocusMode focus)
{
	...
	case SurfaceFocusOut:
		surface->num_focused_seats = MAX(0,
		    surface->num_focused_seats - 1);
	...
	surface->role->funcs.note_focus(surface, surface->role, focus);
}
...
void
SurfaceToWindow(Surface *surface, double x, double y, double *x_out,
    double *y_out)
{
...
void
ScaleToWindow(Surface *surface, double width, double height, double *width_out,
    double *height_out)
{
...
void
WindowToSurface(Surface *surface, double x, double y, double *x_out,
    double *y_out)
{
	*x_out = x / surface->factor - surface->input_delta_x;
	*y_out = y / surface->factor - surface->input_delta_y;
...
void
ScaleToSurface(Surface *surface, double width, double height, double *width_out,
    double *height_out)
{
	*width_out  = width / surface->factor;
	*height_out = height / surface->factor;
...
void
TruncateSurfaceToWindow(Surface *surface, int x, int y, int *x_out, int *y_out)
{
	*x_out = x * surface->factor + surface->input_delta_x;
	*y_out = y * surface->factor + surface->input_delta_y;
}
...
void
TruncateScaleToWindow(Surface *surface, int width, int height, int *width_out,
    int *height_out)
{
	*width_out  = width * surface->factor;
	*height_out = height * surface->factor;
...
void
TruncateWindowToSurface(Surface *surface, int x, int y, int *x_out, int *y_out)
{
	*x_out = x / surface->factor - surface->input_delta_x;
	*y_out = y / surface->factor - surface->input_delta_y;
}
...
void
TruncateScaleToSurface(Surface *surface, int width, int height, int *width_out,
    int *height_out)
{
	*width_out  = width / surface->factor;
	*height_out = height / surface->factor;
