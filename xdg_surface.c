...
AllBuffersReleased(void *data)
{
	...
	if (surface && role->state & StatePendingFrameCallback) {
		RunFrameCallbacks(surface, role, role->pending_frame_time);
...
static void
GetToplevel(struct wl_client *client, struct wl_resource *resource, uint32_t id)
{
	XdgRole *role;
...
static void
GetPopup(struct wl_client *client, struct wl_resource *resource, uint32_t id,
...
Commit(Surface *surface, Role *role)
{
...
#ifdef DEBUG_GEOMETRY_CALCULATION
		fprintf(stderr,
		    "Client set window geometry to: [%d %d %d %d]\n"
		    "State is: %d\n",
		    xdg_role->current_state.window_geometry_x,
		    xdg_role->current_state.window_geometry_y,
		    xdg_role->current_state.window_geometry_width,
	...
	xdg_role->impl->funcs.commit(role, surface, xdg_role->impl);
...
Setup(Surface *surface, Role *role)
{
	...
	xdg_role = XdgRoleFromRole(role);
	ViewSetSubcompositor(surface->view, xdg_role->subcompositor);
	ViewSetSubcompositor(surface->under, xdg_role->subcompositor);

	/* Make sure the under view ends up beneath surface->view.  */
	SubcompositorInsert(xdg_role->subcompositor, surface->under);
	SubcompositorInsert(xdg_role->subcompositor, surface->view);
...
ReleaseBuffer(Surface *surface, Role *role, ExtBuffer *buffer)
{
	...
	if (RenderIsBufferIdle(render_buffer, xdg_role->target))
		...
		ReleaseBufferWithHelper(xdg_role->release_helper, buffer,
		    xdg_role->target);
...
static void
OpaqueRegionChanged(Subcompositor *subcompositor, void *client_data,
    pixman_region32_t *opaque_region)
{
	...
	XChangeProperty(compositor.display, role->window, _NET_WM_OPAQUE_REGION,
...
static void
InputRegionChanged(Subcompositor *subcompositor, void *data,
    pixman_region32_t *input_region)
{
	XdgRole	       *role;
	int		nrects, i;
	...
	XShapeCombineRectangles(compositor.display, role->window, ShapeInput, 0,
	    0, rects, nrects,
...
NoteConfigure(XdgRole *role, XEvent *event)
{
	...
	if (role->role.surface) {
		...
		XLUpdateSurfaceOutputs(role->role.surface,
		    ...
		    event->xconfigure.y + role->min_y, -1, -1);
...
NoteBounds(void *data, int min_x, int min_y, int max_x, int max_y)
{
	...
	if (role->state & StateWaitingForAckCommit)
		/* Don't resize the window until all configure events are
		   ...
		   this, because Firefox updates subsurfaces while the old size
		   is still in effect.  */
		return;
	...
	if (role->bounds_width != bounds_width ||
		...
		if (role->impl->funcs.note_window_pre_resize)
			role->impl->funcs.note_window_pre_resize(&role->role,
			    role->impl, bounds_width, bounds_height);

		XResizeWindow(compositor.display, role->window, bounds_width,
		    bounds_height);
	...
	if (min_x != role->min_x || min_y != role->min_y) {
		...
		XMoveWindow(compositor.display, role->window,
		    root_x + min_x + role->min_x, root_y + min_y + role->min_y);
		run_reconstrain_callbacks = True;
...
ResizeForMap(XdgRole *role)
{
	...
	SubcompositorBounds(role->subcompositor, &min_x, &min_y, &max_x,
	    &max_y);
	...
	XResizeWindow(compositor.display, role->window, max_x - min_x + 1,
	    max_y - min_y + 1);

	if (role->impl->funcs.note_window_resized)
		role->impl->funcs.note_window_resized(&role->role, role->impl,
		    max_x - min_x + 1, max_y - min_y + 1);
}

static void
GetResizeDimensions(Surface *surface, Role *role, int *x_out, int *y_out)
{
...
static void
PostResize(Surface *surface, Role *role, int west_motion, int north_motion,
    int new_width, int new_height)
{
	...
	xdg_role->impl->funcs.post_resize(role, xdg_role->impl, west_motion,
	    north_motion, new_width, new_height);
}
...
static void
SelectExtraEvents(Surface *surface, Role *role, unsigned long event_mask)
{
	XdgRole *xdg_role;
...
NoteFocus(Surface *surface, Role *role, FocusMode focus)
{
	...
	if (xdg_role->impl && xdg_role->impl->funcs.note_focus)
		xdg_role->impl->funcs.note_focus(role, xdg_role->impl, focus);
}
...
static void
Activate(Surface *surface, Role *role, int deviceid, Timestamp timestamp,
    Surface *activator_surface)
{
	...
	if (xdg_role->impl && xdg_role->impl->funcs.activate)
		xdg_role->impl->funcs.activate(role, xdg_role->impl, deviceid,
		    timestamp.milliseconds, activator_surface);
}
    ...
    uint32_t id, struct wl_resource *surface_resource)
{
	...
	role->window = XCreateWindow(compositor.display,
	    DefaultRootWindow(compositor.display), 0, 0, 20, 20, 0,
	...
	role->sync_helper   = MakeSyncHelper(role->subcompositor, role->window,
	      role->target, HandleFrameCallback, &role->role);
	SyncHelperSetResizeCallback(role->sync_helper, HandleResize,
	    CheckFastForward);

	SubcompositorSetTarget(role->subcompositor, &role->target);
	SubcompositorSetInputCallback(role->subcompositor, InputRegionChanged,
	    role);
	SubcompositorSetOpaqueCallback(role->subcompositor, OpaqueRegionChanged,
	    role);
	SubcompositorSetBoundsCallback(role->subcompositor, NoteBounds, role);
...
void
XLXdgRoleCalcNewWindowSize(Role *role, int width, int height, int *new_width,
    int *new_height)
{
	XdgRole *xdg_role;
	...
	SubcompositorBounds(xdg_role->subcompositor, &min_x, &min_y, &max_x,
	    &max_y);
	...
	/* Adjust the current_width and current_height by the scale
	   factor.  */
	TruncateScaleToSurface(role->surface, current_width, current_height,
...
XLXdgRoleGetWidth(Role *role)
{
	...
	SubcompositorBounds(xdg_role->subcompositor, &x, &y, &x1, &y1);
...
XLXdgRoleInputRegionContains(Role *role, int x, int y)
{
	...
	xdg_role = XdgRoleFromRole(role);
	return pixman_region32_contains_point(&xdg_role->input_region, x, y,
	    &dummy_box);
}
...
XLXdgRoleMoveBy(Role *role, int west, int north)
{
	...
	CurrentRootPosition(xdg_role, &root_x, &root_y);
	XMoveWindow(compositor.display, xdg_role->window, root_x - west,
	    root_y - north);
...
XLInitXdgSurfaces(void)
{
	...
	if (!XAllocColor(compositor.display, compositor.colormap, &alloc)) {
	...
	if (!XShapeQueryExtension(compositor.display, &shape_base,
		&shape_error)) {
		fprintf(stderr,
		    "The Nonrectangular Window Shape extension is not"
		    " present on the X server\n");
		exit(1);
	}

	if (!XShapeQueryVersion(compositor.display, &shape_major,
		&shape_minor)) {
		fprintf(stderr,
		    "A supported version of the Nonrectangular Window"
	...
	if (shape_major < 1 || (shape_major == 1 && shape_minor < 1)) {
		fprintf(stderr,
		    "The version of the Nonrectangular Window Shape"
...
void
XLXdgRoleHandlePing(Role *role, XEvent *event, void (*reply_func)(XEvent *))
{
	XdgRole	  *xdg_role;
	PingEvent *record;
