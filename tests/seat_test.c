...
static void	expect_keyboard_modifiers_event(uint32_t, uint32_t, uint32_t,
	uint32_t);
...
make_test_subsurface(void)
{
	...
	subsurface->surface = wl_compositor_create_surface(display->compositor);

	if (!subsurface->surface)
		goto error_2;

	subsurface->subsurface = wl_subcompositor_get_subsurface(subcompositor,
	    subsurface->surface, wayland_surface);
...
run_click_test(struct test_XIButtonState *button_state)
{
	...
	test_seat_controller_dispatch_XI_ButtonPress(display->seat->controller,
	    test_get_time(), TEST_SOURCE_DEVICE, 1, test_get_root(),
	    test_surface_window, None, wl_fixed_from_double(1.0),
	    wl_fixed_from_double(2.0), wl_fixed_from_double(1.0),
	    wl_fixed_from_double(2.0), 0, button_state, NULL, NULL, NULL);
	test_XIButtonState_add_button(button_state, 1);

	test_seat_controller_dispatch_XI_ButtonPress(display->seat->controller,
	    test_get_time(), TEST_SOURCE_DEVICE, 2, test_get_root(),
	    test_surface_window, None, wl_fixed_from_double(1.0),
	    wl_fixed_from_double(2.0), wl_fixed_from_double(1.0),
	    wl_fixed_from_double(2.0), 0, button_state, NULL, NULL, NULL);
	...
	test_seat_controller_dispatch_XI_Motion(display->seat->controller,
	    test_get_time(), TEST_SOURCE_DEVICE, 0, test_get_root(),
	    test_surface_window, None, wl_fixed_from_double(550.0),
	    wl_fixed_from_double(550.0), wl_fixed_from_double(550.0),
	    wl_fixed_from_double(550.0), 0, button_state, NULL, NULL, NULL);
	test_seat_controller_dispatch_XI_Leave(display->seat->controller,
	    test_get_time(), TEST_SOURCE_DEVICE, XINotifyAncestor,
	    test_get_root(), test_surface_window, None,
	    wl_fixed_from_double(550.0), wl_fixed_from_double(550.0),
	    wl_fixed_from_double(550.0), wl_fixed_from_double(550.0),
	    XINotifyNormal, False, True, button_state, NULL, NULL);

	/* And release the buttons.  */
	test_seat_controller_dispatch_XI_ButtonRelease(
	    display->seat->controller, test_get_time(), TEST_SOURCE_DEVICE, 1,
	    test_get_root(), test_surface_window, None,
	    wl_fixed_from_double(550.0), wl_fixed_from_double(550.0),
	    wl_fixed_from_double(550.0), wl_fixed_from_double(550.0), 0,
	    button_state, NULL, NULL, NULL);
	test_XIButtonState_remove_button(button_state, 2);

	test_seat_controller_dispatch_XI_ButtonRelease(
	    display->seat->controller, test_get_time(), TEST_SOURCE_DEVICE, 2,
	    test_get_root(), test_surface_window, None,
	    wl_fixed_from_double(550.0), wl_fixed_from_double(550.0),
	    wl_fixed_from_double(550.0), wl_fixed_from_double(550.0), 0,
	    button_state, NULL, NULL, NULL);
	...
	test_seat_controller_dispatch_XI_Leave(display->seat->controller,
	    ...
	    wl_fixed_from_double(550.0), wl_fixed_from_double(550.0),
	    wl_fixed_from_double(550.0), wl_fixed_from_double(550.0),
	    XINotifyUngrab, False, True, button_state, NULL, NULL);
    ...
    struct test_subsurface		*child)
{
	...
	test_seat_controller_dispatch_XI_Enter(display->seat->controller,
	    ...
	    wl_fixed_from_double(0.0), wl_fixed_from_double(0.0),
	    XINotifyNormal, False, True, NULL, NULL, NULL);
	test_seat_controller_dispatch_XI_Motion(display->seat->controller,
	    test_get_time(), TEST_SOURCE_DEVICE, 0, test_get_root(),
	    test_surface_window, None, wl_fixed_from_double(0.0),
	    wl_fixed_from_double(0.0), wl_fixed_from_double(0.0),
	    wl_fixed_from_double(0.0), 0, button_state, NULL, NULL, NULL);
	test_seat_controller_dispatch_XI_Motion(display->seat->controller,
	    test_get_time(), TEST_SOURCE_DEVICE, 0, test_get_root(),
	    test_surface_window, None, wl_fixed_from_double(150.0),
	    wl_fixed_from_double(150.0), wl_fixed_from_double(150.0),
	    wl_fixed_from_double(150.0), 0, button_state, NULL, NULL, NULL);
	test_seat_controller_dispatch_XI_ButtonPress(display->seat->controller,
	    test_get_time(), TEST_SOURCE_DEVICE, 1, test_get_root(),
	    test_surface_window, None, wl_fixed_from_double(150.0),
	    wl_fixed_from_double(150.0), wl_fixed_from_double(150.0),
	...
	test_seat_controller_dispatch_XI_Motion(display->seat->controller,
	    test_get_time(), TEST_SOURCE_DEVICE, 0, test_get_root(),
	    test_surface_window, None, wl_fixed_from_double(95.0),
	    wl_fixed_from_double(95.0), wl_fixed_from_double(95.0),
	    wl_fixed_from_double(95.0), 0, button_state, NULL, NULL, NULL);
	test_seat_controller_dispatch_XI_ButtonRelease(
	    display->seat->controller, test_get_time(), TEST_SOURCE_DEVICE, 1,
	    test_get_root(), test_surface_window, None,
	    wl_fixed_from_double(95.0), wl_fixed_from_double(90.0),
	    wl_fixed_from_double(95.0), wl_fixed_from_double(95.0), 0,
	    button_state, NULL, NULL, NULL);
	test_XIButtonState_remove_button(button_state, 1);
...
run_valuator_test(void)
{
	...
	info = test_device_controller_get_device_info(
	...
	test_XIDeviceInfo_add_XIScrollClassInfo(info, TEST_SOURCE_DEVICE, 1,
	    XIScrollTypeVertical, wl_fixed_from_double(1.0),
	    XIScrollFlagPreferred);
	test_XIDeviceInfo_add_XIScrollClassInfo(info, TEST_SOURCE_DEVICE, 2,
	    XIScrollTypeHorizontal, wl_fixed_from_double(2.0),
	    XIScrollFlagPreferred);
	test_XIDeviceInfo_add_XIValuatorClassInfo(info, TEST_SOURCE_DEVICE, 1,
	    "Rel Scroll Vertical", wl_fixed_from_double(0.0),
	    wl_fixed_from_double(0.0), wl_fixed_from_double(0.0), 1,
	    XIModeRelative);
	test_XIDeviceInfo_add_XIValuatorClassInfo(info, TEST_SOURCE_DEVICE, 2,
	    "Rel Scroll Horizontal", wl_fixed_from_double(0.0),
	    wl_fixed_from_double(0.0), wl_fixed_from_double(0.0), 1,
	    XIModeRelative);
	...
	valuator_state = test_seat_controller_get_XIValuatorState(
	...
	test_seat_controller_dispatch_XI_Motion(display->seat->controller,
	    test_get_time(), TEST_SOURCE_DEVICE, 0, test_get_root(),
	    test_surface_window, None, wl_fixed_from_double(2.0),
	    wl_fixed_from_double(2.0), wl_fixed_from_double(2.0),
	    wl_fixed_from_double(2.0), 0, NULL, valuator_state, NULL, NULL);
...
run_key_test(void)
{
	test_seat_controller_dispatch_XI_FocusIn(display->seat->controller,
	    ...
	    wl_fixed_from_double(2.0), wl_fixed_from_double(2.0),
	    wl_fixed_from_double(2.0), wl_fixed_from_double(2.0),
	    XINotifyNonlinear, 0, 1, NULL, NULL, NULL);
	test_seat_controller_dispatch_XI_RawKeyPress(display->seat->controller,
	    test_get_time(), TEST_SOURCE_DEVICE, 67, 0, NULL);
	test_seat_controller_dispatch_XI_KeyPress(display->seat->controller,
	    test_get_time(), TEST_SOURCE_DEVICE, 67, test_get_root(),
	    test_surface_window, None, wl_fixed_from_double(2.0),
	    wl_fixed_from_double(2.0), wl_fixed_from_double(2.0),
	    wl_fixed_from_double(2.0), 0, NULL, NULL, NULL, NULL);
	test_seat_controller_dispatch_XI_RawKeyRelease(
	    display->seat->controller, test_get_time(), TEST_SOURCE_DEVICE, 67,
	    0, NULL);
...
run_resize_test(struct test_XIButtonState *button_state)
{
	...
	test_seat_controller_dispatch_XI_ButtonRelease(
	    ...
	    test_get_root(), test_surface_window, None,
	    wl_fixed_from_double(2.0), wl_fixed_from_double(2.0),
	    wl_fixed_from_double(2.0), wl_fixed_from_double(2.0), 0,
	    button_state, NULL, NULL, NULL);
	test_XIButtonState_remove_button(button_state, 1);
...
again:
	...
	case TEST_ENTRY_KIND:
		...
		test_seat_controller_dispatch_XI_Enter(
		    ...
		    test_surface_window, None, wl_fixed_from_double(0.0),
		    wl_fixed_from_double(0.0), wl_fixed_from_double(0.0),
		    wl_fixed_from_double(0.0), XINotifyNormal, False, True,
		    NULL, NULL, NULL);
		...
		test_seat_controller_dispatch_XI_Motion(
		    display->seat->controller, test_get_time(),
		    TEST_SOURCE_DEVICE, 0, test_get_root(), test_surface_window,
		    None, wl_fixed_from_double(1.0), wl_fixed_from_double(2.0),
		    wl_fixed_from_double(1.0), wl_fixed_from_double(2.0), 0,
		    NULL, NULL, NULL, NULL);
	...
	case TEST_CLICK_KIND:
		/* Test clicking and grab processing.  Press buttons 1 and 2,
		   ...
		   only the following events are sent in the given order:
		     ...
		     button (SERIAL, TIME, BTN_LEFT,
		   WL_POINTER_BUTTON_STATE_PRESSED) frame () button (SERIAL,
		   TIME, BTN_MIDDLE, WL_POINTER_BUTTON_STATE_PRESSED) frame ()
		     motion (TIME, 550.0, 550.0)
		     frame ()
		     button (SERIAL, TIME, BTN_LEFT,
		   WL_POINTER_BUTTON_STATE_RELEASED) frame () button (SERIAL,
		   TIME, BTN_MIDDLE, WL_POINTER_BUTTON_STATE_RELEASED) frame ()
		     leave ()
		     frame ()  */
		...
		if (!button_state)
			report_test_failure(
			    "failed to obtain button state resource");
	...
	case TEST_GRAB_KIND:
		/* Test subsurface grabbing.  Create a 100x100 child of the
		   ...
		   the given order:
		     ...
		     frame ()
		     button (SERIAL, TIME, BTN_LEFT,
		   WL_POINTER_BUTTON_STATE_PRESSED) frame () motion (TIME, -5.0,
		   -5.0) frame () button (SERIAL, TIME, BTN_LEFT,
		   WL_POINTER_BUTTON_STATE_RELASED) frame () leave () frame ()
	...
	case TEST_VALUATOR_KIND:
		/* Dispatch a leave event at -1, -1, and then attach the
		 following scroll valuator information to the seat:
		 ...
		 motion (TIME, 2.0, 2.0) (this motion event should arrive
		 because the entry event happened after the scroll valuator
		 information was recorded.  The first motion event to arrive
		 after that should be used to obtain the current value of the
		 valuator, and not for calculating scroll deltas.) frame ();
	...
	case TEST_RESIZE_KIND:
		/* Test simple resize (or rather movement).  Dispatch a
		   ...
		   following events are sent to the pointer and surface.
		     ...
		     button (SERIAL, TIME, BTN_LEFT,
		   WL_POINTER_BUTTON_STATE_PRESSED) frame () leave ()
		     resize_finished ()
		     frame ();
...
expect_axis_value120_event(uint32_t axis, int32_t value120)
{
	...
	if (event->kind == POINTER_AXIS_VALUE120_EVENT) {
		...
		if (axis_value120_event->axis == axis &&
			...
			report_test_failure(
			    "expected axis_value120 event received "
			    "with incorrect parameters (axis: %" PRIu32 ","
			    " value120: %" PRIi32 ")",
			    axis, value120);
	} else
		report_test_failure(
		    "expected axis_value120 event, but it was not "
		    "received");
}
    ...
    size_t num_keys)
{
	...
	if (event->kind == KEYBOARD_ENTER_EVENT) {
		...
		if (keyboard_enter_event->num_keys != num_keys ||
			...
			report_test_failure(
			    "expected keyboard_enter event passed"
...
expect_keyboard_key_event(uint32_t key, uint32_t state)
{
	...
	if (event->kind == KEYBOARD_KEY_EVENT) {
		keyboard_key_event = (struct test_recorded_keyboard_key_event *)
		    event;
    ...
    uint32_t locked, uint32_t group)
{
	...
	if (event->kind == KEYBOARD_MODIFIERS_EVENT) {
		...
		if (keyboard_modifiers_event->base != base ||
			...
			report_test_failure(
			    "expected keyboard_modifiers passed with"
			    " invalid parameters");
		else
			free(event);
	} else
		report_test_failure(
		    "expected keyboard_modifiers_event, but it was"
		    " not received");
}
...
static void
handle_pointer_leave(void *data, struct wl_pointer *wl_pointer, uint32_t serial,
    struct wl_surface *surface)
{
...
static void
handle_pointer_axis(void *data, struct wl_pointer *wl_pointer, uint32_t time,
