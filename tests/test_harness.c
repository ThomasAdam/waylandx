...
#else
#define PNG_JMPBUF(ptr) (*png_set_longjmp_fn(ptr, longjmp, sizeof(jmp_buf)))
#endif
    ...
    const char *display_string)
{
	...
	display->pixmap_formats = XListPixmapFormats(display->x_display,
...
{
	return (display->x_display != NULL && display->num_pixmap_formats &&
    ...
    const char *interface, uint32_t version)
{
	...
	if (!strcmp(interface, "wl_shm") && version >= 1)
		display->shm = wl_registry_bind(registry, id, &wl_shm_interface,
		    1);
	else if (!strcmp(interface, "test_manager"))
		display->test_manager = wl_registry_bind(registry, id,
		    &test_manager_interface, 1);
	else {
		/* Look through the user specified list of interfaces.  */
		for (i = 0; i < display->num_test_interfaces; ++i) {
			if (!strcmp(interface,
				display->interfaces[i].interface) &&
...
open_test_display(struct test_interface *interfaces, int num_interfaces)
{
	...
	test_manager_add_listener(display->test_manager, &test_manager_listener,
	    display);
	wl_display_roundtrip(display->display);
	...
	display->scale_lock = test_manager_get_scale_lock(display->test_manager,
	    1);
...
get_image_stride(struct test_display *display, int depth, int width)
{
	...
	for (i = 0; i < display->num_pixmap_formats; ++i) {
		if (display->pixmap_formats[i].depth == depth) {
			...
			scanline_pad =
			    display->pixmap_formats[i].bits_per_pixel;
...
struct wl_buffer *
upload_image_data(struct test_display *display, const char *data, int width,
    int height, int depth)
{
	...
	mapping = mmap(NULL, stride * height, PROT_WRITE, MAP_SHARED, fd, 0);
	...
	buffer = wl_shm_pool_create_buffer(pool, 0, width, height, stride,
	    (depth == 32 ? WL_SHM_FORMAT_ARGB8888 : WL_SHM_FORMAT_XRGB8888));
    ...
    struct test_surface		     **test_surface_return)
{
	...
	test_surface = test_manager_get_test_surface(display->test_manager,
	    surface);
...
load_png_image(struct test_display *display, const char *filename)
{
	...
	depth = (color_type == PNG_COLOR_TYPE_RGB_ALPHA ? 32 : 24);
	...
	buffer = upload_image_data(display, (const char *)image_data, width,
	    height, depth);
	test_manager_set_buffer_label(display->test_manager, buffer, filename);
...
write_image_data_1(XImage *image, const char *filename)
{
	...
	if ((image->depth != 24 && image->depth != 32) ||
		...
		report_test_failure(
		    "don't know how to save image of depth %d (bpp %d)",
	...
	/* TODO: determine the image format based on the visual of the
	   window.  */
	header.format = (image->depth == 24 ? IMAGE_DATA_XRGB8888_LE :
	...
	if (writev(fd, iovecs, 2) != iovecs[0].iov_len + iovecs[1].iov_len)
		report_test_failure("failed to write to output file %s",
		    filename);
...
static void
compare_single_row_8bpc4(unsigned char *data, unsigned char *xdata, size_t size,
    struct image_difference_statistics *statistics)
{
	...
	for (i = 0; i < size; ++i) {
		...
		statistics->min_diff = MIN(statistics->min_diff, diff);
		statistics->max_diff = MAX(statistics->max_diff, diff);
	}
}

static void
compare_single_row_8bpc4x1(unsigned char *data, unsigned char *xdata,
    size_t size, struct image_difference_statistics *statistics)
{
	unsigned char channel_a, channel_b;
	int	      diff;
	size_t	      i;
    ...
    struct image_difference_statistics *statistics)
{
	...
	xdata = (unsigned char *)(image->data + image->bytes_per_line * row_no);

	if (header->format == IMAGE_DATA_ARGB8888_LE)
		compare_single_row_8bpc4(data, xdata,
		    (bytes_per_pixel * header->width), statistics);
	else if (header->format == IMAGE_DATA_XRGB8888_LE)
		compare_single_row_8bpc4x1(data, xdata,
		    (bytes_per_pixel * header->width), statistics);
    ...
    const char *filename)
{
	...
	if (!image)
		report_test_failure("failed to load from drawable 0x%lx",
		    window);
    ...
    const char *filename)
{
	...
	if (data_bpp * 8 != image->bits_per_pixel)
		report_test_failure(
		    "image data has %d bits per pixel, but reference"
		    " data has %hd * 8",
		    image->bits_per_pixel, data_bpp);

	if (image->width != header.width || image->height != header.height)
		report_test_failure(
		    "image data is %d by %d, but reference data is"
		    " %hd by %hd",
		    image->width, image->height, header.width, header.height);
...
test_init(void)
{
	write_image_data_instead = getenv("TEST_WRITE_REFERENCE") != NULL;
}

static void
handle_seat_controller_device_id(void *data,
    struct test_seat_controller *controller, uint32_t device_id)
{
	struct test_display *display;
...
test_init_seat(struct test_display *display)
{
	...
	display->seat->controller = test_manager_get_test_seat(
	...
	display->seat->device_controller =
	    test_seat_controller_get_device_controller(
	...
	test_seat_controller_add_listener(display->seat->controller,
	    &seat_controller_listener, display);
	wl_display_roundtrip(display->display);
	...
	display->seat->seat =
	    test_seat_controller_bind_seat(display->seat->controller, 8);

	if (!display->seat->seat)
		report_test_failure("failed to bind to test seat");

	display->seat->pointer = wl_seat_get_pointer(display->seat->seat);

	if (!display->seat->pointer)
		report_test_failure("failed to bind to test pointer");

	display->seat->keyboard = wl_seat_get_keyboard(display->seat->seat);
...
void
verify_window_size(struct test_display *display, Window window, int width,
    int height)
{
	XWindowAttributes attrs;
