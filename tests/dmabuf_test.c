...
static struct wl_buffer *create_rainbow_buffer(uint32_t, uint64_t, uint32_t,
    uint32_t, uint32_t);
...
test_single_step(enum test_kind kind)
{
	...
	case ARGB8888_KIND:
		buffer = create_rainbow_buffer(GBM_FORMAT_ARGB8888,
		    DRM_FORMAT_MOD_INVALID, 0xffff0000, 0xff00ff00, 0xff0000ff);
	...
	case ARGB8888_LINEAR_KIND:
		...
		buffer = create_rainbow_buffer(GBM_FORMAT_ARGB8888,
		    DRM_FORMAT_MOD_LINEAR, 0xffff0000, 0xff00ff00, 0xff0000ff);
...
#endif
	{
		...
		buffer = create_rainbow_buffer(GBM_FORMAT_XBGR8888,
		    DRM_FORMAT_MOD_INVALID, 0x0000ff, 0x00ff00, 0xff0000);
...
static void
handle_feedback_done(void *data, struct zwp_linux_dmabuf_feedback_v1 *feedback)
{
    ...
    struct zwp_linux_dmabuf_feedback_v1 *feedback)
{
	...
	if (!test_data->recording_tranche)
		report_test_failure(
		    "tranche_done received but not recording tranche");
...
open_surface(void)
{
	...
	feedback = zwp_linux_dmabuf_v1_get_default_feedback(linux_dmabuf_v1);
	...
	if (!data.device || data.fd < 0 ||
	    (data.format_table_size % sizeof(struct format_modifier_pair)) ||
	    !data.tranches)
		report_test_failure(
		    "received invalid parameters from feedback");
	...
	for (tranche = data.tranches; tranche; tranche = tranche->next) {
		for (i = 0; i < tranche->n_indices; ++i) {
			if (tranche->indices[i] >=
				...
				report_test_failure(
				    "tranche index %" PRIu16 " extends outside"
...
is_format_supported(uint32_t format, uint64_t modifier)
{
	...
	for (tranche = feedback_tranches; tranche; tranche = tranche->next) {
		for (i = 0; i < tranche->n_indices; ++i) {
			...
			if (pair.format == format && pair.modifier == modifier)
				return true;
		}
	}
...
create_rainbow_buffer(uint32_t format, uint64_t modifier, uint32_t red_pixel,
    uint32_t green_pixel, uint32_t blue_pixel)
{
	...
	if (!is_format_supported(format, modifier))
		report_test_failure("the specified format %8" PRIx32
				    " with modifier"
				    " 0x%16" PRIx64 " is not supported",
		    format, modifier);

	if (modifier != DRM_FORMAT_MOD_INVALID)
		buffer_object = gbm_bo_create_with_modifiers2(gbm_device, 500,
		    500, format, &modifier, 1, GBM_BO_USE_RENDERING);
	else
	...
	zwp_linux_buffer_params_v1_add(params, fd, 0,
	    ...
	    gbm_bo_get_stride(buffer_object), modifier >> 32,
	    modifier & 0xffffffff);
