...
verify_async_used(void)
{
	...
	if (used_presentation_mode != 1)
		report_test_failure(
		    "async presentation not used where expected!");
}
...
verify_vsync_used(void)
{
	...
	if (used_presentation_mode == 1)
		report_test_failure(
		    "vsync presentation not used where expected!");
}
...
run_test(void)
{
	...
	tearing_control = wp_tearing_control_manager_v1_get_tearing_control(
	    manager, wayland_surface);
