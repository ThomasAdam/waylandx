...
static void
submit_surface_opaque_region(struct wl_surface *surface, int x, int y,
...
make_test_subsurface_with_parent(struct test_subsurface *parent)
{
	...
	subsurface->subsurface = wl_subcompositor_get_subsurface(subcompositor,
	    subsurface->surface, parent->surface);
...
test_single_step(enum test_kind kind)
{
	...
	case SUBSURFACE_UNDER_KIND:
		...
		subsurface_base_png = load_png_image(display,
		    "subsurface_base.png");

		if (!subsurface_base_png)
			report_test_failure(
			    "failed to load subsurface_base.png");

		subsurface_damage_png = load_png_image(display,
		    "subsurface_damage.png");

		if (!subsurface_damage_png)
			report_test_failure(
			    "failed to load subsurface_damage.png");
		...
		submit_surface_opaque_region(subsurfaces[0]->surface, 0, 0,
		    1024, 1024);
	...
	case SUBSURFACE_OVER_KIND:
		...
		subsurface_1_png = load_png_image(display, "subsurface_1.png");

		if (!subsurface_1_png)
			report_test_failure("failed to load subsurface_1.png");

		subsurface_1_damaged_png = load_png_image(display,
		    "subsurface_1_damaged.png");

		if (!subsurface_1_damaged_png)
			report_test_failure(
			    "failed to load subsurface_1_damaged.png");

		/* Now, move the surface a little.  */
		wl_subsurface_set_position(subsurfaces[1]->subsurface, 40, 40);

		/* Set an opaque region.  */
		submit_surface_opaque_region(subsurfaces[1]->surface, 0, 0, 256,
		    256);

		/* Damage the subsurface.  */
		wl_surface_attach(subsurfaces[1]->surface, subsurface_1_png, 0,
		    0);
		...
		wl_surface_attach(subsurfaces[1]->surface,
		    subsurface_1_damaged_png, 0, 0);
	...
	case SUBSURFACE_MOVE_KIND:
		...
		wl_subsurface_set_position(subsurfaces[1]->subsurface, 100,
		    100);
		wait_frame_callback(wayland_surface);
		sleep_or_verify();
	...
	case SUBSURFACE_STACKING_KIND:
		...
		subsurface_transparency_png = load_png_image(display,
		    "subsurface_transparency.png");

		if (!subsurface_transparency_png)
			report_test_failure(
			    "failed to load subsurface_transparency.png");
		...
		submit_surface_opaque_region(subsurfaces[0]->surface, 640, 640,
		    128, 128);
		wl_surface_damage(subsurfaces[0]->surface, 0, 0, 1024, 1024);
		wl_surface_commit(subsurfaces[0]->surface);
		wait_frame_callback(wayland_surface);
	...
	case SUBSURFACE_TREE_KIND:
		/* Create two subsurfaces attached to the other test subsurface;
		   1 below, 1 over.  */
		subsurfaces[2] = make_test_subsurface_with_parent(
		    subsurfaces[1]);
		subsurfaces[3] = make_test_subsurface_with_parent(
		    subsurfaces[1]);
		...
		cow_transparent_png = load_png_image(display,
		    "cow_transparent.png");

		if (!cow_transparent_png)
			report_test_failure(
			    "failed to load cow_transparent.png");
		...
		wl_surface_attach(subsurfaces[3]->surface, cow_transparent_png,
		    0, 0);
		submit_surface_opaque_region(subsurfaces[2]->surface, 0, 0, 100,
		    300);
		wl_surface_damage(subsurfaces[2]->surface, 0, 0, INT_MAX,
		    INT_MAX);
		wl_surface_damage(subsurfaces[3]->surface, 0, 0, INT_MAX,
		    INT_MAX);
		...
		/* Move subsurface 1 to where it gets obscured by the opaque
		   ...
		   accordingly.  */
		wl_subsurface_set_position(subsurfaces[1]->subsurface, 640,
		    640);
		wait_frame_callback(wayland_surface);
		sleep_or_verify();

		/* Move one of the views further to the right.  */
		wl_subsurface_set_position(subsurfaces[2]->subsurface, 100,
		    100);
		wl_surface_commit(subsurfaces[1]->surface);
		wait_frame_callback(wayland_surface);
		sleep_or_verify();
	...
	case SUBSURFACE_DESYNC_KIND:
		...
		subsurfaces[4] = make_test_subsurface_with_parent(
		    subsurfaces[0]);

		if (!subsurfaces[4])
			report_test_failure("failed to create subsurfaces");

		subsurfaces[5] = make_test_subsurface_with_parent(
		    subsurfaces[4]);
		subsurfaces[6] = make_test_subsurface_with_parent(
		    subsurfaces[4]);

		if (!subsurfaces[5] || !subsurfaces[6])
			report_test_failure("failed to create subsurfaces");

		subsurfaces[7] = make_test_subsurface_with_parent(
		    subsurfaces[6]);
		subsurfaces[8] = make_test_subsurface_with_parent(
		    subsurfaces[6]);
		...
		/* Now, attach subsurface_1.png in subsurfaces[7].  Nothing
		   should be displayed.  */
		wl_surface_attach(subsurfaces[7]->surface, subsurface_1_png, 0,
		    0);
		...
		wl_subsurface_set_position(subsurfaces[7]->subsurface, 100,
		    100);
	...
	case SUBSURFACE_COMPLEX_DAMAGE_KIND:
		...
		/* Move subsurfaces[8] to 270, 270.  This adds some damage to
		   quadrant D.  */
		wl_subsurface_set_position(subsurfaces[8]->subsurface, 600,
		    600);
		...
		/* Attach subsurface_1_complex.png to subsurfaces[7], and apply
		   ...
		   simplification.  */
		subsurface_1_complex_png = load_png_image(display,
		    "subsurface_1_complex.png");

		if (!subsurface_1_complex_png)
			report_test_failure(
			    "failed to load subsurface_1_complex.png");

		subsurface_transparency_damage_png = load_png_image(display,
		    "subsurface_transparency_damage.png");

		if (!subsurface_transparency_damage_png)
			report_test_failure(
			    "failed to load subsurface_transparency_damage.png");
		...
		wl_surface_attach(subsurfaces[7]->surface,
		    subsurface_1_complex_png, 0, 0);
		...
		wl_surface_attach(subsurfaces[0]->surface,
		    subsurface_transparency_damage_png, 0, 0);
	...
	case SUBSURFACE_REPARENT_KIND:
		subsurface_stack_1_png = load_png_image(display,
		    "subsurface_stack_1.png");

		if (!subsurface_stack_1_png)
			report_test_failure(
			    "failed to load subsurface_stack_1.png");

		subsurface_stack_2_png = load_png_image(display,
		    "subsurface_stack_2.png");

		if (!subsurface_stack_2_png)
			report_test_failure(
			    "failed to load subsurface_stack_2.png");
		...
		recreate_subsurface(subsurfaces[6], subsurfaces[4]->surface);
		wait_frame_callback(wayland_surface);
		sleep_or_verify();
		...
		subsurfaces[9] = make_test_subsurface_with_parent(
		    subsurfaces[6]);

		if (!subsurfaces[9])
			report_test_failure("failed to create subsurface");

		wl_surface_attach(subsurfaces[9]->surface,
		    subsurface_stack_1_png, 0, 0);
		wl_surface_damage(subsurfaces[9]->surface, 0, 0, 100, 100);
		wl_surface_commit(subsurfaces[9]->surface);
		wl_subsurface_set_position(subsurfaces[9]->subsurface, 600,
		    600);

		/* The second, subsurfaces[10] is a child of subsurfaces[6].  It
		   is also placed at 600, 600, on top of subsurfaces[9].  */
		subsurfaces[10] = make_test_subsurface_with_parent(
		    subsurfaces[6]);

		if (!subsurfaces[10])
			report_test_failure("failed to create subsurface");

		wl_surface_attach(subsurfaces[10]->surface,
		    subsurface_stack_2_png, 0, 0);
		wl_surface_damage(subsurfaces[10]->surface, 0, 0, 100, 100);
		wl_surface_commit(subsurfaces[10]->surface);
		wl_subsurface_set_position(subsurfaces[10]->subsurface, 600,
		    600);
