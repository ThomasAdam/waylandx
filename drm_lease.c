...
DeleteConnector(DrmLeaseConnector *connector)
{
	...
	DebugPrint("destroying connector %p (crtc %lu output %lu)", connector,
	    connector->crtc, connector->output);
...
DeleteDevice(DrmLeaseDevice *device)
{
	...
	DebugPrint("destroying device %p (%lu) w/ fd %d", device,
	    device->provider, device->fd);
...
CollectDeadResources(void)
{
	...
	while (connector_ref != &all_connector_references) {
		DebugPrint(
		    "marked via connector: connector %p, device %p (%lu)",
		    connector_ref->connector, connector_ref->connector->device,
	...
	while (request != &all_lease_requests) {
		...
		while (item != &request->outputs) {
			DebugPrint(
			    "marked via req: connector %p, device %p (%lu)",
	...
	while (device != &all_devices) {
		...
		while (connector != &device->outputs) {
			DebugPrint("judging connector %p of device %p",
			...
			if (!(last_connector->flags & IsMarked)) {
				DebugPrint(
				    "connector %lu %lu is no longer marked",
				    last_connector->output,
				    last_connector->crtc);
...
#define DuplicateConnector WP_DRM_LEASE_REQUEST_V1_ERROR_DUPLICATE_CONNECTOR
...
HandleLeaseResourceDestroy(struct wl_resource *resource)
{
	...
	if (lease->lease) {
		...
		if (error) {
			DebugPrint("rid: %" PRIu32 ", minor: %" PRIu16
				   ", major: %" PRIu8 ", "
				   "error: %" PRIu8,
...
Submit(struct wl_client *client, struct wl_resource *resource, uint32_t id)
{
	...
	while (item != &request->outputs) {
		if (item->connector->flags & IsRemoved) {
			DebugPrint(
			    "removed connector was used in lease request");

			/* The connector was removed.  This means it can no
			   longer be leased.  */
			wl_resource_set_implementation(lease->resource,
			    &drm_lease_impl, lease, HandleLeaseResourceDestroy);
		...
		DebugPrint("adding output: %u crtc: %u", outputs[i - 1],
		    crtcs[i - 1]);
	...
	cookie = xcb_randr_create_lease(compositor.conn,
	    DefaultRootWindow(compositor.display), lease_id, request->noutputs,
	...
	wl_resource_set_implementation(lease->resource, &drm_lease_impl, lease,
	    HandleLeaseResourceDestroy);
    ...
    uint32_t id)
{
	...
	wl_resource_set_implementation(request->resource,
	    &drm_lease_request_impl, request, HandleRequestResourceDestroy);
}
...
AddConnectorRef(DrmLeaseConnector *connector, DrmLeaseDeviceRef *ref)
{
	DrmLeaseConnectorRef *connector_ref;
	...
	connector_ref->resource = wl_resource_create(client,
	    ...
	    wl_resource_get_version(ref->resource), 0);
	...
	wl_resource_set_implementation(connector_ref->resource,
	    &drm_lease_connector_impl, connector_ref,
	...
	while (connector != &device->outputs) {
		if (!(connector->flags & IsDisconnected) &&
			...
			wp_drm_lease_connector_v1_send_connector_id(
			    connector_ref->resource, connector->connector_id);

			/* Send the unique connector name.  */
			sprintf(buf, "%d", connector->connector_id);
			wp_drm_lease_connector_v1_send_name(
			    connector_ref->resource, buf);

			/* Send the connector description.  */
			wp_drm_lease_connector_v1_send_description(
			    connector_ref->resource, connector->name);

			/* Send done.  */
			wp_drm_lease_connector_v1_send_done(
			    connector_ref->resource);
		}
...
AddProvider(RRProvider provider)
{
	...
	cookie = xcb_dri3_open(compositor.conn,
	    DefaultRootWindow(compositor.display), provider);
	...
	device->global = wl_global_create(compositor.wl_display,
	    &wp_drm_lease_device_v1_interface, 1, device, HandleBind);
    ...
    XRROutputInfo *info)
{
	...
	CatchXErrors();
	rc = XRRGetOutputProperty(compositor.display, output, CONNECTOR_ID, 0,
	    1, False, False, XA_INTEGER, &actual_type, &actual_format, &nitems,
	    &bytes_after, &data);
	UncatchXErrors(NULL);

	if (rc != Success || !data || actual_format != 32 || nitems < 1 ||
...
InitializeProviderOutputs(void)
{
	...
	screen_resources = XRRGetScreenResources(compositor.display, root);

	device = all_devices.next;
	while (device != &all_devices) {
		CatchXErrors();
		info = XRRGetProviderInfo(compositor.display, screen_resources,
		    device->provider);
		UncatchXErrors(NULL);
		...
		DebugPrint("obtained provider info %lu; cap: %u"
			   " ncrtcs: %d noutputs %d",
		    device->provider, info->capabilities, info->ncrtcs,
		    info->noutputs);
		...
		for (i = 0; i < info->noutputs; ++i) {
			...
			output = XRRGetOutputInfo(compositor.display,
			    screen_resources, info->outputs[i]);
			UncatchXErrors(NULL);
...
BuildProviderTree(void)
{
	...
	for (i = 0; i < tree->nproviders; i++)
		cookies[i] = xcb_randr_get_provider_info(compositor.conn,
		    tree->providers[i], reply_timestamp);
	noutputs = 0;
	...
	for (i = 0; i < tree->nproviders; i++) {
		...
		if (replies[i])
			...
			noutputs += xcb_randr_get_provider_info_outputs_length(
			    replies[i]);
	}
	...
	for (i = 0, j = 0; i < tree->nproviders; ++i) {
		...
		num_outputs = xcb_randr_get_provider_info_outputs_length(
		    replies[i]);
		...
		for (k = 0; k < num_outputs; ++k)
			output_cookies[k] = xcb_randr_get_output_info(
			    compositor.conn, outputs[k], reply_timestamp);

		for (k = 0; k < num_outputs; ++k) {
			error		  = NULL;
			output_replies[k] = xcb_randr_get_output_info_reply(
			    compositor.conn, output_cookies[k], &error);
			...
			DebugPrint("nconnectors[%d] became: %d", j,
			    tree->nconnectors[j]);
...
static void
HandleSingleProvider(ProviderOutputTree *tree, int index, int connector_offset)
{
	RRProvider			    provider;
	xcb_randr_output_t		   *outputs;
	...
	if (!device) {
		...
		while (connector != &device->outputs) {
			...
			for (i = 0; i < tree->nconnectors[index]; ++i) {
				DebugPrint("consideration: %p %" PRIu32 " %lu",
				    connector, outputs[i], connector->output);
		...
		for (i = 0; i < tree->nconnectors[index]; ++i) {
			...
			if (!connector) {
				...
				/* It seems a little wrong to use a fake output
				   info structure.  */
				connector = AddOutput(device, outputs[i],
				    info[i]->crtc, &outputinfo);
				SendConnectorToClients(connector);
				DebugPrint("added output named %s",
				    outputinfo.name);
			...
			/* Otherwise, the connector already exists.  Compare it
			   with the new connector info and see what changed.  */
			if (connector->flags & IsDisconnected &&
			    info[i]->connection !=
				XCB_RANDR_CONNECTION_DISCONNECTED) {
				/* The connector was previously disconnected,
				   but not anymore.  Send the connector to
				   clients.  */
				SendConnectorToClients(connector);
				...
				DebugPrint("output named %s was connected",
				    connector->name);
			} else if (!(connector->flags & IsDisconnected) &&
			    info[i]->connection ==
				XCB_RANDR_CONNECTION_DISCONNECTED) {
				/* The connector was just disconnected. Withdraw
				   the connector.  */
				...
				DebugPrint("output named %s disconnected",
				    connector->name);
			}
...
HandleOutputOrResourceChange(Time timestamp)
{
	...
	while (device != &all_devices) {
		...
		DebugPrint("device %p was not found in tree", device);
...
XLInitDrmLease(void)
{
	...
	reply  = xcb_randr_query_version_reply(compositor.conn, cookie, NULL);

	if (!reply)
		return;

	if (reply->major_version < 1 ||
	    (reply->major_version == 1 && reply->minor_version < 6)) {
		free(reply);
		return;
	}
