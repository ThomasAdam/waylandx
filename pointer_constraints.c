	...
	for (tem = last_band_start;
		...
		if (tem->x1 <= lines[*idx].x1 && tem->x2 >= lines[*idx].x2) {
			...
			if (tem->x1 != lines[*idx].x1) {
				/* Here, tem->x1 is less than lines[*idx]->x1.
				   That means we must actually do the split at
				   %(1).  */
				...
				if (tem[1].x2 != lines[*idx].x2) {
					...
					LineMove(tem + 2, tem + 1,
					    max_lines - (tem - lines) - 2);
					tem[2].edges &= ~LeftEdgeClosed;
					tem[1].edges &= ~RightEdgeClosed;

					/* Increase idx to compensate for the
					 * movement.  */
					(*idx)++;
				...
				/* Here, tem->x1 is equal to lines[*idx].x1.
				   That means the split at %(1) is unnecessary.
				 */
	  ...
	  lines[*idx].x2
		...
		if (tem->x1 < lines[*idx].x1 && tem->x2 < lines[*idx].x2 &&
		    tem->x1 < lines[*idx].x2 && tem->x2 > lines[*idx].x1) {
		    ...
		    tem->x2 > lines[*idx].x2 && tem->x1 < lines[*idx].x2 &&
		    tem->x2 > lines[*idx].x1) {
			...
			/* Finally, resolve edges for line[*idx], which may
			   still have overlaps to the top.  */
			...
			if (tem->x1 != lines[*idx].x1) {
				...
				if (tem->x2 != lines[*idx + 1].x2) {
					...
					/* Now, we are in a very complicated
					   state of affairs.  idx is the index
					   of a line that must be processed
					   again.  idx + 1 is the index of the
					   line we are really processing, and
					   idx + 2 is a rectangle that does not
					   overlap with anything above (or
					   otherwise case B or the first
					   category would have run first).
					   First, open tem's bottom corner and
					   lines[idx + 1]'s top corner.  */
					tem->edges &= ~BottomEdgeClosed;
					lines[*idx + 1].edges &= ~TopEdgeClosed;

					/* Next, process splits for lines[*idx].
					 */
					MaybeCloseTopEdge(lines, idx,
					    max_lines);

					/* Finally, increment idx by 2.  */
					*idx += 2;
				} else {
					/* The second split is not required.
					   Simply open tem's bottom edge, and
					   lines[*idx + 1]'s top edge.  */
					tem->edges &= ~BottomEdgeClosed;
					lines[*idx + 1].edges &= ~TopEdgeClosed;
				...
				/* Do the second split.  The first split is not
				   required, as it would generate an empty line.
				   The second split is always required here,
				   because otherwise this would be a first
				   category overlap.  */
				LineMove(&lines[*idx + 1], &lines[*idx],
				    max_lines - *idx - 1);
				lines[*idx + 1].edges &= ~LeftEdgeClosed;
...
ComputeBarrier(pixman_region32_t *region, int *nlines)
{
	...
	for (i = 0; i < nrects; ++i) {
		...
		if (!l) {
			/* Initialize the first line.  The rectangle is
			   definitely closed on the top and the left, and may be
			   open on the right and bottom.  */
			...
			if (lines[l - 1].y1 == boxes[i].y1) {
				/* The rect is in the same band as the line
				   above.  This means that only its x1 and x2
				   are different.  If x1
				...
				if (boxes[i].x1 == lines[l - 1].x2) {
					...
					/* The left and right edges are
					   definitely closed. The bottom edge
					   won't be opened until the next line
					   starts being processed.  */
					lines[l].edges = (LeftEdgeClosed |
					    BottomEdgeClosed | RightEdgeClosed);
				}

				/* Compute whether or not the top edge is
				 * closed.  */
				MaybeCloseTopEdge(lines, &l, max_lines);

				/* l is now the index of the last used line.
				   Increase it.  */
				l++;
				...
				/* Initialize the first line of this new band.
				   The rectangle in this band closed on the
				   left, and may be open on the right, top and
				   bottom.  */
				lines[l].x1    = boxes[i].x1;
				lines[l].y1    = boxes[i].y1;
				lines[l].x2    = boxes[i].x2;
...
HandleSurfaceCommit(Surface *surface, void *data)
{
	...
	if (confinement->flags & PendingRegion) {
		...
		if (confinement->seat)
			/* If the seat is set, recheck pointer confinement.  If
			   this region change would cause the confinement to
			   deactivate, deactivate it and vice versa.  */
			RecheckPointerConfinement(confinement->seat,
			    confinement);
    ...
    wl_fixed_t surface_x, wl_fixed_t surface_y)
{
	...
	if (!confinement->commit_callback && confinement->surface)
		/* Attach a commit callback so we know when to apply the
		   region.  */
		confinement->commit_callback = XLSurfaceRunAtCommit(
		    confinement->surface, HandleSurfaceCommit, confinement);
}
    ...
    struct wl_resource *region_resource)
{
	...
	if (!region_resource) {
		...
		if (!confinement->pending_region) {
			confinement->pending_region = XLMalloc(
			    sizeof *confinement->pending_region);
...
HandleResourceDestroy(struct wl_resource *resource)
{
	...
	if (confinement->surface) {
		...
		if (confinement->commit_callback)
			XLSurfaceCancelCommitCallback(
			    confinement->commit_callback);
		confinement->commit_callback = NULL;
	}

	/* Free all pointer barriers activated.  */
	XIDListFree(confinement->applied_barriers, FreeSingleBarrier);
...
static void
LockPointer(struct wl_client *client, struct wl_resource *resource, uint32_t id,
	...
	if (XLSeatIsInert(seat)) {
		...
		if (!dummy_resource)
			...
			wl_resource_set_implementation(dummy_resource,
			    &locked_pointer_impl, NULL, NULL);

		return;
	}

	data = XLSurfaceGetClientData(surface, PointerConfinementData,
	    sizeof *data, FreePointerConfinementDataRecord);

	/* Potentially initialize confinement data.  */
	InitConfinementData(data);

#define AlreadyConstrained ZWP_POINTER_CONSTRAINTS_V1_ERROR_ALREADY_CONSTRAINED
...
#undef AlreadyConstrained
	...
	confinement->resource = wl_resource_create(client,
	    &zwp_locked_pointer_v1_interface, wl_resource_get_version(resource),
	    id);
	...
	confinement->seat_key = XLSeatRunOnDestroy(seat, HandleSeatDestroyed,
	    confinement);
	...
	wl_resource_set_implementation(confinement->resource,
	    &locked_pointer_impl, confinement, HandleResourceDestroy);
    ...
    uint32_t lifetime)
{
	...
	if (XLSeatIsInert(seat)) {
		...
		if (!dummy_resource)
			...
			wl_resource_set_implementation(dummy_resource,
			    &confined_pointer_impl, NULL, NULL);
...
#undef AlreadyConstrained
	...
	/* Attach the resource implementation.  */
	wl_resource_set_implementation(confinement->resource,
	    &confined_pointer_impl, confinement, HandleResourceDestroy);
...
GetDebugGC(Window window)
{
	...
	if (!XAllocColor(compositor.display, compositor.colormap, &color))
		abort();
	...
	gc = XCreateGC(compositor.display, window,
	    GCForeground | GCLineWidth | GCSubwindowMode, &gcvalues);
	return gc;
}
...
static void
ApplyLines(Window window, PointerConfinement *confinement, BarrierLine *lines,
    int nlines, int root_x, int root_y)
{
	int	       i, device_id;
	...
	if (nlines == 1 && lines[0].edges == AllEdgesClosed) {
		...
		barrier = XFixesCreatePointerBarrier(compositor.display, window,
		    Int16Minimum, root_y + lines[0].y1, Int16Maximum,
		    root_y + lines[0].y1, BarrierPositiveY, 1, &device_id);

		confinement->applied_barriers =
		    XIDListPrepend(confinement->applied_barriers, barrier);

		/* Bottom.  */
		barrier = XFixesCreatePointerBarrier(compositor.display, window,
		    Int16Minimum, root_y + lines[0].y2 - 1, Int16Maximum,
		    root_y + lines[0].y2 - 1, BarrierNegativeY, 1, &device_id);
		...
		/* Left.  */
		barrier = XFixesCreatePointerBarrier(compositor.display, window,
		    root_x + lines[0].x1, Int16Minimum, root_x + lines[0].x1,
		    Int16Maximum, BarrierPositiveX, 1, &device_id);
		...
		barrier = XFixesCreatePointerBarrier(compositor.display, window,
		    ...
		    root_x + lines[0].x2 - 1, Int16Maximum, BarrierNegativeX, 1,
		    &device_id);
	...
	for (i = 0; i < nlines; ++i) {
		...
		if (lines[i].edges & TopEdgeClosed) {
			barrier = XFixesCreatePointerBarrier(compositor.display,
			    window, root_x + lines[i].x1, root_y + lines[i].y1,
			    root_x + lines[i].x2 - 1, root_y + lines[i].y1,
			...
			confinement->applied_barriers = XIDListPrepend(
			    confinement->applied_barriers, barrier);
		}

		if (lines[i].edges & LeftEdgeClosed) {
			barrier = XFixesCreatePointerBarrier(compositor.display,
			    window, root_x + lines[i].x1, root_y + lines[i].y1,
			    root_x + lines[i].x1, root_y + lines[i].y2 - 1,
		...
		if (lines[i].edges & RightEdgeClosed) {
			barrier = XFixesCreatePointerBarrier(compositor.display,
			    window, root_x + lines[i].x2 - 1,
			    root_y + lines[i].y1, root_x + lines[i].x2 - 1,
			    root_y + lines[i].y2 - 1, BarrierNegativeX, 1,
			    &device_id);
#ifdef DEBUG
			XDrawLine(compositor.display, window, gc,
			    lines[i].x2 - 1, lines[i].y1, lines[i].x2 - 1,
			    lines[i].y2 - 1);
#endif
		...
		if (lines[i].edges & BottomEdgeClosed) {
			barrier = XFixesCreatePointerBarrier(compositor.display,
			    window, root_x + lines[i].x1,
			    root_y + lines[i].y2 - 1, root_x + lines[i].x2 - 1,
			    root_y + lines[i].y2 - 1, BarrierNegativeY, 1,
			    &device_id);
#ifdef DEBUG
			XDrawLine(compositor.display, window, gc, lines[i].x1,
...
static Bool
DrawPointerBarriers(PointerConfinement *confinement, pixman_region32_t *region,
    int *root_x_return, int *root_y_return)
{
	...
	if (root_x_return && root_y_return && *root_x_return != INT_MIN &&
	    *root_y_return != INT_MIN) {
		...
		XTranslateCoordinates(compositor.display, window,
		    DefaultRootWindow(compositor.display), 0, 0, &root_x,
		    &root_y, &child);
    ...
    double root_y_subpixel)
{
	...
	/* Top.  */
	barrier = XFixesCreatePointerBarrier(compositor.display, window,
	    Int16Minimum, root_y, Int16Maximum, root_y, BarrierPositiveY, 1,
	    &device_id);
	...
	/* Bottom.  */
	barrier = XFixesCreatePointerBarrier(compositor.display, window,
	    Int16Minimum, root_y + 1, Int16Maximum, root_y + 1,
	...
	/* Left.  */
	barrier = XFixesCreatePointerBarrier(compositor.display, window, root_x,
	    Int16Minimum, root_x, Int16Maximum, BarrierPositiveX, 1,
	    &device_id);
	confinement->applied_barriers =
	    XIDListPrepend(confinement->applied_barriers, barrier);
	...
	/* Right.  */
	barrier = XFixesCreatePointerBarrier(compositor.display, window,
	    root_x + 1, Int16Minimum, root_x + 1, Int16Maximum,
	...
	XIWarpPointer(compositor.display, device_id, None,
	    DefaultRootWindow(compositor.display), 0.0, 0.0, 0.0, 0.0, root_x,
	    root_y);
}
...
WarpToHint(PointerConfinement *confinement)
{
	...
	ViewTranslate(confinement->surface->view, 0, 0, &offset_x, &offset_y);
	XTranslateCoordinates(compositor.display, window,
	    DefaultRootWindow(compositor.display), 0, 0, &root_x, &root_y,
	    &child);

	/* Warp the pointer to the right position.  */
	XIWarpPointer(compositor.display, device_id, None, window, 0.0, 0.0,
	    0.0, 0.0, confinement->cursor_position_x - offset_x,
...
static void
RecomputeConfinement(PointerConfinement *confinement, int *root_x, int *root_y)
{
	Surface		 *surface;
	...
	if (confinement->region)
		...
		pixman_region32_copy(&intersection,
		    &surface->current_state.input);
	...
	pixman_region32_intersect_rect(&intersection, &intersection, 0, 0,
	    ViewWidth(surface->view), ViewHeight(surface->view));
	...
	if (!DrawPointerBarriers(confinement, &intersection, root_x, root_y))
    ...
    int *root_y_return)
{
	...
	DrawLock(confinement, confinement->last_cursor_x - offset_x + root_x,
	    confinement->last_cursor_y - offset_y + root_y);
}

static void
Reconfine(Surface *surface, int *root_x, int *root_y, Bool process_subsurfaces)
{
	...
	while (confinement != &record->confinements) {
		if (confinement->flags & IsActive) {
			if (!(confinement->flags & IsLock))
				RecomputeConfinement(confinement, root_x,
				    root_y);
			else
	...
	if (pixman_region32_contains_point(&intersection, x, y, &box)) {
		if (!(confinement->flags & IsActive)) {
			/* Intersect with the view bounds.  This is done after
			   pixman_region32_contains_point because X and Y must
			   be within the correct bounds for this function to be
			   called in the first place.  */
			pixman_region32_intersect_rect(&intersection,
			    &intersection, 0, 0, ViewWidth(dispatch->view),
			    ViewHeight(dispatch->view));

			/* Translate the region by the offset of the view into
			   the subcompositor.  */
			ViewTranslate(dispatch->view, 0, 0, &offset_x,
			    &offset_y);
			pixman_region32_translate(&intersection, -offset_x,
			    -offset_y);
			...
			if (confinement->flags & IsLock) {
				...
				/* Draw each pointer barrier for confinement. */
				if (!DrawPointerBarriers(confinement,
					&intersection, NULL, NULL))
					/* Rendering the confinement failed.
					   This is one of the oddball regions
					   that require too much memory to
					   process.  */
					DeactivateConfinement(confinement);
			}
...
void
XLPointerConstraintsSurfaceMovedTo(Surface *surface, int root_x, int root_y)
{
	PointerConfinementDataRecord *record;
	PointerConfinement	     *confinement;
	Window			      window;
	...
	while (confinement != &record->confinements) {
		...
		if (confinement->lines)
		    ...
		    confinement->flags & IsLock)
			/* Warp the pointer back to where it originally was
...
XLInitPointerConstraints(void)
{
	pointer_constraints_global = wl_global_create(compositor.wl_display,
	    &zwp_pointer_constraints_v1_interface, 1, NULL, HandleBind);
}
