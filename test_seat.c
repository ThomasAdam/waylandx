...
static void
DestroyXIModifierState(struct wl_client *client, struct wl_resource *resource)
{
	wl_resource_destroy(resource);
}

static void
SetValues(struct wl_client *client, struct wl_resource *resource, int32_t base,
    int32_t latched, int32_t locked, int32_t effective)
{
...
static void
DestroyXIButtonState(struct wl_client *client, struct wl_resource *resource)
{
	wl_resource_destroy(resource);
}
    ...
    uint32_t button)
{
	...
	if (button < 1 || button > 8)
		...
		wl_resource_post_error(resource,
		    TEST_MANAGER_ERROR_INVALID_BUTTON,
...
static void
DestroyXIValuatorState(struct wl_client *client, struct wl_resource *resource)
{
	wl_resource_destroy(resource);
}
...
AddValuatorToTestXIValuatorState(struct wl_client *client,
    struct wl_resource *resource, uint32_t valuator, wl_fixed_t value)
{
	...
	if (valuator < 1 || valuator > 65535) {
		...
		wl_resource_post_error(resource,
		    TEST_MANAGER_ERROR_INVALID_VALUATOR,
	...
	if (XIMaskLen(valuator) <= state->mask_len &&
		...
		wl_resource_post_error(resource,
		    TEST_MANAGER_ERROR_VALUE_EXISTS,
		...
		if (state->mask_len < XIMaskLen(valuator)) {
			...
			memset(state->mask + state->mask_len, 0,
			    XIMaskLen(valuator) - state->mask_len);
		}
		...
		for (i = 0, j = 0;
		    i < MAX(state->mask_len, XIMaskLen(valuator)) * 8; ++i) {
			if (i == valuator)
		...
		state->mask_len = MAX(state->mask_len, XIMaskLen(valuator));
	}
}
    ...
    uint32_t device_id)
{
	...
	if (device_id < 65536) {
		wl_resource_post_error(resource,
		    TEST_MANAGER_ERROR_INVALID_DEVICE_ID,
...
static void
SetUse(struct wl_client *client, struct wl_resource *resource, int32_t use)
{
	TestXIDeviceInfo *info;
...
AddXIScrollClassInfo(struct wl_client *client, struct wl_resource *resource,
    int32_t sourceid, int32_t number, int32_t scroll_type, wl_fixed_t increment,
    int32_t flags)
{
	TestXIDeviceInfo *info;
...
AddXIValuatorClassInfo(struct wl_client *client, struct wl_resource *resource,
    int32_t sourceid, int32_t number, const char *label, wl_fixed_t min,
    wl_fixed_t max, wl_fixed_t value, int32_t resolution, int32_t mode)
{
	TestXIDeviceInfo *info;
	...
	if (!strlen(label)) {
		wl_resource_post_error(resource,
		    TEST_MANAGER_ERROR_INVALID_LABEL,
...
static void
DestroyDeviceController(struct wl_client *client, struct wl_resource *resource)
{
	wl_resource_destroy(resource);
}
	...
	if ((seat && seat->flags & IsTestDeviceSpecified) || deviceinfo) {
		...
		for (i = 0; i < controller->num_ids; ++i) {
			if (controller->device_ids[i] == info->device_id)
				/* It was created by this controller.  Simply
				   update the values.  */
    ...
    uint32_t id)
{
	...
	info->resource = wl_resource_create(client,
	    &test_XIDeviceInfo_interface, wl_resource_get_version(resource),
	    id);
	...
	wl_resource_set_implementation(info->resource, &XIDeviceInfo_impl, info,
	    HandleXIDeviceInfoDestroy);
}
...
static void
DestroySeatController(struct wl_client *client, struct wl_resource *resource)
{
	wl_resource_destroy(resource);
}
    ...
    uint32_t version, uint32_t id)
{
	...
	if (!version || version > 8)
		wl_resource_post_error(resource,
		    TEST_MANAGER_ERROR_BAD_SEAT_VERSION,
    ...
    uint32_t id)
{
	...
	state_resource = wl_resource_create(client,
	    &test_XIModifierState_interface, wl_resource_get_version(resource),
	    id);
    ...
    uint32_t id)
{
	...
	state_resource = wl_resource_create(client,
	    &test_XIButtonState_interface, wl_resource_get_version(resource),
	    id);
    ...
    uint32_t id)
{
	...
	/* Now, try to create the button state resource.  */
	state_resource = wl_resource_create(client,
	    &test_XIValuatorState_interface, wl_resource_get_version(resource),
	    id);
...
static void
TranslateTestValuators(struct wl_resource *resource, XIValuatorState *valuators)
{
	TestXIValuatorState *state;
...
static void
TranslateTestModifiers(struct wl_resource *resource, XIModifierState *modifiers)
{
	TestXIModifierState *state;
...
static void
DispatchTestEvent(TestSeatController *controller, Window window, XIEvent *event)
{
	Surface	      *surface;
	...
	if (event->evtype == XI_FocusIn)
	...
	else if (event->evtype == XI_Enter || event->evtype == XI_Leave)
	...
	else if (event->evtype == XI_KeyPress || event->evtype == XI_KeyRelease)
		...
		DispatchGesturePinch(subcompositor,
		    (XIGesturePinchEvent *)event);
	else if (event->evtype == XI_GestureSwipeBegin ||
	    event->evtype == XI_GestureSwipeUpdate ||
	    event->evtype == XI_GestureSwipeEnd)
		DispatchGestureSwipe(subcompositor,
		    (XIGestureSwipeEvent *)event);
}
...
DispatchXIEnter(struct wl_client *client, struct wl_resource *resource,
    uint32_t time, int32_t sourceid, int32_t detail, uint32_t root,
    uint32_t event, uint32_t child, wl_fixed_t root_x, wl_fixed_t root_y,
    wl_fixed_t event_x, wl_fixed_t event_y, int32_t mode, int32_t focus,
    int32_t same_screen, struct wl_resource *buttons_resource,
    struct wl_resource *mods_resource, struct wl_resource *group_resource)
{
	TestSeatController *controller;
...
DispatchXIMotion(struct wl_client *client, struct wl_resource *resource,
    ...
    struct wl_resource *valuators_resource, struct wl_resource *mods_resource,
    ...
    uint32_t id)
{
	...
	controller->resource = wl_resource_create(client,
	    &test_device_controller_interface,
	...
	wl_resource_set_implementation(controller->resource,
	    &device_controller_impl, controller,
    ...
    uint32_t months, uint32_t milliseconds)
{
	...
	if (TimestampIs(timestamp, Earlier, controller->seat->last_user_time)) {
		wl_resource_post_error(resource,
		    TEST_MANAGER_ERROR_INVALID_USER_TIME,
...
DispatchXIRawKeyPress(struct wl_client *client, struct wl_resource *resource,
    uint32_t time, int32_t sourceid, int32_t detail, int32_t flags,
    ...
    uint32_t id)
{
	...
	memset(controller, 0, sizeof *controller);
	controller->resource = wl_resource_create(client,
	    &test_seat_controller_interface, wl_resource_get_version(resource),
	    id);
	...
	wl_resource_set_implementation(controller->resource,
	    &seat_controller_impl, controller, HandleControllerResourceDestroy);
