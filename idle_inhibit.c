...
DetectSurfaceIdleInhibit(void)
{
	...
	while (inhibitor != &all_inhibitors) {
		if (inhibitor->surface->num_focused_seats) {
			...
			/* This is a subsurface.  If it is visible and the
			   toplevel parent has the input focus, then also
			   inhibit system idle.  */
    ...
    uint32_t id, struct wl_resource *surface_resource)
{
	...
	inhibitor->resource = wl_resource_create(client,
	    &zwp_idle_inhibitor_v1_interface, wl_resource_get_version(resource),
	    id);
	...
	wl_resource_set_implementation(inhibitor->resource,
	    &idle_inhibitor_impl, inhibitor, HandleResourceDestroy);
}
...
ReadCommandResource(const char *name, const char *class)
{
	...
	if (XrmQGetResource(rdb, namelist, classlist, &type, &value) &&
	    type == QString) {
		ParseProcessString((const char *)value.addr, &arguments,
		    &num_args);
...
static int
ReadIntegerResource(const char *name, const char *class, int default_value)
{
	XrmDatabase	  rdb;
	XrmName		  namelist[3];
	XrmClass	  classlist[3];
...
XLInitIdleInhibit(void)
{
	idle_inhibit_manager_global = wl_global_create(compositor.wl_display,
	    &zwp_idle_inhibit_manager_v1_interface, 1, NULL, HandleBind);
