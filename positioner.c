...
static void
SetSize(struct wl_client *client, struct wl_resource *resource, int32_t width,
    int32_t height)
{
...
static void
SetAnchorRect(struct wl_client *client, struct wl_resource *resource, int32_t x,
...
static void
SetOffset(struct wl_client *client, struct wl_resource *resource, int32_t x,
    int32_t y)
{
	Positioner *positioner;
...
static void
SetParentSize(struct wl_client *client, struct wl_resource *resource, int width,
    int height)
{
	Positioner *positioner;
    ...
    int offset)
{
	...
	DebugPrint("new anchor: %s, anchor point: %d, %d; gravity: %s",
	    anchor_gravity_names[new.anchor], positioner->anchor_x,
	    positioner->anchor_y, anchor_gravity_names[new.gravity]);
...
static void
TryResizeX(int x, int width, int cx, int cwidth, int offset, int *new_x,
    int *new_width)
{
...
static void
TryResizeY(int y, int height, int cy, int cheight, int offset, int *new_y,
    int *new_height)
{
...
GetAdjustmentOffset(Role *parent, int *off_x, int *off_y)
{
	...
	XLXdgRoleGetCurrentGeometry(parent, &parent_gx, &parent_gy, NULL, NULL);
...
static void
ApplyConstraintAdjustment(Positioner *positioner, Role *parent, int x, int y,
    int *x_out, int *y_out, int *width_out, int *height_out)
{
	...
	TruncateScaleToWindow(parent->surface, width, height, &width, &height);
	...
	if (!XLGetOutputRectAt(off_x + x, off_y + y, &cx, &cy, &cwidth,
		&cheight))
	...
	fputs("ApplyConstraintAdjustments: constraint adjustments are: ",
	    stderr);
	...
	if (positioner->constraint_adjustment &
	    XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_FLIP_X)
		x = TryFlipX(positioner, x + off_x, width, cx, cwidth, off_x) -
		    off_x;

	if (positioner->constraint_adjustment &
	    XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_FLIP_Y)
		y = TryFlipY(positioner, y + off_y, height, cy, cheight,
			off_y) -
		    off_y;

	if (positioner->constraint_adjustment &
	    XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_SLIDE_X)
		x = TrySlideX(positioner, x + off_x, width, cx, cwidth) - off_x;

	if (positioner->constraint_adjustment &
	    XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_SLIDE_Y)
		y = TrySlideY(positioner, y + off_y, height, cy, cheight) -
		    off_y;

	if (positioner->constraint_adjustment &
	    XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_RESIZE_X)
		TryResizeX(x + off_x, width, cx, cwidth, off_x, &x, &width);

	if (positioner->constraint_adjustment &
	    XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_RESIZE_Y)
		TryResizeY(y + off_y, height, cy, cheight, off_y, &y, &height);
...
void
XLPositionerCalculateGeometry(Positioner *positioner, Role *parent, int *x_out,
    int *y_out, int *width_out, int *height_out)
{
	...
	if (parent->surface)
		ApplyConstraintAdjustment(positioner, parent, x, y, &x, &y,
		    &width, &height);
	else
    ...
    uint32_t id)
{
	...
	positioner->resource = wl_resource_create(client,
	...
	wl_resource_set_implementation(positioner->resource,
	    &xdg_positioner_impl, positioner, HandleResourceDestroy);
}
