...
#define MaskIsSet(ptr, event) \
	(((unsigned char *)(ptr))[(event) >> 3] & (1 << ((event) & 7)))
#define MaskLen(event) (((event) >> 3) + 1)
...
QueryPointer(Seat *seat, Window relative_to, double *x, double *y)
{
	...
	*y = 0;
	...
	if (XIQueryPointer(compositor.display, seat->master_pointer,
		relative_to, &root, &child, &root_x, &root_y, &win_x, &win_y,
...
MaybeCreateCursor(CursorRing *ring, int index)
{
	...
	ring->pixmaps[index] = XCreatePixmap(compositor.display,
	    DefaultRootWindow(compositor.display), ring->width, ring->height,
	    compositor.n_planes);
...
FreeCursor(SeatCursor *cursor)
{
	...
	if (cursor->role.surface)
		XLSurfaceReleaseRole(cursor->role.surface, &cursor->role);
	...
	if (!(cursor->seat->flags & IsInert) && window)
		XIDefineCursor(compositor.display, cursor->seat->master_pointer,
...
ReleaseSeat(Seat *seat)
{
	...
	if (seat->data_source_destroy_callback)
		XLDataSourceCancelDestroyCallback(
		    seat->data_source_destroy_callback);

	if (seat->subcompositor_callback)
		SubcompositorRemoveDestroyCallback(
...
static void
ComputeHotspot(SeatCursor *cursor, int min_x, int min_y, int *x, int *y)
{
	int dx, dy;
	int hotspot_x, hotspot_y;
...
static void
ApplyCursor(SeatCursor *cursor, RenderTarget target, int min_x, int min_y)
{
	Window	window;
	int	x, y;
	Picture picture;
	...
	cursor->cursor = XRenderCreateCursor(compositor.display, picture,
	    MAX(0, x), MAX(0, y));
...
UpdateCursorFromSubcompositor(SeatCursor *cursor)
{
	...
	SubcompositorBounds(cursor->subcompositor, &min_x, &min_y, &max_x,
	    &max_y);
	...
	SubcompositorSetProjectiveTransform(cursor->subcompositor, MAX(0, -x),
	    MAX(0, -x));
...
MakeCurrentCursor(Seat *seat, Surface *surface, int x, int y)
{
	...
	XIDefineCursor(compositor.display, seat->master_pointer, window,
	    InitDefaultCursor());
	...
	UpdateCursorOutput(role, seat->last_motion_x, seat->last_motion_y);
    ...
    int32_t hotspot_y)
{
	...
	if (surface->role) {
		UpdateCursor(CursorFromRole(surface->role), hotspot_x,
		    hotspot_y);
		return;
	}
...
static void
GetPointer(struct wl_client *client, struct wl_resource *resource, uint32_t id)
{
	Seat		   *seat;
	SeatClientInfo	   *info;
	Pointer		   *pointer;
...
UpdateSingleKeyboard(Keyboard *keyboard)
{
	...
	wl_keyboard_send_keymap(keyboard->resource,
	    WL_KEYBOARD_KEYMAP_FORMAT_XKB_V1, keymap_fd, statb.st_size);

	SendRepeatKeys(keyboard->resource);
}

static void
GetKeyboard(struct wl_client *client, struct wl_resource *resource, uint32_t id)
{
	SeatClientInfo	   *info;
	Seat		   *seat;
	Keyboard	   *keyboard;
	...
	if (seat->focus_surface &&
	    (wl_resource_get_client(seat->focus_surface->resource) == client))
...
static void
GetTouch(struct wl_client *client, struct wl_resource *resource, uint32_t id)
{
...
static void
HandleBind1(struct wl_client *client, Seat *seat, uint32_t version, uint32_t id)
{
	struct wl_resource *resource;
...
UpdateValuators(Seat *seat, XIDeviceInfo *device)
{
	...
	for (i = 0; i < device->num_classes; ++i) {
		if (device->classes[i]->type == XIScrollClass)
			AddValuator(seat,
			    (XIScrollClassInfo *)device->classes[i]);
	}
}
    ...
    XIDeviceInfo *pointer_info)
{
	...
	seat->global	      = wl_global_create(compositor.wl_display,
		     &wl_seat_interface, 8, seat, HandleBind);
	...
	if (UncatchXErrors(NULL))
		/* If the device was disabled or removed, a HierarchyChange
		   event will be sent shortly afterwards, causing the seat to be
	...
	/* If a keymap is already attached, initialize seat->key_pressed
	   now.  */
	if (xkb_desc)
		seat->key_pressed = XLCalloc(MaskLen(xkb_desc->max_key_code -
...
UpdateScrollMethods(DeviceInfo *info, int deviceid)
{
	...
	rc = XIGetProperty(compositor.display, deviceid,
	    libinput_Scroll_Methods_Available, 0, 3, False, XIAnyPropertyType,
...
UpdateScrollPixelDistance(DeviceInfo *info, int deviceid)
{
	...
	/* This only works with the libinput driver.  */
	rc = XIGetProperty(compositor.display, deviceid,
	    libinput_Scrolling_Pixel_Distance, 0, 1, False, XIAnyPropertyType,
	    &actual_type, &actual_format, &nitems, &bytes_after, &data);
...
RecordDeviceInformation(XIDeviceInfo *deviceinfo)
{
	...
	if (deviceinfo->use == XISlavePointer) {
		...
		/* Obtain the "libinput Scroll Methods Enabled" property and use
		   it to compute what scroll methods are available.  This
		   naturally only works with the libinput driver.  */
...
SetupInitialDevices(void)
{
	...
	deviceinfo = XIQueryDevice(compositor.display, XIAllDevices, &ndevices);
	...
	for (i = 0; i < ndevices; ++i) {
		if (deviceinfo[i].use == XIMasterPointer)
			MakeSeatForDevicePair(deviceinfo[i].attachment,
			    deviceinfo[i].deviceid, &deviceinfo[i]);
    ...
    XIDeviceEvent *xev)
{
	...
	/* Ungrab the pointer.  */
	XIUngrabDevice(compositor.display, seat->master_pointer, time);
	...
	if (dispatch) {
		TransformToSurface(dispatch, xev->event_x, xev->event_y, &x,
		    &y);
...
InterceptMotionEventForResize(Seat *seat, XIDeviceEvent *xev)
{
	...
	if (seat->resize_axis_flags & ResizeAxisRight)
		/* diff_x is computed differently here, since root_x grows in
		   the correct resize direction.  */
	...
	XLSurfacePostResize(seat->resize_surface,
	    ...
	    seat->resize_width + abs_diff_x, seat->resize_height + abs_diff_y);
...
NoticeDeviceEnabled(int deviceid)
{
	...
	CatchXErrors();
	info = XIQueryDevice(compositor.display, deviceid, &ndevices);
	UncatchXErrors(NULL);
...
#define KeyIsPressed(seat, keycode) \
	(MaskIsSet((seat)->key_pressed, (keycode) - xkb_desc->min_key_code))
#define KeySetPressed(seat, keycode, pressed)                 \
	(!pressed ? ClearMask((seat)->key_pressed,            \
...
static void
SendKeyboardKey(Seat *seat, Surface *focus, Time time, uint32_t key,
    uint32_t state)
{
	Keyboard       *keyboard;
	SeatClientInfo *info;
	...
	for (; keyboard != &info->keyboards; keyboard = keyboard->next)
		wl_keyboard_send_key(keyboard->resource, serial, time, key,
		    state);
}
...
HandleResizeComplete(Seat *seat)
{
	...
	XSendEvent(compositor.display, DefaultRootWindow(compositor.display),
	    False, SubstructureRedirectMask | SubstructureNotifyMask, &msg);
...
static void SendButton(Seat *, Surface *, Time, uint32_t, uint32_t, double,
    double);
...
HandleRawButton(XIRawEvent *event)
{
	...
	if (seat->resize_in_progress || seat->flags & IsWindowMenuShown) {
		if (seat->last_seen_surface) {
			...
			/* When a RawButtonPress is received while resizing is
			   still in progress, release the button on the current
			   surface.

			   Since leave and entry events generated by grabs are
			   ignored, the client will not get leave events
			   correctly while Metacity is resizing a frame.  This
			   results in programs such as GTK tracking the button
			   state incorrectly if the pointer never leaves the
			   surface during a resize operation.
...
HandleDeviceChanged(XIDeviceChangedEvent *event)
{
	...
	CatchXErrors();
	info = XIQueryDevice(compositor.display, event->deviceid, &ndevices);
	UncatchXErrors(NULL);
...
HandleDragMotionEvent(XIDeviceEvent *xev)
{
	...
	if (seat->icon_surface)
		XLMoveIconSurface(seat->icon_surface, xev->root_x, xev->root_y);
...
SelectDeviceEvents(void)
{
	...
	XISelectEvents(compositor.display,
	    DefaultRootWindow(compositor.display), &mask, 1);
...
SendKeyboardLeave(Seat *seat, Surface *focus)
{
	...
	for (; keyboard != &info->keyboards; keyboard = keyboard->next)
		wl_keyboard_send_leave(keyboard->resource, serial,
		    focus->resource);
}
...
{
	wl_keyboard_send_modifiers(keyboard->resource, serial, seat->base,
	    seat->latched, seat->locked, seat->effective_group);
}
...
static void
HackKeyboardModifiers(Seat *seat, Surface *focus, int effective, int group)
{
	Keyboard       *keyboard;
	uint32_t	serial;
	SeatClientInfo *info;
...
SendUpdatedModifiers(Seat *seat)
{
	...
	for (callback = seat->modifier_callbacks.next;
	    callback != &seat->modifier_callbacks; callback = callback->next)
...
static void
UpdateModifiersForSeat(Seat *seat, unsigned int base, unsigned int locked,
    unsigned int latched, int base_group, int locked_group, int latched_group,
    int effective_group)
{
	seat->base	      = base;
	seat->locked	      = locked;
...
SetFocusSurface(Seat *seat, Surface *focus)
{
	...
	seat->focus_destroy_callback = XLSurfaceRunOnFree(focus,
	    ClearFocusSurface, seat);
...
FindSurfaceUnder(Subcompositor *subcompositor, double x, double y)
{
	...
	view = SubcompositorLookupView(subcompositor, x, y, &x_off, &y_off);
...
DragLeave(Seat *seat)
{
	if (seat->drag_last_surface) {
		if (seat->flags & IsDragging)
			...
			/* If nothing is being dragged anymore, avoid sending
			   flags to the source after drop or cancel.  */
			XLDataDeviceSendLeave(seat, seat->drag_last_surface,
			    NULL);
...
DragEnter(Seat *seat, Surface *surface, double x, double y)
{
	...
	seat->drag_last_surface_destroy_callback = XLSurfaceRunOnFree(surface,
	    HandleDragLastSurfaceDestroy, seat);

	XLDataDeviceSendEnter(seat, surface, x, y, seat->data_source);
}

static void
DragMotion(Seat *seat, Surface *surface, double x, double y, Time time)
{
	if (!seat->drag_last_surface)
		return;
...
DragButton(Seat *seat, XIDeviceEvent *xev)
{
	...
	if (MaskPopCount(&xev->buttons) == 1) {
		...
		if (seat->data_source && XLDoDragDrop(seat))
			...
			if (seat->drag_last_surface) {
				if (!seat->data_source ||
					...
					XLDataDeviceSendDrop(seat,
					    seat->drag_last_surface);
					XLDataSourceSendDropPerformed(
					    seat->data_source);
				} else
					/* Otherwise, the data source is not
					   eligible for dropping; simply send
					   cancel.  */
					XLDataSourceSendDropCancelled(
					    seat->data_source);
		...
		CancelDrag(seat, xev->event, xev->event_x, xev->event_y);
	}
}

static void
SendMotion(Seat *seat, Surface *surface, double x, double y, Time time)
{
	Pointer	       *pointer;
	uint32_t	serial;
	SeatClientInfo *info;
...
static void
SendButton(Seat *seat, Surface *surface, Time time, uint32_t button,
    uint32_t state, double x, double y)
{
	Pointer	       *pointer;
	uint32_t	serial;
	SeatClientInfo *info;
	...
	for (; pointer != &info->pointers; pointer = pointer->next) {
		...
		wl_pointer_send_button(pointer->resource, serial, time, button,
		    state);
...
SwapGrabSurface(Seat *seat, Surface *surface)
{
	...
	if (surface) {
		...
		seat->grab_surface_callback = XLSurfaceRunAtUnmap(surface,
		    ClearGrabSurface, seat);
	}
}
...
UndefineCursorOn(Seat *seat, Surface *surface)
{
	...
	XIUndefineCursor(compositor.display, seat->master_pointer, window);
...
static void
EnteredSurface(Seat *seat, Surface *surface, Time time, double x, double y,
    Bool preserve_cursor)
{
	...
	if (seat->flags & IsInPinchGesture
		...
		SendGesturePinchEnd(seat, seat->last_seen_surface, gesture_time,
		    1);
	...
	if (seat->flags & IsInSwipeGesture
		...
		/* Send the gesture end event.  */
		SendGestureSwipeEnd(seat, seat->last_seen_surface, gesture_time,
		    1);
	...
	if (surface) {
		...
		seat->last_seen_surface_callback = XLSurfaceRunOnFree(surface,
		    ClearLastSeenSurface, seat);
		...
		if (seat->flags & IsDragging)
			...
			/* Apparently what is done by other compositors when no
			   wl_pointer object exists for the surface's client is
			   to revert back to the default cursor.  */
    ...
    double *x_out, double *y_out)
{
	...
	XTranslateCoordinates(compositor.display, source, target, int_x, int_y,
	    &t1, &t2, &child_return);
...
static Surface *
ComputeGrabPosition(Seat *seat, Surface *dispatch, double *event_x,
    double *event_y)
{
	...
	TranslateCoordinates(toplevel, grab, *event_x, *event_y, event_x,
	    event_y);
    ...
    double *event_y)
{
	...
	TranslateCoordinates(window, grab, *event_x, *event_y, event_x,
	    event_y);
	return;
}
...
DispatchEntryExit(Subcompositor *subcompositor, XIEnterEvent *event)
{
	...
	if (event->evtype == XI_Leave ||
		...
		if (event->evtype == XI_Enter) {
			...
			seat->subcompositor_callback  = SubcompositorOnDestroy(
			     subcompositor, HandleSubcompositorDestroy, seat);
	...
	if (event->mode == XINotifyUngrab && seat->grab_surface)
		/* Any explicit grab was released, so release the grab surface
		   as well.  */
		SwapGrabSurface(seat, NULL);

	if (event->mode == XINotifyUngrab && seat->flags & IsDragging)
		/* The active grab was released.  */
		CancelDrag(seat, event->event, event->event_x, event->event_y);

	if (event->evtype == XI_Leave &&
	    (event->mode == XINotifyGrab || event->mode == XINotifyUngrab))
	...
	if (seat->grab_surface) {
		...
		TranslateGrabPosition(seat, event->event, &event_x, &event_y);
	...
	if (!dispatch)
		...
		if (!CanDeliverEvents(seat, dispatch))
			dispatch = ComputeGrabPosition(seat, dispatch, &event_x,
			    &event_y);
...
static void
InterpolateAxes(Surface *surface, DeviceInfo *info, double movement_x,
    double movement_y, double *x_out, double *y_out)
{
	if (!info) {
...
static void
SendScrollAxis(Seat *seat, Surface *surface, Time time, double x, double y,
    double axis_x, double axis_y, int flags, int sourceid)
{
	Pointer	       *pointer;
	uint32_t	serial;
	SeatClientInfo *info;
	...
	for (; pointer != &info->pointers; pointer = pointer->next) {
		...
		if (wl_resource_get_version(pointer->resource) < 8
			...
			/* Interpolate the increment-relative axis values to
			   pixel values.  */
			...
			/* Send value120 events if they are available; those
			   events make more sense.  */
		...
		if (axis_x != 0.0 || axis_y != 0.0 || flags ||
		    pointer->state & StateIsRaw) {
			if (wl_resource_get_version(pointer->resource) >= 5)
				wl_pointer_send_frame(pointer->resource);
		}
    ...
    XIDeviceEvent *event)
{
	...
	for (i = 0; i < event->valuators.mask_len * 8; ++i) {
		...
		if (!valuator)
			/* We still have to increment values even if we don't
			   know about the valuator in question.  */
			goto next;
...
DispatchMotion(Subcompositor *subcompositor, XIDeviceEvent *xev)
{
	...
	if (!dispatch) {
		if (seat->grab_surface) {
			/* If the grab surface is set, translate the coordinates
			   to it and use it instead.  */
			TranslateGrabPosition(seat, xev->event, &event_x,
			    &event_y);
			dispatch = seat->grab_surface;
	...
	if (seat->cursor)
		UpdateCursorOutput(seat->cursor, xev->root_x, xev->root_y);
	...
	if (!HandleValuatorMotion(seat, dispatch, x, y, xev)) {
		if (seat->flags & IsDragging)
			...
			/* Send relative motion.  Relative motion is handled by
			   ...
			   seat->last_surface_y, unless pointer motion reporting
			   is locked, in which case XI barrier motion events are
			   used instead.  */
			if (x - seat->last_surface_x != 0.0 ||
			    y - seat->last_surface_y != 0.0)
				SendRelativeMotion(seat, dispatch,
				    x - seat->last_surface_x,
				    y - seat->last_surface_y, xev->time);

			/* Check if this motion would cause a pointer constraint
			   to activate.  */
...
static void
CancelGrab1(Seat *seat, Subcompositor *subcompositor, Time time, double x,
    double y)
{
	Surface *surface;
...
static void
CancelGrab(Seat *seat, Time time, Window source, double x, double y)
{
	if (!seat->grab_held)
		return;
	...
	if (seat->last_seen_subcompositor) {
		...
		if (source != seat->last_seen_subcompositor_window)
			TranslateCoordinates(source,
			    seat->last_seen_subcompositor_window, x, y, &x, &y);
...
CancelGrabEarly(Seat *seat)
{
	...
	CancelGrab(seat, seat->its_press_time,
	    DefaultRootWindow(compositor.display), seat->its_root_x,
	    seat->its_root_y);
}
...
SetButtonSurface(Seat *seat, Surface *surface)
{
	...
	seat->last_button_press_surface_callback = XLSurfaceRunOnFree(surface,
	    ClearLastButtonPressSurface, seat);
}
...
after_dispatch_set:
	...
	SendButton(seat, dispatch, xev->time, button, state, x, y);
	...
	if (xev->evtype == XI_ButtonPress)
		...
		CancelGrab(seat, xev->time, xev->event, xev->event_x,
		    xev->event_y);
}
...
DispatchKey(XIDeviceEvent *xev)
{
	...
	if (seat->focus_surface) {
		...
		if (input_funcs && seat->flags & IsTextInputSeat &&
		    input_funcs->filter_input(seat, seat->focus_surface, xev,
			&keycode))
		...
		if (xev->evtype == XI_KeyPress)
			SendKeyboardKey(seat, seat->focus_surface, xev->time,
...
DispatchBarrierHit(XIBarrierEvent *barrier)
{
	...
	if (seat->last_seen_surface)
		SendRelativeMotion(seat, seat->last_seen_surface, barrier->dx,
		    barrier->dy, barrier->time);
...
static void
SendGesturePinchBegin(Seat *seat, Surface *dispatch, Time time, int detail)
{
	...
	for (; gesture != &info->pinch_gestures; gesture = gesture->next)
		zwp_pointer_gesture_pinch_v1_send_begin(gesture->resource,
		    serial, time, dispatch->resource, detail);
}

static void
SendGesturePinchUpdate(Seat *seat, Surface *dispatch, Time time, double dx,
    double dy, double scale, double rotation)
{
	...
	for (; gesture != &info->pinch_gestures; gesture = gesture->next)
		zwp_pointer_gesture_pinch_v1_send_update(gesture->resource,
		    time, wl_fixed_from_double(dx), wl_fixed_from_double(dy),
...
SendGesturePinchEnd(Seat *seat, Surface *dispatch, Time time, int cancelled)
{
	...
	for (; gesture != &info->pinch_gestures; gesture = gesture->next)
		zwp_pointer_gesture_pinch_v1_send_end(gesture->resource, serial,
		    time, cancelled);
}
...
DispatchGesturePinch(Subcompositor *subcompositor, XIGesturePinchEvent *pinch)
{
	...
	if (!dispatch) {
		if (seat->grab_surface) {
			/* If the grab surface is set, translate the coordinates
			   to it and use it instead.  */
			TranslateGrabPosition(seat, pinch->event, &event_x,
			    &event_y);
			dispatch = seat->grab_surface;
...
after_dispatch_set:
	...
	case XI_GesturePinchBegin:
		...
		/* Send a begin event.  */
		SendGesturePinchBegin(seat, dispatch, pinch->time,
		    pinch->detail);
	...
	case XI_GesturePinchUpdate:
		...
		SendGesturePinchUpdate(seat, dispatch, pinch->time,
		    pinch->delta_x, pinch->delta_y, pinch->scale,
		    pinch->delta_angle);
		break;
...
static void
SendGestureSwipeBegin(Seat *seat, Surface *dispatch, Time time, int detail)
{
...
static void
SendGestureSwipeUpdate(Seat *seat, Surface *dispatch, Time time, double dx,
    double dy)
{
	SwipeGesture   *gesture;
	SeatClientInfo *info;
...
SendGestureSwipeEnd(Seat *seat, Surface *dispatch, Time time, int cancelled)
{
	...
	for (; gesture != &info->swipe_gestures; gesture = gesture->next)
		zwp_pointer_gesture_swipe_v1_send_end(gesture->resource, serial,
		    time, cancelled);
}
...
DispatchGestureSwipe(Subcompositor *subcompositor, XIGestureSwipeEvent *swipe)
{
	...
	if (!dispatch) {
		if (seat->grab_surface) {
			/* If the grab surface is set, translate the coordinates
			   to it and use it instead.  */
			TranslateGrabPosition(seat, swipe->event, &event_x,
			    &event_y);
			dispatch = seat->grab_surface;
...
after_dispatch_set:
	...
	case XI_GestureSwipeBegin:
		...
		/* Send a begin event.  */
		SendGestureSwipeBegin(seat, dispatch, swipe->time,
		    swipe->detail);
...
WriteKeymap(void)
{
	...
	if (!ok)
		fprintf(stderr,
		    "Warning: the XKB keymap could not be written\n"
...
AfterMapUpdate(void)
{
	...
	if (XkbGetControls(compositor.display, XkbAllControlsMask, xkb_desc) !=
	    Success) {
		fprintf(stderr, "Could not load keyboard controls\n");
		exit(1);
	}

	if (XkbGetCompatMap(compositor.display, XkbAllCompatMask, xkb_desc) !=
	    Success) {
		fprintf(stderr, "Could not load compatibility map\n");
		exit(1);
	}

	if (XkbGetNames(compositor.display, XkbAllNamesMask, xkb_desc) !=
	    Success) {
...
UpdateKeymapInfo(void)
{
	...
	for (tem = live_seats; tem; tem = tem->next) {
		...
		for (keyboard = seat->keyboards.next1;
		    keyboard != &seat->keyboards; keyboard = keyboard->next1)
			UpdateSingleKeyboard(keyboard);
	}
}
...
SetupKeymap(void)
{
	...
	xkb_desc = XkbGetMap(compositor.display, XkbAllMapComponentsMask,
	    XkbUseCoreKbd);
...
HandleXkbEvent(XkbEvent *event)
{
	...
	if (event->any.xkb_type == XkbMapNotify ||
		...
		XkbFreeKeyboard(xkb_desc, XkbAllMapComponentsMask, True);
		...
		if (!xkb_desc) {
			fprintf(stderr,
			    "Failed to retrieve keymap from X server\n");
			exit(1);
		}
		...
		if (seat)
			UpdateModifiersForSeat(seat, event->state.base_mods,
			    event->state.locked_mods, event->state.latched_mods,
			    event->state.base_group, event->state.locked_group,
			    event->state.latched_group, event->state.group);
		return True;
	}
...
HandleResizeUnmapped(void *data)
{
	...
	seat = data;
	CancelResizeOperation(seat, seat->resize_time, NULL, NULL);
}

static Bool
FakePointerEdge(Seat *seat, Surface *target, uint32_t serial, ResizeEdge edge)
{
	Cursor	    cursor;
	Status	    state;
	Window	    window;
	...
	if (!(seat->flags & IsTestSeat)) {
		...
		state = XIGrabDevice(compositor.display, seat->master_pointer,
		    window, seat->its_press_time, cursor, XIGrabModeAsync,
	...
	seat->resize_surface_callback = XLSurfaceRunAtUnmap(
	    seat->resize_surface, HandleResizeUnmapped, seat);
...
static Bool
HandlePointerEdge(Seat *seat, Surface *target, uint32_t serial, ResizeEdge edge)
{
	Surface *surface;
	XEvent	 msg;
	...
	if (!XLWmSupportsHint(_NET_WM_MOVERESIZE) ||
	    (seat->flags & IsTestSeat) || getenv("USE_BUILTIN_RESIZE"))
...
InitDefaultCursor(void)
{
	...
	if (empty_cursor == None) {
		...
		pixmap	    = XCreateBitmapFromData(compositor.display,
			 DefaultRootWindow(compositor.display), no_data, 1, 1);
		color.pixel = 0;
		color.red   = 0;
		color.green = 0;
		color.blue  = 0;
		...
		empty_cursor = XCreatePixmapCursor(compositor.display, pixmap,
...
void *
XLSeatRunAfterResize(Seat *seat, void (*func)(void *, void *), void *data)
{
...
void *
XLSeatRunOnDestroy(Seat *seat, void (*destroy_func)(void *), void *data)
{
...
XLSeatExplicitlyGrabSurface(Seat *seat, Surface *surface, uint32_t serial)
{
	...
	state = XIGrabDevice(compositor.display, seat->master_pointer, window,
	    time, cursor, XIGrabModeAsync, XIGrabModeAsync, True, &mask);
	...
	state = XIGrabDevice(compositor.display, seat->master_keyboard, window,
	    time, None, XIGrabModeAsync, XIGrabModeAsync, False, &mask);
...
void
XLSeatShowWindowMenu(Seat *seat, Surface *surface, int root_x, int root_y)
{
	XEvent msg;
	Window window;
...
ForceEntry(Seat *seat, Window source, double x, double y)
{
	...
	if (seat->last_seen_surface) {
		...
		if (target == None) {
			if (source != target)
				/* If the source is something other than the
				   target, translate the coordinates to the
				   target.  */
				TranslateCoordinates(source, target, x, y, &x,
				    &y);
...
HandleDragSurfaceUnmapped(void *data)
{
	...
	QueryPointer(seat, DefaultRootWindow(compositor.display), &root_x,
	    &root_y);

	/* And cancel the drag with the pointer position.  */

	CancelDrag(seat, DefaultRootWindow(compositor.display), root_x, root_y);
}
...
MakeGrabWindow(void)
{
	...
	window = XCreateWindow(compositor.display,
	    DefaultRootWindow(compositor.display), 0, 0, 1, 1, 0,
	    CopyFromParent, InputOnly, CopyFromParent, CWOverrideRedirect,
	    &attrs);

	/* Clear the input region of the window.  */
	XShapeCombineRectangles(compositor.display, window, ShapeInput, 0, 0,
    ...
    Surface *icon_surface, uint32_t serial)
{
	...
	state = XIGrabDevice(compositor.display, seat->master_pointer,
	    seat->grab_window, time, None, XIGrabModeAsync, XIGrabModeAsync,
	    True, &mask);
	...
	seat->drag_start_unmap_callback = XLSurfaceRunAtUnmap(start_surface,
	    HandleDragSurfaceUnmapped, seat);
...
XLInitSeats(void)
{
	...
	if (XQueryExtension(compositor.display, "XInputExtension", &xi2_opcode,
		...
		if (xi2_major < 2 || (xi2_major == 2 && xi2_minor < 3) || rc) {
			fprintf(stderr,
			    "version 2.3 or later of of the X Input Extension is"
...
XLSeatGetMouseData(Seat *seat, Surface **last_seen_surface,
    double *last_surface_x, double *last_surface_y, double *its_root_x,
    double *its_root_y)
{
...
XLSeatApplyExternalGrab(Seat *seat, Surface *surface)
{
	...
	state = XIGrabDevice(compositor.display, seat->master_keyboard, window,
	    seat->last_focus_time.milliseconds, None, XIGrabModeAsync,
